...
{
	widget_t hMsg = msgdlg_create(text, mid, g_hMain);

	widget_show(hMsg, WS_SHOW_NORMAL);
	widget_set_focus(hMsg);

	int rt = widget_do_modal(hMsg);
...
int DoInput(const tchar_t* title, tchar_t* buf, int max)
{
	...
	widget_t hInput = inputdlg_create(title, buf, max, g_hMain);

	widget_show(hInput, WS_SHOW_NORMAL);

	int rt = widget_do_modal(hInput);
...
bool_t InitInstance(void)
{
   ...
   get_runpath(NULL, szPath, PATH_LEN);
...
void LoadResource()
{
	...
	get_runpath(NULL, path, PATH_LEN);
