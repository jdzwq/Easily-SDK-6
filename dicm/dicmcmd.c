...
bool_t parse_dicm_command_from_stream(dicm_command_t* pcmd, bool_t bigEndian, dword_t maxLength, stream_t stm)
{
	...
	pp.pdicm = dicm_alloc_command();
	pp.pdicm->b_BigEndian = bigEndian;
	pp.pdicm->n_maxLength = maxLength;
	pp.pcmd = pcmd;

	dicm_set_command_read(pp.pdicm, (void*)stm, _on_read);
	dicm_set_command_parse(pp.pdicm, (void*)&pp, NULL, _on_parse_data);

	b = dicm_parse_command(pp.pdicm);
	dicm_free_command(pp.pdicm);
...
bool_t format_dicm_command_to_stream(dicm_command_t* pcmd, bool_t bigEndian, dword_t maxLength, stream_t stm)
{
	...
	fp.pdicm = dicm_alloc_command();
	fp.pdicm->b_BigEndian = bigEndian;
	fp.pdicm->n_maxLength = maxLength;
	fp.pcmd = pcmd;
	fp.mask = pcmd->mask;

	dicm_set_command_write(fp.pdicm, (void*)stm, _on_write);
	dicm_set_command_format(fp.pdicm, (void*)&fp, _on_format_item, _on_format_data);

	b = dicm_format_command(fp.pdicm);
	dicm_free_command(fp.pdicm);
