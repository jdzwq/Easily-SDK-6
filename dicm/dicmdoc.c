...
#else
			bytes = mbs_to_utf8(_T("PIXEL"), -1, NULL, MAX_LONG);
#endif
		else
#if defined(_UNICODE) || defined(UNICODE)
			bytes = ucs_to_mbs(_T("PIXEL"), -1, NULL, MAX_LONG);
#else
			bytes = mbs_to_mbs(_T("PIXEL"), -1, NULL, MAX_LONG);
#endif
...
#else
			bytes = mbs_to_utf8(pta[i].text, -1, NULL, MAX_LONG);
#endif
		else
#if defined(_UNICODE) || defined(UNICODE)
			bytes = ucs_to_mbs(pta[i].text, -1, NULL, MAX_LONG);
#else
			bytes = mbs_to_mbs(pta[i].text, -1, NULL, MAX_LONG);
#endif
		xsprintf(token, _T("%d"), bytes);
		set_dom_node_attr(nlk, _T("VL"), -1, token, -1);
...
#else
			bytes = mbs_to_utf8(token, -1, NULL, MAX_LONG);
#endif
		else
#if defined(_UNICODE) || defined(UNICODE)
			bytes = ucs_to_mbs(token, -1, NULL, MAX_LONG);
#else
			bytes = mbs_to_mbs(token, -1, NULL, MAX_LONG);
#endif
		xsprintf(token, _T("%d"), bytes);
		set_dom_node_attr(nlk, _T("VL"), -1, token, -1);
...
#else
			bytes = mbs_to_utf8(pta[i].align, -1, NULL, MAX_LONG);
#endif
		else
#if defined(_UNICODE) || defined(UNICODE)
			bytes = ucs_to_mbs(pta[i].align, -1, NULL, MAX_LONG);
#else
			bytes = mbs_to_mbs(pta[i].align, -1, NULL, MAX_LONG);
#endif
		xsprintf(token, _T("%d"), bytes);
		set_dom_node_attr(nlk, _T("VL"), -1, token, -1);
...
#else
			bytes = mbs_to_utf8(pga[i].type, -1, NULL, MAX_LONG);
#endif
		else
#if defined(_UNICODE) || defined(UNICODE)
			bytes = ucs_to_mbs(pga[i].type, -1, NULL, MAX_LONG);
#else
			bytes = mbs_to_mbs(pga[i].type, -1, NULL, MAX_LONG);
#endif
		xsprintf(token, _T("%d"), bytes);
		set_dom_node_attr(nlk, _T("VL"), -1, token, -1);
	...
	while (nlk)
	{
...
#if defined(_UNICODE) || defined(UNICODE)
			ucs_to_mbs(tmp, -1, (schar_t*)di.moda, 16);
#else
			mbs_to_mbs(tmp, -1, (schar_t*)di.moda, 16);
#endif
		}
		...
		else if (compare_text(get_dom_node_name_ptr(nlk), -1, _T("PixelSpacing"), -1, 1) == 0) //pixel space
		{
			...
			if (!is_null(tmp))
			{
				...
				if (*tmp != _T('\0'))
					tmp++;
				di.ymm_pixel = (int)(xstof(tmp) * 10000);
			}
		}
...
{
	link_t_ptr nlk;
	tchar_t vf[NUM_LEN + 1];
	tchar_t vl[INT_LEN + 1];
	const tchar_t* vr;
	dword_t len;
	tchar_t* tmp;
	...
	while (nlk)
	{
		...
		{
#if defined(_UNICODE) || defined(UNICODE)
			mbs_to_ucs((schar_t*)di.moda, 16, vf, 16);
#else
			mbs_to_mbs((schar_t*)di.moda, 16, vf, 16);
#endif
