...
static bool_t _on_parse_item(void* pp, dword_t tag, const byte_t* vr, dword_t vl)
{
	...
	tchar_t tvg[KEY_LEN + 1] = { 0 };
	tchar_t tvn[KEY_LEN + 1] = { 0 };
	tchar_t tvt[NUM_LEN + 1] = { 0 };
	tchar_t tvx[INT_LEN + 1] = { 0 };
	tchar_t tvl[INT_LEN + 1] = { 0 };
	tchar_t tvr[3] = { 0 };
...
static bool_t _on_parse_data(void* pp, dword_t tag, const byte_t* vr, const byte_t* vf, dword_t vl)
{
	...
	{
		memcpy((void*)&sl, (void*)vf, sizeof(int));
		tval = xsalloc(NUM_LEN);
		n = ltoxs(sl, tval, NUM_LEN);
	}
	else if (vr[0] == 'U' && vr[1] == 'S') //2
	{
		memcpy((void*)&us, (void*)vf, sizeof(sword_t));
		tval = xsalloc(NUM_LEN);
		n = ustoxs(us, tval, NUM_LEN);
	}
	else if (vr[0] == 'U' && vr[1] == 'L') //4
	{
		memcpy((void*)&ul, (void*)vf, sizeof(dword_t));
		tval = xsalloc(NUM_LEN);
...
#else
			n = utf8_to_mbs(vf, vl, NULL, MAX_LONG);
#endif
			tval = xsalloc(n + 1);
#if defined(_UNICODE) || defined(UNICODE)
			utf8_to_ucs(vf, vl, tval, n);
#else
			utf8_to_mbs(vf, vl, tval, n);
#endif
		}
		else
		{
#if defined(_UNICODE) || defined(UNICODE)
			n = mbs_to_ucs((schar_t*)vf, vl, NULL, MAX_LONG);
#else
			n = mbs_to_mbs((schar_t*)vf, vl, NULL, MAX_LONG);
#endif
			tval = xsalloc(n + 1);
#if defined(_UNICODE) || defined(UNICODE)
			mbs_to_ucs((schar_t*)vf, vl, tval, n);
#else
			mbs_to_mbs((schar_t*)vf, vl, tval, n);
#endif
		}
	}
...
bool_t parse_dicm_doc_from_stream(link_t_ptr ptr, dword_t op, stream_t stm)
{
	...
	if (op & DICM_OPERA_FILEHEAD)
	{
		pp.ph = dicm_alloc_filehead();
		pp.dom = ptr;
		pp.plk = ptr;
		pp.nlk = NULL;

		dicm_set_filehead_read(pp.ph, (void*)stm, _on_read);
		dicm_set_filehead_parse(pp.ph, (void*)&pp, NULL);

		b = dicm_parse_filehead(pp.ph);

		dicm_free_filehead(pp.ph);
	}
...
#else
				*pvl = mbs_to_utf8(get_dom_node_text_ptr(pa->nlk), -1, NULL, MAX_LONG);
#endif
			else
#if defined(_UNICODE) || defined(UNICODE)
				*pvl = ucs_to_mbs(get_dom_node_text_ptr(pa->nlk), -1, NULL, MAX_LONG);
#else
				*pvl = mbs_to_mbs(get_dom_node_text_ptr(pa->nlk), -1, NULL, MAX_LONG);
#endif
		}
	}
...
static bool_t _on_format_data(void* fp, dword_t* ptag, byte_t* pvr, byte_t* pvf, dword_t* pvl)
{
	...
	if (*pvl)
	{
		...
		{
			sl = xstol(tval);
			memcpy((void*)pvf, (void*)&sl, sizeof(int));
		}
		else if (pvr[0] == 'U' && pvr[1] == 'S') //2
		{
			us = xstous(tval);
			memcpy((void*)pvf, (void*)&us, sizeof(sword_t));
		}
		else if (pvr[0] == 'U' && pvr[1] == 'L') //4
		{
			ul = xstoul(tval);
			memcpy((void*)pvf, (void*)&ul, sizeof(dword_t));
		}
		else if ((pvr[0] == 'O' && pvr[1] == 'B') || (pvr[0] == 'O' && pvr[1] == 'W'))
		{
...
#else
				mbs_to_utf8(tval, -1, pvf, *pvl);
#endif
			}
			else
			{
#if defined(_UNICODE) || defined(UNICODE)
				ucs_to_mbs(tval, -1, (schar_t*)pvf, *pvl);
#else
				mbs_to_mbs(tval, -1, (schar_t*)pvf, *pvl);
#endif
			}
		}
	}
...
bool_t format_dicm_doc_to_stream(link_t_ptr ptr, dword_t op, stream_t stm)
{
	...
	if (op & DICM_OPERA_FILEHEAD)
	{
		fp.ph = dicm_alloc_filehead();
		fp.dom = ptr;
		fp.plk = ptr;
		fp.nlk = NULL;

		dicm_set_filehead_write(fp.ph, (void*)stm, _on_write);
		dicm_set_filehead_format(fp.ph, (void*)&fp, NULL);

		b = dicm_format_filehead(fp.ph);

		dicm_free_filehead(fp.ph);
	}
	else if (op & DICM_OPERA_COMMAND)
	{
