...
# begin GNU MAKE file
# making order:
# 1. make -f xdb_oci.so.mk test --if need to creating some directory or file-list
# 2. make -f xdb_oci.so.mk clean
# 3. make -f xdb_oci.so.mk
# 4. make -f xdb_oci.so.mk install
#-----------------------------------------------------------------------------
CC = clang
...
INC_OCI = /usr/local/oci/include
LIB_OCI = /usr/local/oci/instantclient_23_3
...
DIRS = $(wildcard $(SRC_PATH)/xdb_oci.c)
SRCS = $(notdir $(DIRS))
COBS = $(patsubst %.c, %.o, $(SRCS))
OBJS = $(addprefix $(OBJ_PATH)/,$(COBS))

$(OBJ_PATH)/%.o : $(SRC_PATH)/xdb_oci.c
