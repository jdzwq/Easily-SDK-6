...
# begin GNU MAKE file
# making order:
# 1. make -f xdb_postgre.so.mk test --if need to creating some directory or file-list
# 2. make -f xdb_postgre.so.mk clean
# 3. make -f xdb_postgre.so.mk
# 4. make -f xdb_postgre.so.mk install
#-----------------------------------------------------------------------------
CC = clang
...
INC_PQ = /opt/homebrew/opt/libpq/include
LIB_PQ = /opt/homebrew/opt/libpq/lib
...
LIBS = -lxdk -lxdg -lxdl -L $(LIB_PQ) -lpq
DIRS = $(wildcard $(SRC_PATH)/xdb_postgre.c)
SRCS = $(notdir $(DIRS))
COBS = $(patsubst %.c, %.o, $(SRCS))
OBJS = $(addprefix $(OBJ_PATH)/,$(COBS))

$(OBJ_PATH)/%.o : $(SRC_PATH)/xdb_postgre.c
	$(CC) $(CFLAGS) -c $< -o $@ -I $(INC_PATH) -I $(INC_PQ)
