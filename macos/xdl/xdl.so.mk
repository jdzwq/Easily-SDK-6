...
# begin GNU MAKE file
# making order:
# 1. make -f xdl.so.mk test --if need to creating some directory or file-list
# 2. make -f xdl.so.mk clean
# 3. make -f xdl.so.mk
# 4. make -f xdl.so.mk install
#-----------------------------------------------------------------------------
CC = clang
...
LIBS = -L $(LNK_PATH) -lxdk -lxgc -lcrypt
DIRS = $(wildcard \
	$(SRC_PATH)/*.c \
	$(SRC_PATH)/bag/*.c \
	$(SRC_PATH)/bio/*.c \
