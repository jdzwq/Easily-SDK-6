...
static void _xhttps_get_config(const tchar_t* site, tchar_t* sz_path, tchar_t* sz_track, tchar_t* sz_trace, tchar_t* sz_proc)
{
	...
	while (nlk_site)
	{
		if (compare_text(get_dom_node_name_ptr(nlk_site), -1, site, -1, 1) == 0)
		{
			...
			while (nlk)
			{
				...
				else if (compare_text(get_dom_node_name_ptr(nlk), -1, XPORTD_CONFIG_TRACK, -1, 1) == 0 && sz_track)
				{
					get_dom_node_text(nlk, sz_track, PATH_LEN);
				}
				else if (compare_text(get_dom_node_name_ptr(nlk), -1, XPORTD_CONFIG_TRACE, -1, 1) == 0 && sz_trace)
				{
...
void _xhttps_dispatch(xhand_t http, void* p)
{
	...
	xdk_set_track((PF_TRACK_ERROR)_xhttps_track_error, (void*)pb);
	...
	{
		get_param_item(pxp->sz_param, XPORTD_PARAM_SECURITY, sz_secu, RES_LEN);
		get_param_item(pxp->sz_param, XPORTD_PARAM_SECURITY_VERIFY, sz_verify, RES_LEN);
		get_param_item(pxp->sz_param, XPORTD_PARAM_CA_NAME, sz_ca, RES_LEN);
		...
		get_param_item(pxp->sz_param, XPORTD_PARAM_KEY_PASSWORD, sz_pass, NUM_LEN);

		dw_crt = X509_CERT_SIZE;
        buf_crt = (byte_t*)xmem_alloc(dw_crt);
        
		if (!get_ssl_crt(sz_path, sz_crt, buf_crt, &dw_crt))
		{
		...
		if (!get_ssl_key(sz_path, sz_key, buf_key, &dw_key))
		{
			raise_user_error(_T("_https_invoke"), _T("http get ssl rsa key failed"));
		}

		xssl_set_cert(bio, buf_crt, dw_crt);

		xssl_set_rsa(bio, buf_key, dw_key, sz_pass, -1);
		...
		if (!is_null(sz_verify))
		{
			...
			xssl_set_ca(bio, buf_crt, dw_crt);
		...
		if (compare_text(sz_verify, -1, XPORTD_PARAM_SECURITY_VERIFY_REQUEST, -1, 1) == 0)
			xssl_set_verify(bio, SSL_VERIFY_REQUIRED);
		else if (compare_text(sz_verify, -1, XPORTD_PARAM_SECURITY_VERIFY_OPTIONAL, -1, 1) == 0)
	...
	else if (pxp->n_secu == _SECU_SSH)
	{
		...
		if (!get_ssh_key(sz_path, sz_key, buf_key, &dw_key))
		{
	...
	if (is_null(sz_site))
	{
		...
		_xhttps_get_config(sz_site + 1, sz_path, sz_track, sz_trace, sz_proc);
	}
	else
	{
	...
	get_param_item(pxp->sz_param, XPORTD_PARAM_AUTHORIZATION, sz_auth, INT_LEN);
...
void _xhttps_start(xhttps_param_t* pxp)
{
	...
	get_param_item(pxp->sz_param, XPORTD_PARAM_AUTHORIZATION, sz_auth, RES_LEN);
	...
	if (IS_THREAD_MODE(pxp->sz_mode))
		pxp->lis_http = xhttp_start_thread(port, pxp->n_secu, _xhttps_dispatch, pxp);
	else
		pxp->lis_http = xhttp_start_process(port, pxp->n_secu, pxp->sz_module, pxp->sz_param);
