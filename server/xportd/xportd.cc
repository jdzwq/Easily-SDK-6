...
#include "../xtcps/xtcps.h"
#include "../xudps/xudps.h"
...
void xportd_start()
{
	...
	while (nlk_port)
	{
		if (compare_text(get_dom_node_attr_ptr(nlk_port, XPORTD_PORT_ATTR_TYPE, -1), -1, XPORTD_PORT_ATTR_TYPE_HTTP, -1, 1) == 0)
		{
			xhttps_param_t* phttps = (xhttps_param_t*)xmem_alloc(sizeof(xhttps_param_t));
			
			get_dom_node_attr(nlk_port, XPORTD_PORT_ATTR_BIND, -1, phttps->sz_port, INT_LEN);
			...
			while (nlk_child)
			{
				if (compare_text(get_dom_node_name_ptr(nlk_child), -1, XPORTD_MODE, -1, 1) == 0)
					get_dom_node_text(nlk_child, phttps->sz_mode, INT_LEN);
				else if (compare_text(get_dom_node_name_ptr(nlk_child), -1, XPORTD_MODULE, -1, 1) == 0)
					get_dom_node_text(nlk_child, sz_file, PATH_LEN);
				else if (compare_text(get_dom_node_name_ptr(nlk_child), -1, XPORTD_PARAM, -1, 1) == 0)
				{
					xscpy(phttps->sz_param, _T(""));
					...
					while (nlk_param)
					{
						if (!xsisnil(get_dom_node_text_ptr(nlk_param)))
						{
							...
							xscat(phttps->sz_param, get_dom_node_text_ptr(nlk_param));
							xscat(phttps->sz_param, _T(" "));
						}
		...
		else if (compare_text(get_dom_node_attr_ptr(nlk_port, XPORTD_PORT_ATTR_TYPE, -1), -1, XPORTD_PORT_ATTR_TYPE_TCP, -1, 1) == 0)
		{
			xtcps_param_t* ptcps = (xtcps_param_t*)xmem_alloc(sizeof(xtcps_param_t));

			get_dom_node_attr(nlk_port, XPORTD_PORT_ATTR_BIND, -1, ptcps->sz_port, INT_LEN);
				...
				else if (compare_text(get_dom_node_name_ptr(nlk_child), -1, XPORTD_PARAM, -1, 1) == 0)
				{
					...
					while (nlk_param)
					{
						if (!xsisnil(get_dom_node_text_ptr(nlk_param)))
						{
							...
							xscat(ptcps->sz_param, get_dom_node_text_ptr(nlk_param));
							xscat(ptcps->sz_param, _T(" "));
						}
		...
		else if (compare_text(get_dom_node_attr_ptr(nlk_port, XPORTD_PORT_ATTR_TYPE, -1), -1, XPORTD_PORT_ATTR_TYPE_UDP, -1, 1) == 0)
		{
			xudps_param_t* pudps = (xudps_param_t*)xmem_alloc(sizeof(xudps_param_t));

			get_dom_node_attr(nlk_port, XPORTD_PORT_ATTR_BIND, -1, pudps->sz_port, INT_LEN);
				...
				else if (compare_text(get_dom_node_name_ptr(nlk_child), -1, XPORTD_PARAM, -1, 1) == 0)
				{
					...
					while (nlk_param)
					{
						if (!xsisnil(get_dom_node_text_ptr(nlk_param)))
						{
							...
							xscat(pudps->sz_param, get_dom_node_text_ptr(nlk_param));
							xscat(pudps->sz_param, _T(" "));
						}
