...
/**********************************************************************************************************************/
void _xtcps_dispatch(xhand_t bio, void* p)
{
	...
	xdk_set_track((PF_TRACK_ERROR)_xtcps_track_error, (void*)pb);
...
void _xtcps_start(xtcps_param_t* pxp)
{
	...
	if (IS_THREAD_MODE(pxp->sz_mode))
		pxp->lis_tcp = xtcp_start_thread(port, pxp->n_secu, _xtcps_dispatch, pxp);
	else
		pxp->lis_tcp = xtcp_start_process(port, pxp->n_secu, pxp->sz_module, pxp->sz_param);
