...
/***********************************************************************
	...
	@module	xtimerd.cc | timer dispatch implement file
...
void STDCALL thread_timer_proc(void* param, res_timer_t tid)
{
...
void STDCALL process_timer_proc(void* param, res_timer_t tid)
{
	xtimerd_param_t* ptimer = (xtimerd_param_t*)param;
	...
	for (int i = 0; i < n; i++)
	{
		...
		if (dure)
		{
			if (compare_text(ptimer->mode, -1, _T("thread"), -1, 1) == 0)
				ptimer->timer = create_timer(g_queue, dure, dure, (PF_TIMERFUNC)thread_timer_proc, (void*)ptimer);
			else
				ptimer->timer = create_timer(g_queue, dure, dure, (PF_TIMERFUNC)process_timer_proc, (void*)ptimer);
		}
		else
		{
			ptimer->timer = NULL;
