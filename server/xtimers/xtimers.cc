...
bool_t xtimers_config(const tchar_t* task, tchar_t* tname, tchar_t* tpath, tchar_t* tproc, tchar_t* ttrack)
{
	...
	while (nlk)
	{
		if (compare_text(get_dom_node_name_ptr(nlk), -1, XTIMERD_CONFIG_NAME, -1, 1) == 0 && tname)
		{
			get_dom_node_text(nlk, tname, PATH_LEN);
		}
		else if (compare_text(get_dom_node_name_ptr(nlk), -1, XTIMERD_CONFIG_PATH, -1, 1) == 0 && tpath)
		{
			get_dom_node_text(nlk, tpath, PATH_LEN);
		}
		else if (compare_text(get_dom_node_name_ptr(nlk), -1, XTIMERD_CONFIG_PROC, -1, 1) == 0 && tproc)
		{
			get_dom_node_text(nlk, tproc, PATH_LEN);
		}
		else if (compare_text(get_dom_node_name_ptr(nlk), -1, XTIMERD_CONFIG_TRACK, -1, 1) == 0 && ttrack)
		{
...
void xtimers_dispatch(xtimers_param_t* pts)
{
	...
	xdk_set_track((PF_TRACK_ERROR)_xtimers_track_error, (void*)ptb);
