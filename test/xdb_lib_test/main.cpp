...
unsigned int STDCALL test_xdb_schema(void* param)
{
    ...
    {
#ifdef _UNICODE
		wprintf(L"%s %s (%d, %d)\n", get_col_name_ptr(clk), get_col_data_type_ptr(clk), get_col_data_len(clk), get_col_data_dig(clk));
#else
		printf("%s %s (%d, %d)\n", get_col_name_ptr(clk), get_col_data_type_ptr(clk), get_col_data_len(clk), get_col_data_dig(clk));
#endif
...
unsigned int STDCALL test_xdb_select(void* param)
{
    ...
    if(!clk1) clk1 = get_col(grid, _T("DID"));
    clk2 = get_col(grid, _T("dname"));
    if(!clk2) clk2 = get_col(grid, _T("DNAME"));
    clk3 = get_col(grid, _T("dage"));
    if(!clk3) clk3 = get_col(grid, _T("DAGE"));
    clk4 = get_col(grid, _T("dprice"));
    if(!clk4) clk4 = get_col(grid, _T("DPRICE"));
...
unsigned int STDCALL test_xdb_proc(void* param)
{
    ...
    set_func_name(func, _T("dog_price"));
	...
	n = get_func_return_integer(func);
#ifdef _UNICODE
    wprintf(L"%d\n",n);
#else
    printf("%d\n", n);
