...
void ctr_drbg_set_entropy_len(ctr_drbg_context *ctx, uint32_t len)
{
	ctx->entropy_len = len;
}
	...
	const unsigned char *data, uint32_t data_len)
{
	...
	memset((void*)buf, 0, CTR_DRBG_MAX_SEED_INPUT + CTR_DRBG_BLOCKSIZE + 16);
	aes_init(&aes_ctx);
	...
	*/
	...
	{
		p = buf;
		memset((void*)chain, 0, CTR_DRBG_BLOCKSIZE);
		use_len = buf_len;
...
exit:
	...
	*/
	...
	if (0 != ret)
	{
		...
		*/
		memset((void*)output, 0, CTR_DRBG_SEEDLEN);
	}
	...
	const unsigned char data[CTR_DRBG_SEEDLEN])
{
	...
	memset((void*)tmp, 0, CTR_DRBG_SEEDLEN);
...
exit:
	memset((void*)add_input, 0, sizeof(add_input));
	return(ret);
}
	...
	const unsigned char *additional, uint32_t len)
{
	...
	memset((void*)seed, 0, CTR_DRBG_MAX_SEED_INPUT);
	...
	uint32_t len)
{
	...
	memset((void*)key, 0, CTR_DRBG_KEYSIZE);
...
int ctr_drbg_seed_entropy_len(
	...
	int(*f_entropy)(void *, unsigned char *, uint32_t), void *p_entropy,
	const unsigned char *custom, uint32_t len,
	...
	const unsigned char *additional, uint32_t add_len)
{
	...
	memset((void*)add_input, 0, CTR_DRBG_SEEDLEN);
...
int ctr_drbg_random(void *p_rng, unsigned char *output, uint32_t output_len)
{
	int ret;
	ctr_drbg_context *ctx = (ctr_drbg_context *)p_rng;
