...
* DHM Error codes
*/
#define ERR_DHM_BAD_INPUT_DATA                    -0x3080  /**< Bad input parameters. */
#define ERR_DHM_READ_PARAMS_FAILED                -0x3100  /**< Reading of the DHM parameters failed. */
#define ERR_DHM_MAKE_PARAMS_FAILED                -0x3180  /**< Making of the DHM parameters failed. */
#define ERR_DHM_READ_PUBLIC_FAILED                -0x3200  /**< Reading of the public values failed. */
#define ERR_DHM_MAKE_PUBLIC_FAILED                -0x3280  /**< Making of the public value failed. */
#define ERR_DHM_CALC_SECRET_FAILED                -0x3300  /**< Calculation of the DHM secret failed. */
#define ERR_DHM_INVALID_FORMAT                    -0x3380  /**< The ASN.1 data is not formatted correctly. */
#define ERR_DHM_ALLOC_FAILED                      -0x3400  /**< Allocation of memory failed. */
#define ERR_DHM_FILE_IO_ERROR                     -0x3480  /**< Read or write of file failed. */

/* ERR_DHM_HW_ACCEL_FAILED is deprecated and should not be used. */
#define ERR_DHM_HW_ACCEL_FAILED                   -0x3500  /**< DHM hardware accelerator failed. */

#define ERR_DHM_SET_GROUP_FAILED                  -0x3580  /**< Setting the modulus and generator failed. */

/**
* \brief          The DHM context structure.
*/
typedef struct dhm_context
{
	uint32_t len;         /*!<  The size of \p P in Bytes. */
	mpi P;      /*!<  The prime modulus. */
	mpi G;      /*!<  The generator. */
	mpi X;      /*!<  Our secret value. */
	mpi GX;     /*!<  Our public key = \c G^X mod \c P. */
	mpi GY;     /*!<  The public key of the peer = \c G^Y mod \c P. */
	mpi K;      /*!<  The shared secret = \c G^(XY) mod \c P. */
	mpi RP;     /*!<  The cached value = \c R^2 mod \c P. */
	mpi Vi;     /*!<  The blinding value. */
	mpi Vf;     /*!<  The unblinding value. */
	mpi pX;     /*!<  The previous \c X. */
	}
dhm_context;
...
extern "C" {
	...
	*/
	OEM_EXP_API void dhm_init(dhm_context *ctx);
	...
	* \return         An \c ERR_DHM_XXX error code on failure.
	*/
	OEM_EXP_API int dhm_read_params(dhm_context *ctx,
		unsigned char **p,
	...
	*                 dhm_set_group() below in conjunction with
	*                 mpi_read_binary() and mpi_read_string().
	*
	...
	*                 mpi_size() to computing the byte-size of an MPI.
	...
	* \return         An \c ERR_DHM_XXX error code on failure.
	*/
	OEM_EXP_API int dhm_make_params(dhm_context *ctx, int x_size,
	...
	*                 in preparation for dhm_make_params().
	*
	...
	* \return         An \c ERR_DHM_XXX error code on failure.
	*/
	OEM_EXP_API int dhm_set_group(dhm_context *ctx,
		const mpi *P,
		const mpi *G);
	...
	*                 its DHM parameters set, e.g. via dhm_set_group().
	...
	* \return         An \c ERR_DHM_XXX error code on failure.
	*/
	OEM_EXP_API int dhm_read_public(dhm_context *ctx,
		const unsigned char *input, uint32_t ilen);
	OEM_EXP_API int dhm_make_public_size(dhm_context *ctx, int x_size);
	/**
	...
	* \return         An \c ERR_DHM_XXX error code on failure.
	*/
	OEM_EXP_API int dhm_make_public(dhm_context *ctx, int x_size,
		unsigned char *output, uint32_t olen,
		int(*f_rng)(void *, unsigned char *, uint32_t),
		void *p_rng);
	...
	* \return              An \c ERR_DHM_XXX error code on failure.
	*/
	OEM_EXP_API int dhm_calc_secret(dhm_context *ctx,
	...
	*/
	OEM_EXP_API void dhm_free(dhm_context *ctx);
	...
	*
	* \return            \c 0 on success.
	* \return            An \c ERR_DHM_XXX or \c ERR_PEM_XXX error
	*                    code on failure.
	*/
	OEM_EXP_API int dhm_parse_dhm(dhm_context *dhm, const unsigned char *dhmin,
		uint32_t dhminlen);
	...
	*/
	OEM_EXP_API int dhm_self_test(int verbose);
...
* IETF Standards</em>.
*/
#define DHM_RFC5114_MODP_2048_P                         \
    DEPRECATED_STRING_CONSTANT(                         \
...
*/
#define DHM_RFC5114_MODP_2048_G                         \
    DEPRECATED_STRING_CONSTANT(                         \
...
*
*/
#define DHM_RFC3526_MODP_2048_P                         \
    DEPRECATED_STRING_CONSTANT(                         \
...
* Diffie-Hellman groups for Internet Key Exchange (IKE)</em>.
*/
#define DHM_RFC3526_MODP_2048_G                         \
    DEPRECATED_STRING_CONSTANT( "02" )
...
* Diffie-Hellman groups for Internet Key Exchange (IKE)</em>.
*/
#define DHM_RFC3526_MODP_3072_P                         \
    DEPRECATED_STRING_CONSTANT(                         \
        "FFFFFFFFFFFFFFFFC90FDAA22168C234C4C6628B80DC1CD1"      \
        "29024E088A67CC74020BBEA63B139B22514A08798E3404DD"      \
        "EF9519B3CD3A431B302B0A6DF25F14374FE1356D6D51C245"      \
...
* Diffie-Hellman groups for Internet Key Exchange (IKE)</em>.
*/
#define DHM_RFC3526_MODP_3072_G                      \
    DEPRECATED_STRING_CONSTANT( "02" )
...
* Diffie-Hellman groups for Internet Key Exchange (IKE)</em>.
*/
#define DHM_RFC3526_MODP_4096_P                      \
    DEPRECATED_STRING_CONSTANT(                      \
        "FFFFFFFFFFFFFFFFC90FDAA22168C234C4C6628B80DC1CD1"   \
        "29024E088A67CC74020BBEA63B139B22514A08798E3404DD"   \
        "EF9519B3CD3A431B302B0A6DF25F14374FE1356D6D51C245"   \
...
* Diffie-Hellman groups for Internet Key Exchange (IKE)</em>.
*/
#define DHM_RFC3526_MODP_4096_G                      \
    DEPRECATED_STRING_CONSTANT( "02" )
...
#define DHM_RFC3526_MODP_2048_P_BIN {        \
...
#define DHM_RFC3526_MODP_2048_G_BIN { 0x02 }

#define DHM_RFC3526_MODP_3072_P_BIN {       \
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, \
    0xC9, 0x0F, 0xDA, 0xA2, 0x21, 0x68, 0xC2, 0x34, \
    0xC4, 0xC6, 0x62, 0x8B, 0x80, 0xDC, 0x1C, 0xD1, \
    0x29, 0x02, 0x4E, 0x08, 0x8A, 0x67, 0xCC, 0x74, \
...
#define DHM_RFC3526_MODP_3072_G_BIN { 0x02 }

#define DHM_RFC3526_MODP_4096_P_BIN  {       \
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,  \
    0xC9, 0x0F, 0xDA, 0xA2, 0x21, 0x68, 0xC2, 0x34,  \
    0xC4, 0xC6, 0x62, 0x8B, 0x80, 0xDC, 0x1C, 0xD1,  \
    0x29, 0x02, 0x4E, 0x08, 0x8A, 0x67, 0xCC, 0x74,  \
...
#define DHM_RFC3526_MODP_4096_G_BIN { 0x02 }

#define DHM_RFC7919_FFDHE2048_P_BIN {        \
     0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, \
...
#define DHM_RFC7919_FFDHE2048_G_BIN { 0x02 }

#define DHM_RFC7919_FFDHE3072_P_BIN { \
     0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, \
     0xAD, 0xF8, 0x54, 0x58, 0xA2, 0xBB, 0x4A, 0x9A, \
     0xAF, 0xDC, 0x56, 0x20, 0x27, 0x3D, 0x3C, 0xF1, \
     0xD8, 0xB9, 0xC5, 0x83, 0xCE, 0x2D, 0x36, 0x95, \
...
#define DHM_RFC7919_FFDHE3072_G_BIN { 0x02 }

#define DHM_RFC7919_FFDHE4096_P_BIN {        \
     0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, \
     0xAD, 0xF8, 0x54, 0x58, 0xA2, 0xBB, 0x4A, 0x9A, \
     0xAF, 0xDC, 0x56, 0x20, 0x27, 0x3D, 0x3C, 0xF1, \
     0xD8, 0xB9, 0xC5, 0x83, 0xCE, 0x2D, 0x36, 0x95, \
...
#define DHM_RFC7919_FFDHE4096_G_BIN { 0x02 }

#define DHM_RFC7919_FFDHE6144_P_BIN {        \
     0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, \
     0xAD, 0xF8, 0x54, 0x58, 0xA2, 0xBB, 0x4A, 0x9A, \
     0xAF, 0xDC, 0x56, 0x20, 0x27, 0x3D, 0x3C, 0xF1, \
     0xD8, 0xB9, 0xC5, 0x83, 0xCE, 0x2D, 0x36, 0x95, \
...
#define DHM_RFC7919_FFDHE6144_G_BIN { 0x02 }

#define DHM_RFC7919_FFDHE8192_P_BIN {        \
     0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, \
     0xAD, 0xF8, 0x54, 0x58, 0xA2, 0xBB, 0x4A, 0x9A, \
     0xAF, 0xDC, 0x56, 0x20, 0x27, 0x3D, 0x3C, 0xF1, \
     0xD8, 0xB9, 0xC5, 0x83, 0xCE, 0x2D, 0x36, 0x95, \
...
#define DHM_RFC7919_FFDHE8192_G_BIN { 0x02 }
