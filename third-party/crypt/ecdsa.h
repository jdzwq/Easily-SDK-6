 ...
 */
OEM_EXP_API int ecdsa_sign(ecp_group *grp, mpi *r, mpi *s,
                const mpi *d, const unsigned char *buf, uint32_t blen,
 ...
 */
OEM_EXP_API int ecdsa_sign_det(ecp_group *grp, mpi *r,
                            mpi *s, const mpi *d,
 ...
 *                  error code on failure.
 */
OEM_EXP_API int ecdsa_sign_det_ext(ecp_group *grp, mpi *r,
                                mpi *s, const mpi *d,
                                const unsigned char *buf, uint32_t blen,
                                md_type_t md_alg,
 ...
 */
OEM_EXP_API int ecdsa_verify(ecp_group *grp,
                          const unsigned char *buf, uint32_t blen,
 ...
 */
OEM_EXP_API int ecdsa_write_signature(ecdsa_context *ctx,
                                   md_type_t md_alg,
                           const unsigned char *hash, uint32_t hlen,
 ...
 *                  \c ERR_ASN1_XXX error code on failure.
 */
OEM_EXP_API int ecdsa_write_signature_restartable(ecdsa_context *ctx,
                           md_type_t md_alg,
                           const unsigned char *hash, uint32_t hlen,
                           unsigned char *sig, uint32_t *slen,
                           int (*f_rng)(void *, unsigned char *, uint32_t),
 ...
 *                  error code on failure for any other reason.
 */
OEM_EXP_API int ecdsa_read_signature(ecdsa_context *ctx,
                          const unsigned char *hash, uint32_t hlen,
 ...
 *                  error code on failure for any other reason.
 */
OEM_EXP_API int ecdsa_read_signature_restartable(ecdsa_context *ctx,
                          const unsigned char *hash, uint32_t hlen,
                          const unsigned char *sig, uint32_t slen,
                          ecdsa_restart_ctx *rs_ctx );
 ...
 */
OEM_EXP_API int ecdsa_genkey(ecdsa_context *ctx, ecp_group_id gid,
                  int (*f_rng)(void *, unsigned char *, uint32_t), void *p_rng );
 ...
 * \return          An \c ERR_ECP_XXX code on failure.
 */
OEM_EXP_API int ecdsa_from_keypair(ecdsa_context *ctx,
 ...
 */
OEM_EXP_API void ecdsa_init(ecdsa_context *ctx);
 ...
 */
OEM_EXP_API void ecdsa_free(ecdsa_context *ctx);
