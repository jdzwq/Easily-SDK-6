...
void ecp_group_free(ecp_group *grp)
{
	...
	memset((void*)grp, 0, sizeof(ecp_group));
}
...
*/
static void ecp_mpi_load(mpi *X, const mpi_uint *p, uint32_t len)
{
	X->s = 1;
...
static int _ecp_group_load(ecp_group *grp,
	const mpi_uint *p, uint32_t plen,
	const mpi_uint *a, uint32_t alen,
	const mpi_uint *b, uint32_t blen,
	const mpi_uint *gx, uint32_t gxlen,
	const mpi_uint *gy, uint32_t gylen,
	const mpi_uint *n, uint32_t nlen)
{
...
*/
static int fix_negative(mpi *N, signed char c, mpi *C, uint32_t bits)
{
	int ret;
...
#define INIT( b )                                                       \
    ...
    C.p = Cp;                                                           \
    memset((void*) Cp, 0, C.n * sizeof( mpi_uint ) );                  \
                                                                        \
...
static int ecp_mod_koblitz(mpi *N, mpi_uint *Rp, uint32_t p_limbs,
	uint32_t adjust, uint32_t shift, mpi_uint mask)
{
	int ret;
	uint32_t i;
	mpi M, R;
...
int ecp_point_read_binary(const ecp_group *grp,
	ecp_point *pt,
	const unsigned char *buf, uint32_t ilen)
{
	int ret;
	uint32_t plen;
...
int ecp_tls_read_point(const ecp_group *grp,
	ecp_point *pt,
	const unsigned char **buf, uint32_t buf_len)
{
...
*/
int ecp_tls_write_group(const ecp_group *grp, uint32_t *olen,
	unsigned char *buf, uint32_t blen)
{
	const ecp_curve_info *curve_info;
	...
	ecp_point *T[], uint32_t T_size)
{
	...
	if ((c = malloc(T_size * sizeof(mpi))) == NULL)
	{
		return (int)(-1);
	}
...
*/
static void ecp_comb_recode_core(unsigned char x[], uint32_t d,
...
static int ecp_mul_comb_core(const ecp_group *grp, ecp_point *R,
	const ecp_point T[], unsigned char T_size,
	const unsigned char x[], uint32_t d,
	int(*f_rng)(void *, unsigned char *, uint32_t),
	void *p_rng,
	ecp_restart_ctx *rs_ctx)
{
	int ret;
	...
	ecp_restart_ctx *rs_ctx)
{
	...
	{
		T = malloc(T_size * sizeof(ecp_point));
		if (T == NULL)
		{
			ret = (int)(-1);
			goto cleanup;
...
static int group_from_specified(const asn1_buf *params, ecp_group *grp)
{
	...
	*/
	...
	if (len != OID_SIZE(OID_ANSI_X9_62_PRIME_FIELD) || memcmp(p, OID_ANSI_X9_62_PRIME_FIELD, len) != 0)
	{
		return (int)(-1);
	}
