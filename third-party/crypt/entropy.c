...
int entropy_func(void *data, unsigned char *output, uint32_t len)
{
...
static int entropy_source_self_test_gather(unsigned char *buf, uint32_t buf_len)
{
	int ret = 0;
	uint32_t entropy_len = 0;
	uint32_t olen = 0;
	uint32_t attempts = buf_len;
