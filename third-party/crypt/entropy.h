...
extern "C" {
	...
	*/
	typedef int(*entropy_f_source_ptr)(void *data, unsigned char *output, uint32_t len,
		uint32_t *olen);
	...
	*/
	typedef struct entropy_source_state
	{
		...
		uint32_t          size;       /**< Amount received in bytes */
		uint32_t          threshold;  /**< Minimum bytes required before release */
	...
	*/
	OEM_EXP_API void entropy_init(entropy_context *ctx);
	...
	*/
	OEM_EXP_API void entropy_free(entropy_context *ctx);
	...
	*/
	OEM_EXP_API int entropy_add_source(entropy_context *ctx,
		entropy_f_source_ptr f_source, void *p_source,
		uint32_t threshold, int strong);
	...
	*/
	OEM_EXP_API int entropy_gather(entropy_context *ctx);
	...
	* \return          0 if successful, or ERR_ENTROPY_SOURCE_FAILED
	*/
	OEM_EXP_API int entropy_func(void *data, unsigned char *output, uint32_t len);
	...
	*/
	OEM_EXP_API int entropy_update_manual(entropy_context *ctx,
	...
	*/
	OEM_EXP_API int entropy_self_test(int verbose);
#endif /* SELF_TEST */
