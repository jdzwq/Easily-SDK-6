...
#define ENTROPY_MIN_HARDWARE     32     /**< Minimum for the hardware source */
	...
	*/
	OEM_EXP_API int null_entropy_poll(void *data,
		unsigned char *output, uint32_t len, uint32_t *olen);

	/**
	* \brief           Platform-specific entropy poll callback
	*/
	OEM_EXP_API int platform_entropy_poll(void *data,
		unsigned char *output, uint32_t len, uint32_t *olen);
	...
	*/
	OEM_EXP_API int havege_poll(void *data,
		unsigned char *output, uint32_t len, uint32_t *olen);

	/**
	* \brief           timing_hardclock-based entropy poll callback
	*/
	OEM_EXP_API int hardclock_poll(void *data,
		unsigned char *output, uint32_t len, uint32_t *olen);
