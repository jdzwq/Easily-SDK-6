...
extern "C" {
	...
	*/
	OEM_EXP_API int gcm_setkey(gcm_context *ctx,
	...
	*/
	OEM_EXP_API int gcm_crypt_and_tag(gcm_context *ctx,
		int mode,
		uint32_t length,
	...
	*                  failed.
	*/
	OEM_EXP_API int gcm_auth_decrypt(gcm_context *ctx,
		uint32_t length,
		const unsigned char *iv,
		uint32_t iv_len,
		const unsigned char *add,
	...
	* \return          \c 0 on success.
	*/
	OEM_EXP_API int gcm_starts(gcm_context *ctx,
		const unsigned char *iv,
		uint32_t iv_len,
		const unsigned char *add,
		uint32_t add_len);
	...
	*/
	OEM_EXP_API int gcm_update(gcm_context *ctx,
		uint32_t length,
		const unsigned char *input,
		unsigned char *output);
	...
	* \return          #ERR_GCM_BAD_INPUT on failure.
	*/
	OEM_EXP_API int gcm_finish(gcm_context *ctx,
		unsigned char *tag,
		uint32_t tag_len);
	...
	*/
	OEM_EXP_API int gcm_self_test(int verbose);
