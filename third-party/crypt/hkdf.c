...
int hkdf(const md_info_t *md, const unsigned char *salt,
	...
	unsigned char *okm, uint32_t okm_len)
{
	int ret;
...
int hkdf_expand(const md_info_t *md, const unsigned char *prk,
	...
	uint32_t info_len, unsigned char *okm, uint32_t okm_len)
{
	uint32_t hash_len;
	uint32_t where = 0;
	uint32_t n;
...
static int parse_octet_string(const char* octet, int len, unsigned char* buf, uint32_t max)
{
	uint32_t pos = 0;
	int total = 0;
	unsigned char bh, bl;

	if (len < 0) len = strlen(octet);
...
static int format_octet_string(const unsigned char* octet, uint32_t len, int upper, char* buf, int max)
{
	int pos = 0;
	uint32_t total = 0;
	unsigned char bh, bl;
...
void test_hkdf(int verbos)
{
	...
	for (i = 0; i < 6; i++)
	{
		...
		ikm_len = parse_octet_string(hkdf_case[i].ikm.octet, (hkdf_case[i].ikm.size + 1) * 2, ikm, 128);
		salt_len = parse_octet_string(hkdf_case[i].salt.octet, (hkdf_case[i].salt.size + 1) * 2, salt, 128);
		info_len = parse_octet_string(hkdf_case[i].info.octet, (hkdf_case[i].info.size + 1) * 2, info, 128);

		memset((void*)prk, 0, 128);
		...
		prk_octet[0] = '0';
		prk_octet[1] = 'x';
		prk_len = format_octet_string(prk, hkdf_case[i].prk.size, 0, (prk_octet + 2), 256);

		ret = memcmp((void*)hkdf_case[i].prk.octet, (void*)prk_octet, prk_len + 2);
		if (ret != 0)
		{
		...
		okm_octet[0] = '0';
		okm_octet[1] = 'x';
		okm_len = format_octet_string(okm, hkdf_case[i].okm.size, 0, (okm_octet + 2), 256);

		ret = memcmp((void*)hkdf_case[i].okm.octet, (void*)okm_octet, okm_len + 2);
		if (ret != 0)
		{
