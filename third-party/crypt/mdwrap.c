...
int md_update(const md_info_t *md_info, void *ctx, const unsigned char *input, uint32_t ilen)
{
	switch (md_info->type)
	{
	case MD_MD2:
...
int md_finish(const md_info_t *md_info, void *ctx, unsigned char *output)
{
	switch (md_info->type)
	{
	case MD_MD2:
...
int md(const md_info_t *md_info, const unsigned char *input, uint32_t ilen,
...
int md_hmac(const md_info_t *md_info, const unsigned char *key, uint32_t keylen,
...
int md_hmac_starts(const md_info_t *md_info, void *ctx, const unsigned char *key, uint32_t keylen)
{
	switch (md_info->type)
	{
	case MD_MD2:
...
int md_hmac_update(const md_info_t *md_info, void *ctx, const unsigned char *input, uint32_t ilen)
{
	switch (md_info->type)
	{
	case MD_MD2:
...
int md_hmac_finish(const md_info_t *md_info, void *ctx, unsigned char *output)
{
	switch (md_info->type)
	{
	case MD_MD2:
...
void* md_alloc(const md_info_t *md_info)
{
	...
	{
	case MD_MD2:
		ctx = malloc(sizeof(md2_context));
		md2_init((md2_context*)ctx);
		return ctx;
	case MD_MD4:
		ctx = malloc(sizeof(md4_context));
	...
	case MD_MD5:
		...
		return ctx;
	case MD_RIPEMD160:
		ctx = malloc(sizeof(ripemd160_context));
