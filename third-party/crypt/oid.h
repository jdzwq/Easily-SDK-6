...
typedef struct oid_descriptor_t
{
	...
	uint32_t asn1_len;                /*!< length of asn1                 */
 ...
 */
OEM_EXP_API int oid_get_numeric_string(char *buf, uint32_t size, const asn1_buf *oid);
 ...
 */
OEM_EXP_API int oid_get_x509_ext_type(const asn1_buf *oid, int *ext_type);
 ...
 * \return         0 if successful, or ERR_OID_NOT_FOUND
 */
OEM_EXP_API int oid_get_attr_short_name(const asn1_buf *oid, const char **short_name);
 ...
 * \return         0 if successful, or ERR_OID_NOT_FOUND
 */
OEM_EXP_API int oid_get_pk_alg(const asn1_buf *oid, pk_type_t *pk_alg);

/**
 * \brief          Translate pk_type_t into PublicKeyAlgorithm OID
 *
 ...
 * \return         0 if successful, or ERR_OID_NOT_FOUND
 */
OEM_EXP_API int oid_get_oid_by_pk_alg(pk_type_t pk_alg,
                           const char **oid, uint32_t *olen );
 ...
 * \return         0 if successful, or ERR_OID_NOT_FOUND
 */
OEM_EXP_API int oid_get_ec_grp(const asn1_buf *oid, ecp_group_id *grp_id);
 ...
 * \return         0 if successful, or ERR_OID_NOT_FOUND
 */
OEM_EXP_API int oid_get_oid_by_ec_grp(ecp_group_id grp_id,
                           const char **oid, uint32_t *olen );

/**
 * \brief          Translate SignatureAlgorithm OID into md_type_t and pk_type
 *
 * \param oid      OID to use
 ...
 * \return         0 if successful, or ERR_OID_NOT_FOUND
 */
OEM_EXP_API int oid_get_sig_alg(const asn1_buf *oid,
 ...
 * \return         0 if successful, or ERR_OID_NOT_FOUND
 */
OEM_EXP_API int oid_get_sig_alg_desc(const asn1_buf *oid, const char **desc);

/**
 * \brief          Translate md_type_t and pk_type_t into SignatureAlgorithm OID
 *
 ...
 * \return         0 if successful, or ERR_OID_NOT_FOUND
 */
OEM_EXP_API int oid_get_oid_by_sig_alg(pk_type_t pk_alg, md_type_t md_alg,
                            const char **oid, uint32_t *olen );
 ...
 * \return         0 if successful, or ERR_OID_NOT_FOUND
 */
OEM_EXP_API int oid_get_md_alg(const asn1_buf *oid, md_type_t *md_alg);
 ...
 * \return         0 if successful, or ERR_OID_NOT_FOUND
 */
OEM_EXP_API int oid_get_md_hmac(const asn1_buf *oid, md_type_t *md_hmac);
 ...
 * \return         0 if successful, or ERR_OID_NOT_FOUND
 */
OEM_EXP_API int oid_get_extended_key_usage(const asn1_buf *oid, const char **desc);

/**
 * \brief          Translate md_type_t into hash algorithm OID
 *
 * \param md_alg   message digest algorithm
 * \param oid      place to store ASN.1 OID string pointer
 * \param olen     length of the OID
 ...
 * \return         0 if successful, or ERR_OID_NOT_FOUND
 */
OEM_EXP_API int oid_get_oid_by_md(md_type_t md_alg, const char **oid, uint32_t *olen);
 ...
 * \return         0 if successful, or ERR_OID_NOT_FOUND
 */
OEM_EXP_API int oid_get_cipher_alg(const asn1_buf *oid, cipher_type_t *cipher_alg);

/**
 * \brief          Translate PKCS#12 PBE algorithm OID into md_type_t and
 *                 cipher_type
 *
 * \param oid           OID to use
 * \param md_alg        place to store message digest algorithm
 ...
 * \return         0 if successful, or ERR_OID_NOT_FOUND
 */
OEM_EXP_API int oid_get_pkcs12_pbe_alg(const asn1_buf *oid, md_type_t *md_alg, cipher_type_t *cipher_alg);
