...
static uint32_t rsa_get_bitlen(const void *ctx)
{
...
static uint32_t eckey_get_bitlen(const void *ctx)
{
...
int pk_verify_ext(pk_type_t pktype, void *pk_ctx, md_type_t opt_mgf1_md, int opt_salt_len,
	md_type_t md_alg,
	const unsigned char *hash, uint32_t hash_len,
	const unsigned char *sig, uint32_t sig_len)
{
	...
	{
		return (ecdsa_read_signature((ecdsa_context *)pk_ctx, hash, hash_len, sig, sig_len) == 0) ? (-1) : (0);
	}
	else if (pktype == PK_ECKEY || pktype == PK_ECKEY_DH)
	{
		...
		{
			ret = (ecdsa_read_signature(&ecdsa, hash, hash_len, sig, sig_len) == 0) ? (-1) : (0);
		}
...
int pk_sign(pk_type_t pktype, void *pk_ctx, md_type_t md_alg,
	const unsigned char *hash, uint32_t hash_len,
	unsigned char *sig, uint32_t *sig_len,
...
int pk_decrypt(pk_type_t pktype, void *pk_ctx,
	...
	unsigned char *output, uint32_t *olen, uint32_t osize,
	int(*f_rng)(void *, unsigned char *, uint32_t), void *p_rng)
{
	switch (pktype)
	{
