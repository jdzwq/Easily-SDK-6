...
int rsa_test_parse(int verbose)
{
	...
	ret = rsa_parse_key(&rsa, (const unsigned char *)TEST_CA_KEY_RSA_PEM, strlen(TEST_CA_KEY_RSA_PEM), TEST_CA_PWD_RSA_PEM, strlen(TEST_CA_PWD_RSA_PEM));
	if (ret != 0)
	{
		if (verbose != 0)
			printf("failed\n");
...
static int myrand(void *rng_state, unsigned char *output, uint32_t len)
{
