	...
	while (1)
	{
		...
		*/
		...
		while (1)
		{
			...
			cur->next = malloc(sizeof(x509_name));
...
static int x509_parse_int(unsigned char **p, uint32_t n, int *res)
{
	*res = 0;
...
* field.
*/
static int x509_parse_time(unsigned char **p, uint32_t len, uint32_t yearlen,
	x509_time *tm)
{
	int ret;
...
*/
int x509_dn_gets(char *buf, uint32_t size, const x509_name *dn)
{
	int ret;
	uint32_t i, n;
...
* than size characters will be written
*/
int x509_serial_gets(char *buf, uint32_t size, const x509_buf *serial)
{
	int ret;
	uint32_t i, n, nr;
	char *p;
...
*/
int x509_sig_alg_gets(char *buf, uint32_t size, const x509_buf *sig_oid,
...
*/
int x509_key_size_helper(char *buf, uint32_t buf_size, const char *name)
{
	char *p = buf;
	uint32_t n = buf_size;
	int ret;
...
typedef struct {
	const char *name; /* String representation of AttributeType, e.g.
					  * "CN" or "emailAddress". */
	uint32_t name_len;  /* Length of 'name', without trailing 0 byte. */
...
static const x509_attr_descriptor_t *x509_attr_descr_from_name(const char *name, uint32_t name_len)
{
...
*/
int x509_set_extension(asn1_named_data **head, const char *oid, uint32_t oid_len,
	int critical, const unsigned char *val, uint32_t val_len)
{
	asn1_named_data *cur;
...
*  AttributeValue ::= ANY DEFINED BY AttributeType
*/
static int x509_write_name(unsigned char **p, unsigned char *start, asn1_named_data* cur_name)
{
	int ret;
	uint32_t len = 0;
	const char *oid = (const char*)cur_name->oid.p;
	uint32_t oid_len = cur_name->oid.len;
	const unsigned char *name = cur_name->val.p;
	uint32_t name_len = cur_name->val.len;
	...
	x509_sequence *subject_alt_name)
{
	...
	while (*p < end)
	{
		...
		if (cur->buf.p != NULL)
		{
			...
			cur->next = malloc(sizeof(asn1_sequence));
...
*/
int x509_parse_subpubkey(unsigned char **p, const unsigned char *end, pk_type_t* pkalg, void** pk_ctx)
{
	int ret;
	uint32_t len;
	asn1_buf alg_params;
	...
	{
		*pkalg = pk_alg;
		*pk_ctx = malloc(sizeof(rsa_context));
	...
	{
		*pkalg = pk_alg;
		*pk_ctx = malloc(sizeof(ecp_keypair));
...
*/
static int x509_write_ec_private(unsigned char **p, unsigned char *start,
	ecp_keypair *ec)
{
	int ret;
	uint32_t byte_length = (ec->grp.pbits + 7) / 8;
	unsigned char tmp[ECP_MAX_BYTES];
...
exit:
	memset(tmp, 0, byte_length);
	return(ret);
}

int x509_write_pubkey(unsigned char **p, unsigned char *start,
...
int x509_write_pubkey_der(pk_type_t pktype, void *pk_ctx, unsigned char *buf, uint32_t size)
{
	int ret;
	unsigned char *c;
	uint32_t len = 0, par_len = 0, oid_len;
	const char *oid;
...
int x509_write_key_der(pk_type_t pktype, void *pk_ctx, unsigned char *buf, uint32_t size)
{
	int ret;
	unsigned char *c;
	uint32_t len = 0;
...
int x509_write_pubkey_pem(pk_type_t pktype, void *pk_ctx, unsigned char *buf, uint32_t size)
{
	int ret;
...
int x509_write_key_pem(pk_type_t pktype, void *pk_ctx, unsigned char *buf, uint32_t size)
{
	int ret;
