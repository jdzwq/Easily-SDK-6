 ...
 */
OEM_EXP_API int x509_dn_gets(char *buf, uint32_t size, const x509_name *dn);
 ...
 *                 terminated nul byte), or a negative error code.
 */
OEM_EXP_API int x509_serial_gets(char *buf, uint32_t size, const x509_buf *serial);
 ...
 *                 0 otherwise.
 */
OEM_EXP_API int x509_time_is_past(const x509_time *to);
 ...
 *                 0 otherwise.
 */
OEM_EXP_API int x509_time_is_future(const x509_time *from);

/*
 * Internal module functions. You probably do not want to use these unless you
 * know you do.
 */
OEM_EXP_API int x509_get_name(unsigned char **p, const unsigned char *end,
                   x509_name *cur );
OEM_EXP_API int x509_get_alg_null(unsigned char **p, const unsigned char *end,
                       x509_buf *alg );
OEM_EXP_API int x509_get_alg(unsigned char **p, const unsigned char *end,
                  x509_buf *alg, x509_buf *params );
OEM_EXP_API int x509_get_rsassa_pss_params(const x509_buf *params,
                                md_type_t *md_alg, md_type_t *mgf_md,
                                int *salt_len );
OEM_EXP_API int x509_get_sig(unsigned char **p, const unsigned char *end, x509_buf *sig);
OEM_EXP_API int x509_get_sig_alg(const x509_buf *sig_oid, const x509_buf *sig_params,
                      md_type_t *md_alg, pk_type_t *pk_alg,
                      md_type_t* mgf_md, int *salt_len );
OEM_EXP_API int x509_get_time(unsigned char **p, const unsigned char *end,
                   x509_time *t );
OEM_EXP_API int x509_get_serial(unsigned char **p, const unsigned char *end,
                     x509_buf *serial );
OEM_EXP_API int x509_get_ext(unsigned char **p, const unsigned char *end,
                  x509_buf *ext, int tag );
OEM_EXP_API int x509_sig_alg_gets(char *buf, uint32_t size, const x509_buf *sig_oid,
                       pk_type_t pk_alg, md_type_t md_alg,
					   md_type_t mgf_md, int salt_len );
OEM_EXP_API int x509_key_size_helper(char *buf, uint32_t buf_size, const char *name);
OEM_EXP_API int x509_string_to_names(asn1_named_data **head, const char *name);
OEM_EXP_API int x509_set_extension(asn1_named_data **head, const char *oid, uint32_t oid_len,
                        int critical, const unsigned char *val,
                        uint32_t val_len );
OEM_EXP_API int x509_write_extensions(unsigned char **p, unsigned char *start,
                           asn1_named_data *first );
OEM_EXP_API int x509_write_names(unsigned char **p, unsigned char *start,
                      asn1_named_data *first );
OEM_EXP_API int x509_write_sig(unsigned char **p, unsigned char *start,
                    const char *oid, uint32_t oid_len,
                    unsigned char *sig, uint32_t size );
OEM_EXP_API int x509_write_pubkey(unsigned char **p, unsigned char *start,
	pk_type_t pktype, void *pk_ctx);
OEM_EXP_API int x509_write_pubkey_der(pk_type_t pktype, void *pk_ctx, unsigned char *buf, uint32_t size);
OEM_EXP_API int x509_parse_subpubkey(unsigned char **p, const unsigned char *end, 
	pk_type_t* pk_type, void** pk_ctx);
OEM_EXP_API int x509_get_basic_constraints(unsigned char **p,const unsigned char *end,
	int *ca_istrue,
	int *max_pathlen);
OEM_EXP_API int x509_get_key_usage(unsigned char **p,
	const unsigned char *end,
	unsigned int *key_usage);
OEM_EXP_API int x509_get_ext_key_usage(unsigned char **p,
	const unsigned char *end,
	x509_sequence *ext_key_usage);
OEM_EXP_API int x509_get_subject_alt_name(unsigned char **p,
	const unsigned char *end,
	x509_sequence *subject_alt_name);
OEM_EXP_API int x509_get_ns_cert_type(unsigned char **p,
	const unsigned char *end,
	unsigned char *ns_cert_type);
OEM_EXP_API int x509_get_version(unsigned char **p,
	const unsigned char *end,
	int *ver);
OEM_EXP_API int x509_get_dates(unsigned char **p,
	const unsigned char *end,
	x509_time *from,
	x509_time *to);
OEM_EXP_API int x509_get_uid(unsigned char **p,
	const unsigned char *end,
	x509_buf *uid, int n);

#if defined(OEM_SELF_TEST)
OEM_EXP_API int x509_self_test(int verbose);
#endif
