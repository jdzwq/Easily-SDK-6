...
*/
static int x509_memcasecmp(const void *s1, const void *s2, uint32_t len)
{
	uint32_t i;
	unsigned char diff;
...
{
	uint32_t i;
	uint32_t cn_idx = 0, cn_len = strlen(cn);
	...
	uint32_t buflen)
{
	...
	*/
	...
	crt->raw.p = p = malloc(crt->raw.len);
	if (p == NULL)
	{
		return(-1);
	}
	...
	uint32_t buflen)
{
	...
	*/
	...
	{
		crt->next = malloc(sizeof(x509_crt));
...
* list
*/
int x509_crt_parse(x509_crt *chain, const unsigned char *buf, uint32_t buflen)
{
...
static int x509_info_subject_alt_name(char **buf, uint32_t *size,
...
static int x509_info_cert_type(char **buf, uint32_t *size,
...
static int x509_info_key_usage(char **buf, uint32_t *size,
...
static int x509_info_ext_key_usage(char **buf, uint32_t *size,
...
int x509_crt_info(char *buf, uint32_t size, const char *prefix,
	const x509_crt *crt)
{
	int ret;
	uint32_t n;
...
int x509_crt_verify_info(char *buf, uint32_t size, const char *prefix,
	uint32_t flags)
{
	int ret;
...
{
	int flags = 0;
	unsigned char hash[MD_MAX_SIZE];
	...
	x509_crt_restart_ctx *rs_ctx)
{
	...
	for (parent = candidates; parent != NULL; parent = parent->next)
	{
		...
		if (parent->max_pathlen > 0 &&
			(uint32_t)parent->max_pathlen < 1 + path_cnt - self_cnt)
		{
			continue;
		}
...
{
	int ret;
	unsigned char buf[MPI_MAX_SIZE * 2 + 20]; /* tag, length + 2xMPI */
	unsigned char *c = buf + sizeof(buf);
	uint32_t len = 0;
...
static uint32_t crt_get_unused_bits_for_named_bitstring(unsigned char bitstring,
	uint32_t bit_offset)
{
...
int x509write_crt_der(x509write_cert *ctx, unsigned char *buf, uint32_t size,
	...
	void *p_rng)
{
	...
	uint32_t sub_len = 0, pub_len = 0, sig_and_oid_len = 0, sig_len;
	uint32_t len = 0;
	pk_type_t pk_alg;
...
int x509write_crt_pem(x509write_cert *crt, unsigned char *buf, uint32_t size,
	int(*f_rng)(void *, unsigned char *, uint32_t),
	void *p_rng)
{
	int ret;
