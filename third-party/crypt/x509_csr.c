...
*/
int x509_csr_parse(x509_csr *csr, const unsigned char *buf, uint32_t buflen)
{
	int ret;
	uint32_t use_len;
	pem_context pem;
...
*/
int x509_csr_info(char *buf, uint32_t size, const char *prefix,
	const x509_csr *csr)
{
	int ret;
	uint32_t n;
...
static uint32_t csr_get_unused_bits_for_named_bitstring(unsigned char bitstring,
	uint32_t bit_offset)
{
...
int x509write_csr_der(x509write_csr *ctx, unsigned char *buf, uint32_t size,
	...
	void *p_rng)
{
	...
	uint32_t pub_len = 0, sig_and_oid_len = 0, sig_len;
	uint32_t len = 0;
	pk_type_t pk_alg;
...
int x509write_csr_pem(x509write_csr *ctx, unsigned char *buf, uint32_t size,
	int(*f_rng)(void *, unsigned char *, uint32_t),
	void *p_rng)
{
	int ret;
