...
#include "Rmath.h" /* includes config.h, math.h */
#include "dpq.h"
...
#define SET_0_swap   \
	...
	n = 20; /* goto L130; */
	*w1 = bup(b0, a0, y0, x0, n, eps, 0); did_bup = 1;
...
/* ----------------------------------------------------------------------- */
	...
	// warn only when the result seems to matter:
		if ((log_p && !(a*sum > -1. && fabs(log1p(a * sum)) < eps*fabs(ans))) || (!log_p && fabs(a*sum + 1.) != 1.)) { MATHLIB_WARNING; }
    }
    if(log_p) {
	if (a*sum > -1.) ans += log1p(a * sum);
	else {
		if (ans > ML_NEGINF) { MATHLIB_WARNING; }
...
/* -----------------------------------------------------------------------
   ...
   -----------------------------------------------------------------------*/
	...
	return ML_NAN; // TODO: could we know better?
    }
    if (!log_p && brc == 0.) {
	return 0.;
    }
...
/*        CONTINUED FRACTION CALCULATION */
	...
	if (n >= 10000 && fabs(r - r0) > eps * r){ MATHLIB_WARNING; }
