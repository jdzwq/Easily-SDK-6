...
void gen_glyph(FT_Face ft_face, glyph_info_t* pgi)
{
	...
	for (i = 0; i < 16; i++)
	{
		...
		do
		{
			...
			FT_Set_Pixel_Sizes(ft_face, 0, ft_size);

			rt = FT_Load_Char(ft_face, wc, FT_LOAD_RENDER);

			ft_bmp = ft_face->glyph->bitmap;
			xs.w = (ft_bmp.width)? ft_bmp.width : w;
			xs.h = (ft_bmp.rows)? ft_bmp.rows : h;
			...
			while (pt.y < xs.h)
			{
				...
				while (pt.x < xs.w)
				{
					if(ft_bmp.buffer && pt.x < ft_bmp.width)
						ft_clr = ft_bmp.buffer[pt.y * ft_bmp.width + pt.x];
					else
						ft_clr = 0;
...
int main(int argc, const char * argv[]) {
	...
	/*get_runpath(NULL, fpath, 1024);
	xscat(fpath, _T("/ttf/arial.ttf"));
	FT_New_Face(ft_lib, fpath, 0, &ft_face);
	gen_glyph(ft_face, ascii_medium_regular_list);
	FT_Done_Face(ft_face);

	get_runpath(NULL, fpath, 1024);
	xscat(fpath, _T("/ttf/arialb.ttf"));
	FT_New_Face(ft_lib, fpath, 0, &ft_face);
	gen_glyph(ft_face, ascii_bold_regular_list);
	FT_Done_Face(ft_face);

	get_runpath(NULL, fpath, 1024);
	xscat(fpath, _T("/ttf/ariali.ttf"));
	FT_New_Face(ft_lib, fpath, 0, &ft_face);
	gen_glyph(ft_face, ascii_medium_italic_list);
	FT_Done_Face(ft_face);

	get_runpath(NULL, fpath, 1024);
	xscat(fpath, _T("/ttf/arialbi.ttf"));
	FT_New_Face(ft_lib, fpath, 0, &ft_face);
	gen_glyph(ft_face, ascii_bold_italic_list);
	FT_Done_Face(ft_face);*/

	/*get_runpath(NULL, fpath, 1024);
	xscat(fpath, _T("/ttf/arial.ttf"));
	FT_New_Face(ft_lib, fpath, 0, &ft_face);
	gen_glyph(ft_face, gb2312_medium_regular_list);
	FT_Done_Face(ft_face);

	get_runpath(NULL, fpath, 1024);
	xscat(fpath, _T("/ttf/arialb.ttf"));
	FT_New_Face(ft_lib, fpath, 0, &ft_face);
	gen_glyph(ft_face, gb2312_bold_regular_list);
	FT_Done_Face(ft_face);

	get_runpath(NULL, fpath, 1024);
	xscat(fpath, _T("/ttf/ariali.ttf"));
	FT_New_Face(ft_lib, fpath, 0, &ft_face);
	gen_glyph(ft_face, gb2312_medium_italic_list);
	FT_Done_Face(ft_face);

	get_runpath(NULL, fpath, 1024);
	xscat(fpath, _T("/ttf/arialbi.ttf"));
	FT_New_Face(ft_lib, fpath, 0, &ft_face);
	gen_glyph(ft_face, gb2312_bold_italic_list);
	FT_Done_Face(ft_face);*/

	get_runpath(NULL, fpath, 1024);
	xscat(fpath, _T("/ttf/arial.ttf"));
	FT_New_Face(ft_lib, fpath, 0, &ft_face);
	gen_glyph(ft_face, unicode_medium_regular_list);
	FT_Done_Face(ft_face);

	get_runpath(NULL, fpath, 1024);
	xscat(fpath, _T("/ttf/arialb.ttf"));
	FT_New_Face(ft_lib, fpath, 0, &ft_face);
	gen_glyph(ft_face, unicode_bold_regular_list);
	FT_Done_Face(ft_face);

	get_runpath(NULL, fpath, 1024);
	xscat(fpath, _T("/ttf/ariali.ttf"));
	FT_New_Face(ft_lib, fpath, 0, &ft_face);
	gen_glyph(ft_face, unicode_medium_italic_list);
	FT_Done_Face(ft_face);

	get_runpath(NULL, fpath, 1024);
	xscat(fpath, _T("/ttf/arialbi.ttf"));
	FT_New_Face(ft_lib, fpath, 0, &ft_face);
	gen_glyph(ft_face, unicode_bold_italic_list);
	FT_Done_Face(ft_face);
