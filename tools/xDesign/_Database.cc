...
void	_DBShowError(DBCTX* pct, widget_t widget);
...
DBCTX* _CreateDBCtx(void)
{
	...
	pct->pf_db_open = (PF_DB_OPEN)get_address(lib, "db_open");
	pct->pf_db_close = (PF_DB_CLOSE)get_address(lib, "db_close");
	pct->pf_db_exec = (PF_DB_EXEC)get_address(lib, "db_exec");
	pct->pf_db_select = (PF_DB_SELECT)get_address(lib, "db_select");
	pct->pf_db_schema = (PF_DB_SCHEMA)get_address(lib, "db_schema");
	pct->pf_db_fetch = (PF_DB_FETCH)get_address(lib, "db_fetch");
	pct->pf_db_update = (PF_DB_UPDATE)get_address(lib, "db_update");
	pct->pf_db_batch = (PF_DB_BATCH)get_address(lib, "db_batch");
	pct->pf_db_rows = (PF_DB_ROWS)get_address(lib, "db_rows");
	pct->pf_db_call_func = (PF_DB_CALL_FUNC)get_address(lib, "db_call_func");
	pct->pf_db_error = (PF_DB_ERROR)get_address(lib, "db_error");
...
void _DBClose(DBCTX* pct)
{
	XDK_ASSERT(pct && pct->pf_db_close);
...
{
	XDK_ASSERT(pct && pct->pf_db_batch);
	XDK_ASSERT(pct->xdb);
...
bool_t _DBSchema(DBCTX* pct, link_t_ptr grid, const tchar_t* sql)
{
	XDK_ASSERT(pct && pct->pf_db_schema);
	XDK_ASSERT(pct->xdb);
...
bool_t _DBSelect(DBCTX* pct, link_t_ptr grid, const tchar_t* sql)
{
	XDK_ASSERT(pct && pct->pf_db_select);
	XDK_ASSERT(pct->xdb);
...
bool_t _DBFetch(DBCTX* pct, link_t_ptr grid)
{
	XDK_ASSERT(pct && pct->pf_db_fetch);
	XDK_ASSERT(pct->xdb);
...
bool_t _DBUpdate(DBCTX* pct, link_t_ptr grid)
{
	XDK_ASSERT(pct && pct->pf_db_update);
	XDK_ASSERT(pct->xdb);
...
{
	XDK_ASSERT(pct && pct->pf_db_call_func);
	XDK_ASSERT(pct->xdb);
...
int _DBError(DBCTX* pct, tchar_t* buf, int max)
{
	XDK_ASSERT(pct && pct->pf_db_error);
	XDK_ASSERT(pct->xdb);
