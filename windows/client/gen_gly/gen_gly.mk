...
# 1. nmake /f gen_gly.mk test --if need to creating some directory or file-list
# 2. nmake /f gen_gly.mk clean
# 3. nmake /f gen_gly.mk
# 4. nmake /f gen_gly.mk install
#-----------------------------------------------------------------------------
!if EXIST("gen_gly.txt")
!include gen_gly.txt
!endif
...
{$(SRC_PATH)/gen_gly}.cc{$(OBJ_PATH)}.obj::
...
test:
	echo SOURCES= \>$(MODULE).txt
 	for %%i in ($(SRC_PATH)/gen_gly/gen_system.cc) do @echo $(OBJ_PATH)/%%~nxi \>>$(MODULE).txt
