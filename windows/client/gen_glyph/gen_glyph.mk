...
# 1. nmake /f gen_glyph.mk test --if need to creating some directory or file-list
# 2. nmake /f gen_glyph.mk clean
# 3. nmake /f gen_glyph.mk
# 4. nmake /f gen_glyph.mk install
#-----------------------------------------------------------------------------
!if EXIST("gen_glyph.txt")
!include gen_glyph.txt
!endif
...
{$(SRC_PATH)/gen_glyph}.cc{$(OBJ_PATH)}.obj::
...
test:
	echo SOURCES= \>$(MODULE).txt
 	for %%i in ($(SRC_PATH)/gen_glyph/gen_system.cc) do @echo $(OBJ_PATH)/%%~nxi \>>$(MODULE).txt
