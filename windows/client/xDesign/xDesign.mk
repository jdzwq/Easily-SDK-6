...
# making order for debug:
# 1. nmake /f xDesign.mk test --if need to creating some directory or file-list
# 2. nmake /f xDesign.mk clean
# 3. nmake /f xDesign.mk
# 4. nmake /f xDesign.mk
#-----------------------------------------------------------------------------
!if EXIST("xDesign.txt")
!include xDesign.txt
!endif
...
{$(SRC_PATH)/xDesign}.cc{$(OBJ_PATH)}.obj::
...
test:
	echo SOURCES= \>$(MODULE).txt
 	for %%i in ($(SRC_PATH)/xDesign/*.cc) do @echo $(OBJ_PATH)/%%~nxi \>>$(MODULE).txt
