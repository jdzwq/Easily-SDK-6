...
# making order:
# 1. nmake /f crypt.ar.mk test --if need to creating some directory or file-list
# 2. nmake /f crypt.ar.mk clean
# 3. nmake /f crypt.ar.mk
# 4. nmake /f crypt.ar.mk install
#-----------------------------------------------------------------------------
!if EXIST("crypt.txt")
!include crypt.txt
!endif
...
test:
 	...
 	for %i in ($(SRC_PATH)/crypt/*.c) do @echo $(OBJ_PATH)/%i \>>$(MODULE).txt
