...
# 1. nmake /f xService.mk test --if need to creating some directory or file-list
# 2. nmake /f xService.mk clean
# 3. nmake /f xService.mk
# 4. nmake /f xService.mk install
#-----------------------------------------------------------------------------
!if EXIST("xService.txt")
!include xService.txt
!endif
...
LFLAGS = /OUT:"$(TARGET)" /NXCOMPAT /PDB:"$(DATABASE)" \
	...
	/DEBUG /MACHINE:$(ARCH) /LTCG:PGINSTRUMENT /PGD:"$(INSTRUMENT)" /SUBSYSTEM:WINDOWS /ENTRY:mainCRTStartup \
	/LIBPATH:"$(LIB_PATH)" \
...
{$(SRC_PATH)/xService}.cc{$(OBJ_PATH)}.obj::
	$(CC) $(CFLAGS) /I $(INC_PATH) $<
...
test:
	...
	for %%i in ($(SRC_PATH)/xportd/xportd.cc) do @echo $(OBJ_PATH)/%%~nxi \>>$(MODULE).txt
	for %%i in ($(SRC_PATH)/xService/*.cc) do @echo $(OBJ_PATH)/%%~nxi \>>$(MODULE).txt
