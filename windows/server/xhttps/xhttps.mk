...
# 1. nmake /f xhttps.mk test --if need to creating some directory or file-list
# 2. nmake /f xhttps.mk clean
# 3. nmake /f xhttps.mk
# 4. nmake /f xhttps.mk install
#-----------------------------------------------------------------------------
!if EXIST("xhttps.txt")
!include xhttps.txt
!endif
...
test:
	echo SOURCES= \>$(MODULE).txt
 	for %%i in ($(SRC_PATH)/xhttps/*.cc) do @echo $(OBJ_PATH)/%%~nxi \>>$(MODULE).txt
