...
# 1. nmake /f xportm.mk test --if need to creating some directory or file-list
# 2. nmake /f xportm.mk clean
# 3. nmake /f xportm.mk
# 4. nmake /f xportm.mk install
#-----------------------------------------------------------------------------
!if EXIST("xportm.txt")
!include xportm.txt
!endif
...
{$(SRC_PATH)/xportm}.cc{$(OBJ_PATH)}.obj::
...
test:
	echo SOURCES= \>$(MODULE).txt
 	for %%i in ($(SRC_PATH)/xportm/*.cc) do @echo $(OBJ_PATH)/%%~nxi \>>$(MODULE).txt
