...
# 1. nmake /f xtimers.mk test --if need to creating some directory or file-list
# 2. nmake /f xtimers.mk clean
# 3. nmake /f xtimers.mk
# 4. nmake /f xtimers.mk install
#-----------------------------------------------------------------------------
!if EXIST("xtimers.txt")
!include xtimers.txt
!endif
...
test:
	echo SOURCES= \>$(MODULE).txt
 	for %%i in ($(SRC_PATH)/xtimers/*.cc) do @echo $(OBJ_PATH)/%%~nxi \>>$(MODULE).txt
