...
# 1. nmake /f xudps.mk test --if need to creating some directory or file-list
# 2. nmake /f xudps.mk clean
# 3. nmake /f xudps.mk
# 4. nmake /f xudps.mk install
#-----------------------------------------------------------------------------
!if EXIST("xudps.txt")
!include xudps.txt
!endif
...
test:
	echo SOURCES= \>$(MODULE).txt
 	for %%i in ($(SRC_PATH)/xudps/*.cc) do @echo $(OBJ_PATH)/%%~nxi \>>$(MODULE).txt
