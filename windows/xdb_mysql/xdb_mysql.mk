...
# 1. nmake /f xdb_mysql.mk test --if need to creating some directory or file-list
# 2. nmake /f xdb_mysql.mk clean --if no longer debuging
# 3. nmake /f xdb_mysql.mk
# 4. nmake /f xdb_mysql.mk install
#-----------------------------------------------------------------------------
!if EXIST("xdb_mysql.txt")
...
{$(SRC_PATH)/mysql}.c{$(OBJ_PATH)}.obj::
	$(CC) $(CFLAGS) /I $(INC_PATH) /I $(INC_MYSQL) $<
...
test:
 	...
 	for %i in ($(SRC_PATH)/mysql/*.c) do @echo $(OBJ_PATH)/%i \>>$(MODULE).txt
