...
# making order:
# 1. nmake /f xdb_oci.so.mk test --if need to creating some directory or file-list
# 2. nmake /f xdb_oci.so.mk clean --if no longer debuging
# 3. nmake /f xdb_oci.so.mk
# 4. nmake /f xdb_oci.so.mk install
#-----------------------------------------------------------------------------
!if EXIST("xdb_oci.txt")
!include xdb_oci.txt
!endif
...
INC_oci = "D:/oracle/instantclient_19c/sdk/include"
LIB_oci = "D:/oracle/instantclient_19c/sdk/lib/msvc"
...
LFLAGS = /OUT:"$(TARGET)" /MANIFEST /NXCOMPAT /PDB:"$(DATABASE)" \
	...
	/LIBPATH:"$(LIB_PATH)" /LIBPATH:$(LIB_oci) \
...
{$(SRC_PATH)}.c{$(OBJ_PATH)}.obj::
	$(CC) $(CFLAGS) /I $(INC_PATH) /I $(INC_oci) $<
...
test:
 	...
 	for %i in ($(SRC_PATH)/xdb_oci.*) do @echo $(OBJ_PATH)/%i \>>$(MODULE).txt
