...
# making order:
# 1. nmake /f xdb_postgre.so.mk test --if need to creating some directory or file-list
# 2. nmake /f xdb_postgre.so.mk clean --if no longer debuging
# 3. nmake /f xdb_postgre.so.mk
# 4. nmake /f xdb_postgre.so.mk install
#-----------------------------------------------------------------------------
!if EXIST("xdb_postgre.txt")
!include xdb_postgre.txt
!endif

ARCH = x64
MODULE = xdb_postgre

INC_postgre = "D:/pgsql/include"
LIB_postgre = "D:/pgsql/lib"
...
LFLAGS = /OUT:"$(TARGET)" /MANIFEST /NXCOMPAT /PDB:"$(DATABASE)" \
	/DYNAMICBASE "kernel32.lib" "user32.lib" "advapi32.lib" /DEBUG /DLL /MACHINE:$(ARCH) \
	/LTCG:PGINSTRUMENT /PGD:"$(INSTRUMENT)" /SUBSYSTEM:WINDOWS \
	/DEF:"$(MODULE)_$(ARCH).def" /IMPLIB:"$(LIBRARY)" \
	/LIBPATH:"$(LIB_PATH)" /LIBPATH:$(LIB_postgre) \
...
{$(SRC_PATH)}.c{$(OBJ_PATH)}.obj::
	$(CC) $(CFLAGS) /I $(INC_PATH) /I $(INC_postgre) $<
...
test:
 	...
 	for %i in ($(SRC_PATH)/xdb_postgre.*) do @echo $(OBJ_PATH)/%i \>>$(MODULE).txt
