...
# making order:
# 1. nmake /f xdb_sqlite.so.mk test --if need to creating some directory or file-list
# 2. nmake /f xdb_sqlite.so.mk clean --if no longer debuging
# 3. nmake /f xdb_sqlite.so.mk
# 4. nmake /f xdb_sqlite.so.mk install
#-----------------------------------------------------------------------------
...
test:
 	...
 	for %i in ($(SRC_PATH)/xdb_sqlite.*) do @echo $(OBJ_PATH)/%i \>>$(MODULE).txt
