...
# making order:
# 1. nmake /f xdb_stub.so.mk test --if need to creating some directory or file-list
# 2. nmake /f xdb_stub.so.mk clean --if no longer debuging
# 3. nmake /f xdb_stub.so.mk
# 4. nmake /f xdb_stub.so.mk install
#-----------------------------------------------------------------------------
!if EXIST("xdb_stub.txt")
...
test:
 	...
 	for %i in ($(SRC_PATH)/xdb_stub.*) do @echo $(OBJ_PATH)/%i \>>$(MODULE).txt
