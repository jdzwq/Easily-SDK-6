...
bool_t STDCALL db_parse_dsn(const tchar_t* dsnfile, tchar_t* srv_buf, int srv_len, tchar_t* dbn_buf, int dbn_len, tchar_t* uid_buf, int uid_len, tchar_t* pwd_buf, int pwd_len)
{
	...
	read_proper(d_ptr, _T("PGSQL"), -1, DSN_SERVER, -1, srv_buf, srv_len);
	read_proper(d_ptr, _T("PGSQL"), -1, DSN_DATABASE, -1, dbn_buf, dbn_len);
	read_proper(d_ptr, _T("PGSQL"), -1, DSN_UID, -1, uid_buf, uid_len);
	read_proper(d_ptr, _T("PGSQL"), -1, DSN_PWD, -1, pwd_buf, pwd_len);
...
bool_t STDCALL db_call_func(xdb_t db, LINKPTR func)
{
    ...
    XDK_ASSERT(db && db->tag == _DB_PGSQL);
	...
	res = PQexecParams(pdb->ctx, d_sql, ind, poid, (const char* const*)pbuf, plen, pfmt, 0);
...
bool_t STDCALL db_import(xdb_t db, stream_t stream, const tchar_t* table)
{
	...
	while (1)
	{
		...
		res = PQexecParams(pdb->ctx, d_sql, cols, poid, (const char* const*)pbuf, plen, pfmt, 0);
