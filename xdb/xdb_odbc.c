...
static bool_t _lob_read(XHANDLE io, byte_t* data, dword_t* pb)
{
	lob_t* plob = (lob_t*)io;
	SQLRETURN rt;
	SQLLEN len = 0;
...
void _db_tran(xdb_odbc_context* pdb)
{
	pdb->tran = 1;
...
int _db_call_argv(xdb_odbc_context* pdb, const tchar_t* procname, const tchar_t* fmt, va_list* parg)
{
	...
	rt = SQLPrepare(pdb->stm, (SQLTCHAR*)d_sql, sqllen);
	if (rt != SQL_SUCCESS && rt != SQL_SUCCESS_WITH_INFO)
	{
		_raise_stmt_error(pdb->stm);
	}
...
bool_t STDCALL db_import(xdb_t db, stream_t stream, const tchar_t* table)
{
	...
	t_sql = (SQLTCHAR*)string_ptr(vs_sql);
	rt = SQLPrepare(pdb->stm, t_sql, SQL_NTS);
	if (rt != SQL_SUCCESS && rt != SQL_SUCCESS_WITH_INFO)
	{
		_raise_stmt_error(pdb->stm);
	}
...
bool_t STDCALL db_read_xdoc(xdb_t db, LINKPTR dom, const tchar_t* sqlstr)
{
	xdb_odbc_context* pdb = (xdb_odbc_context*)db;
...
bool_t STDCALL db_write_xdoc(xdb_t db, LINKPTR dom, const tchar_t* sqlfmt)
{
	xdb_odbc_context* pdb = (xdb_odbc_context*)db;
