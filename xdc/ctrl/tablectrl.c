...
static void _tablectrl_item_rect(widget_t widget, link_t_ptr plk, xrect_t* pxr)
{
	...
	default_xface(&xa);
	calc_table_item_rect(&im, pxf, &xa, ptd->table, plk, pxr);
...
static void _tablectrl_item_key_rect(widget_t widget, link_t_ptr plk, xrect_t* pxr)
{
	...
	default_xface(&xa);
	calc_table_item_key_rect(&im, pxf, &xa, ptd->table, ptd->ratio, plk, pxr);
...
static void _tablectrl_item_val_rect(widget_t widget, link_t_ptr plk, xrect_t* pxr)
{
	...
	default_xface(&xa);
	calc_table_item_val_rect(&im, pxf, &xa, ptd->table, ptd->ratio, plk, pxr);
...
static void _tablectrl_reset_page(widget_t widget)
{
	...
	if (ptd->table)
	{
		...
		default_xface(&xa);
		(*im.mea->pf_measure_font)(im.ctx, pxf, &xs);
		xs.fw = xs.fh;
		xs.fh = calc_table_height(&im, pxf, &xa, ptd->table);
...
void hand_tablectrl_lbutton_down(widget_t widget, const xpoint_t* pxp)
{
	...
	default_xface(&xa);
	ilk = NULL;
	hint = calc_table_hint(&im, pxf, &xa, &pt, ptd->table, ptd->ratio, &ilk);
...
void hand_tablectrl_paint(widget_t widget, visual_t dc, const xrect_t* pxr)
{
	...
	draw_table(&ifc, pxf, &xa, &xp, &xb, ptd->table, ptd->ratio);
...
link_t_ptr tablectrl_insert_item(widget_t widget, link_t_ptr pre)
{
	...
	{
#ifdef _DEBUG
		XDK_ASSERT(is_string_entity(ptd->table, pre));
#endif
	}
