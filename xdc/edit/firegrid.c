...
static int sub_editbox_notice(widget_t widget, NOTICE* pnt, uid_t subid, vword_t delta)
{
	widget_t ctrl;
...
widget_t firegrid_create(widget_t widget, const xrect_t* pxr, link_t_ptr data)
{
	widget_t editor,ctrlbox = (widget_t)0;
	xsize_t xs;
	xrect_t xr_ed, xr = { 0 };
...
link_t_ptr firegrid_get_data(widget_t widget)
{
	widget_t ctrl;

	ctrl = (widget_t)widget_get_subproc_delta(widget, IDS_EDITBOX);
...
link_t_ptr firegrid_get_item(widget_t widget)
{
	widget_t ctrl;
