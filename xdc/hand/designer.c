...
static int _designer_calc_hint(designer_context* ptd, const xpoint_t* pxp, void** pobj)
{
	...
	grp = (*pdi->pf_get_next_grp)(ptd->widget, LINK_FIRST);
	while (grp)
	{
		obj = (*pdi->pf_get_next_obj)(ptd->widget, grp, LINK_FIRST);
		while (obj)
		{
			...
			obj = (*pdi->pf_get_next_obj)(ptd->widget, grp, obj);
		}

		grp = (*pdi->pf_get_next_grp)(ptd->widget, grp);
	}
...
static void _designer_noti_object_drop(designer_context* ptd, int x, int y)
{
	...
	while (grp)
	{
		...
		while (obj)
		{
			if ((pdi->with_opera & WITH_DRAG_FOCUSED && obj == obf) || (pdi->with_opera & WITH_DRAG_SELECTED && (*pdi->pf_get_obj_selected)(ptd->widget, obj)))
			{
				(*pdi->pf_get_obj_rect)(ptd->widget, obj, &xr);
				...
				if ((*pdi->pf_set_obj_point)(ptd->widget, obj, &pt))
				{
					cn++;
...
static void _designer_noti_object_sized(designer_context* ptd, int x, int y)
{
	...
	if(!(*pdi->pf_set_obj_size)(ptd->widget, obf, &xs))
	{
		_designer_discard(ptd);
		return;
	}
...
int designer_sub_lbutton_up(widget_t widget, const xpoint_t* pxp, uid_t sid, vword_t delta)
{
	...
	_designer_noti_object_lbclick(ptd, obj, pxp);

	return 1;
}

int designer_sub_rbutton_down(widget_t widget, const xpoint_t* pxp, uid_t sid, vword_t delta)
{
	designer_context* ptd = (designer_context*)delta;
...
int designer_sub_rbutton_up(widget_t widget, const xpoint_t* pxp, uid_t sid, vword_t delta)
{
	designer_context* ptd = (designer_context*)delta;
	int hint;
	void *obj;
	...
	_designer_noti_object_rbclick(ptd, obj, pxp);
...
int designer_sub_lbutton_dbclick(widget_t widget, const xpoint_t* pxp, uid_t sid, vword_t delta)
{
	...
	_designer_noti_object_dbclick(ptd, obj, pxp);
...
int designer_sub_mousemove(widget_t widget, dword_t mk, const xpoint_t* pxp, uid_t sid, vword_t delta)
{
	...
	if (pdi->with_opera & WITH_SIZE_HEIGHT &&  hint == HINT_HORZ_SPLIT && obj == obf && !(mk & KS_WITH_CONTROL))
	{
		widget_set_cursor(widget, CURSOR_SIZENS);
	...
	else if (pdi->with_opera & WITH_SIZE_WIDTH &&  hint == HINT_VERT_SPLIT && obj == obf && !(mk & KS_WITH_CONTROL))
	{
		widget_set_cursor(widget, CURSOR_SIZEWE);
	...
	else if (pdi->with_opera & WITH_SIZE_WIDTH &&  pdi->with_opera & WITH_SIZE_HEIGHT &&  hint == HINT_CROSS_SPLIT && obj == obf && !(mk & KS_WITH_CONTROL))
	{
		widget_set_cursor(widget, CURSOR_SIZEALL);
	...
	else if (pdi->with_opera & WITH_DRAG_FOCUSED && hint == HINT_OBJECT && obj == obf && !(mk & KS_WITH_CONTROL))
	{
		if (mk & MS_WITH_LBUTTON)
		{
	...
	else if (pdi->with_opera & WITH_MOUSE_GROUPED && hint == HINT_NONE)
	{
		if (mk & MS_WITH_LBUTTON)
		{
...
int designer_sub_keydown(widget_t widget, dword_t ks, int nKey, uid_t sid, vword_t delta)
{
	...
	if (nKey == KEY_UP || nKey == KEY_DOWN || nKey == KEY_LEFT || nKey == KEY_RIGHT)
	{
		...
		(*pdi->pf_get_point_scale)(widget, &mx, &my);

		grp = (*pdi->pf_get_next_grp)(widget, LINK_FIRST);
		while (grp)
		{
			obj = (*pdi->pf_get_next_obj)(widget, grp, LINK_FIRST);
			while (obj)
			{
				if (obj != obf && !(*pdi->pf_get_obj_selected)(ptd->widget, obj))
				{
					obj = (*pdi->pf_get_next_obj)(widget, grp, obj);
					continue;
				}
				...
				case KEY_UP:
					if (b_sft)
						xr.h = (xr.h - my < 0) ? xr.h : xr.h - my;
					else
						xr.y = (xr.y - my < 0) ? xr.y : xr.y - my;
					break;
				case KEY_LEFT:
					if (b_sft)
						xr.w = (xr.w - mx < 0) ? xr.w : xr.w - mx;
					else
						xr.x = (xr.x - mx < 0) ? xr.x : xr.x - mx;
					break;
				case KEY_RIGHT:
					if (b_sft)
						xr.w += mx;
				...
				if ((*pdi->pf_set_obj_rect)(widget, obj, &xr))
				{
					if (obj == obf)
					{
						if (b_sft)
	...
	}else if(nKey == KEY_BACK || nKey == KEY_DELETE)
	{
		...
		while (grp)
		{
			...
			while (obj)
			{
				...
				org = (*pdi->pf_get_next_obj)(widget, grp, obj);
...
int designer_sub_paint(widget_t widget, visual_t dc, const xrect_t* pxr, uid_t sid, vword_t delta)
{
	...
	if (widget_can_paging(widget) && pdi->with_ruler)
	{
	...
	else if (obf)
	{
		...
		mask = 0;
		if(pdi->with_opera & WITH_SIZE_WIDTH)
			mask |= SIZING_RIGHTCENTER;
		if(pdi->with_opera & WITH_SIZE_HEIGHT)
			mask |= SIZING_BOTTOMCENTER;
		if(pdi->with_opera & WITH_SIZE_WIDTH && pdi->with_opera & WITH_SIZE_HEIGHT)
			mask |= SIZING_BOTTOMRIGHT;

		draw_sizing_raw(&ifv, &xc, &xr, ALPHA_SOLID, mask);
	}
	...
	while (grp)
	{
		...
		grp = (*pdi->pf_get_next_grp)(widget, grp);
	}
