...
LOC_API void set_point_raw(visual_t rdc, const xcolor_t* pxc, const xpoint_t* ppt);
LOC_API void set_point(canvas_t canv, const xcolor_t* pxc, const xpoint_t* ppt);
...
@INPUT const xspan_t* prl: the large radius using integer member;
@INPUT const xspan_t* prs: the short radius using integer member;
@INPUT double fang: the from angle PI value.
