...
#define ASCII_GLYPH_INDEX(pch)		(pch[1] - 32)
#define GB2312_GLYPH_INDEX(pch)		((pch[0] - 161) * 94 + pch[1] - 161)
#define UNICODE_GLYPH_INDEX(pch)	((pch[0] - 78) * 256 + pch[1])

#define a_glyph_list_length		64
#define c_glyph_list_length		64
...
typedef int(*GLYPH_INDEX)(byte_t* pch);

typedef struct _glyph_info_t{
	tchar_t			code[64];		/*glyph coding*/
	tchar_t			charset[32];	/*glyph charset*/
	tchar_t			family[32];		/*glyph name*/
	...
	GLYPH_INDEX		pf_index; /*calc glyph pixmap position */
} glyph_info_t;

extern glyph_info_t a_glyph_list[a_glyph_list_length];
extern glyph_info_t c_glyph_list[c_glyph_list_length];
...
extern "C" {
	...
	EXP_API bool_t get_glyph_metrix(const glyph_info_t* pgi, const tchar_t *str, glyph_metrix_t* pmetr);

	EXP_API int get_glyph_pixmap(const glyph_info_t* pgi, const tchar_t* str, byte_t* pdata);
	...
	EXP_API bool_t gly_export(void);
#endif
