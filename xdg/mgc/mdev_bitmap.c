...
static device_t open_device(const dev_prn_t* devPrint)
{
	...
	pdev->horz_size = (int)((float)(devPrint->paper_width) / 10.0f);
	pdev->vert_size = (int)((float)(devPrint->paper_height) / 10.0f);

	pdev->logic_width = (int)((float)(pdev->horz_size) * PTPERMM);
	pdev->logic_height = (int)((float)(pdev->vert_size) * PTPERMM);
	...
	pdev->bitmap_info.height = pdev->physi_height;
...
static void get_device_caps(device_t dev, dev_cap_t* pcap)
{
	...
	pcap->horz_res = pdev->logic_width;
	pcap->vert_res = pdev->logic_height;
...
static void lp_to_dp(device_t dev, xpoint_t* ppt, int n)
{
	bitmap_device_t* pdev = (bitmap_device_t*)dev;
	int i;
	XDK_ASSERT(dev && dev->tag == _DEVICE_BITMAP);
	...
	for (i = 0; i < n; i++)
	{
		ppt[i].x = (int)((float)ppt[i].x * (float)pdev->physi_sx / (float)pdev->logic_sx);
		ppt[i].y = (int)((float)ppt[i].y * (float)pdev->physi_sy / (float)pdev->logic_sy);
	}
}

static void dp_to_lp(device_t dev, xpoint_t* ppt, int n)
{
	bitmap_device_t* pdev = (bitmap_device_t*)dev;
	int i;
	XDK_ASSERT(dev && dev->tag == _DEVICE_BITMAP);
