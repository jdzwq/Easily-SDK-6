...
void mgc_get_point_raw(visual_t mgc, xcolor_t *pxc, const xpoint_t *ppt)
{
	device_t hand;
	int rop;
	mem_device_ptr pdev;
	...
	(*(pdev->LpToDp))(hand, &pt, 1);

	(*(pdev->getPoint))(hand, &pt, pxc);
}

void mgc_get_point(canvas_t canv, xcolor_t* pxc, const xpoint_t* ppt)
{
	visual_t view;
	xpoint_t pt;
	...
	mgc_get_point_raw(view, pxc, &pt);
}

void mgc_set_point_raw(visual_t mgc, const xcolor_t *pxc, const xpoint_t *ppt)
{
	device_t hand;
	int rop;
	mem_device_ptr pdev;
	...
	(*(pdev->setPoint))(hand, &pt, pxc, rop);
}

void mgc_set_point(canvas_t canv, const xcolor_t* pxc, const xpoint_t* ppt)
{
	visual_t view;
	xpoint_t pt;
...
void mgc_draw_line_raw(visual_t mgc, const xpen_t *pxp, const xpoint_t *ppt1, const xpoint_t *ppt2)
{
	...
	copy_point(&pt1, ppt1);
	copy_point(&pt2, ppt2);
	(*(pdev->LpToDp))(hand, &pt1, 1);
	(*(pdev->LpToDp))(hand, &pt2, 1);

	pt_screen_to_world(pt1, &pt2, 1);

	n = dot_line(fs, ds, pt2.x, pt2.y, NULL, MAX_LONG);
	ppt = (xpoint_t *)xmem_alloc(n * sizeof(xpoint_t));
	n = dot_line(fs, ds, pt2.x, pt2.y, ppt, n);

	pt_world_to_screen(pt1, ppt, n);

	(*(pdev->drawPoints))(hand, ppt, n, &xc, 1, rop);
...
void mgc_draw_polyline_raw(visual_t mgc, const xpen_t *pxp, const xpoint_t *ppt, int pn)
{
	...
	for (i = 1; i < pn; i++)
	{
		copy_point(&pt[0], &ppt[i-1]);
		copy_point(&pt[1], &ppt[i]);
		(*(pdev->LpToDp))(hand, pt, 2);

		pt_screen_to_world(pt[0], &pt[1], 1);

		n = dot_line(fs, ds, pt[1].x, pt[1].y, NULL, MAX_LONG);
		ppt_buff = (xpoint_t *)xmem_realloc(ppt_buff, (total + n) * sizeof(xpoint_t));
		n = dot_line(fs, ds, pt[1].x, pt[1].y, ppt_buff + total, n);

		pt_world_to_screen(pt[0], ppt_buff + total, n);
		total += n;
	}

	(*(pdev->drawPoints))(hand, ppt_buff, total, &xc, 1, rop);
...
void mgc_draw_bezier_raw(visual_t mgc, const xpen_t *pxp, const xpoint_t *ppt1, const xpoint_t *ppt2, const xpoint_t *ppt3, const xpoint_t *ppt4)
{
	...
	pt[0].x = ppt1->x, pt[0].y = ppt1->y;
	pt[1].x = ppt2->x, pt[1].y = ppt2->y;
	pt[2].x = ppt3->x, pt[2].y = ppt3->y;
	pt[3].x = ppt4->x, pt[3].y = ppt4->y;
	(*(pdev->LpToDp))(hand, pt, 4);

	pt_screen_to_world(pt[0], &pt[1], 3);

	n = dot_curve3(fs, ds, &pt[1], &pt[2], &pt[3], NULL, MAX_LONG);
	ppt = (xpoint_t *)xmem_alloc(n * sizeof(xpoint_t));
	n = dot_curve3(fs, ds, &pt[1], &pt[2], &pt[3], ppt, n);

	pt_world_to_screen(pt[0], ppt, n);
...
void mgc_draw_curve_raw(visual_t mgc, const xpen_t *pxp, const xpoint_t *ppt_ctl, int ppt_count)
{
	...
	if (ppt_count == 3)
	{
		pt[0].x = ppt_ctl[0].x, pt[0].y = ppt_ctl[0].y;
		pt[1].x = ppt_ctl[1].x, pt[1].y = ppt_ctl[1].y;
		pt[2].x = ppt_ctl[2].x, pt[2].y = ppt_ctl[2].y;
		(*(pdev->LpToDp))(hand, pt, 3);

		pt_screen_to_world(pt[0], &pt[1], 2);

		n = dot_curve2(fs, ds, &pt[1], &pt[2], NULL, MAX_LONG);
		ppt = (xpoint_t *)xmem_alloc(n * sizeof(xpoint_t));
		n = dot_curve2(fs, ds, &pt[1], &pt[2], ppt, n);
	...
	else if (ppt_count == 4)
	{
		...
		pt[1].x = ppt_ctl[1].x, pt[1].y = ppt_ctl[1].y;
		pt[2].x = ppt_ctl[2].x, pt[2].y = ppt_ctl[2].y;
		pt[3].x = ppt_ctl[3].x, pt[3].y = ppt_ctl[3].y;
		(*(pdev->LpToDp))(hand, pt, 4);
...
void mgc_draw_arc_raw(visual_t mgc, const xpen_t *pxp, const xpoint_t *ppt1, const xpoint_t *ppt2, const xsize_t *pxs, bool_t clockwise, bool_t largearc)
{
	...
	pt[0].x = 0, pt[0].y = 0;
	pt[1].x = ppt1->x, pt[1].y = ppt1->y;
	pt[2].x = ppt2->x, pt[2].y = ppt2->y;
	(*(pdev->LpToDp))(hand, pt, 3);

	clockwise = pt_calc_radian(clockwise, largearc, pxs->w, pxs->h, &pt[1], &pt[2], &pt[0], &a1, &a2);
...
void mgc_flood_fill_raw(visual_t mgc, const xbrush_t* pxb, const xrect_t* pxr, const xpoint_t* ppt)
{
	...
	(*(pdev->LpToDp))(hand, &pk, 1);
	(*(pdev->LpToDp))(hand, RECTPOINT(&xr), 2);

	if (compare_text(pxb->style, -1, GDI_ATTR_FILL_STYLE_GRADIENT, -1, 1) == 0)
	{
		(*(pdev->radialLinear))(hand, &xr, &pk, xc, rop);
	}
	else
	{
		(*(pdev->floodFill))(hand, &xr, &pk, xc, rop);
	}
}

void mgc_flood_fill(canvas_t canv, const xbrush_t* pxb, const xrect_t* pxr, const xpoint_t* ppt)
{
	visual_t view;
	xrect_t xr;
	xpoint_t pt;
...
void mgc_draw_triangle_raw(visual_t mgc, const xpen_t *pxp, const xbrush_t *pxb, const xrect_t *pxr, const tchar_t *orient)
{
	...
	{
		mgc_flood_fill_raw(mgc, pxb, pxr, NULL);
	}
}
...
void mgc_draw_rect_raw(visual_t mgc, const xpen_t *pxp, const xbrush_t *pxb, const xrect_t *pxr)
{
	...
	n = dot_rect(fs, ds, xr.w, xr.h, NULL, MAX_LONG);
	ppt = (xpoint_t *)xmem_alloc(n * sizeof(xpoint_t));
	n = dot_rect(fs, ds, xr.w, xr.h, ppt, n);
...
void mgc_draw_ellipse_raw(visual_t mgc, const xpen_t *pxp, const xbrush_t *pxb, const xrect_t *pxr)
{
	...
	n = dot_ellipse(fs, ds, xr.w / 2, xr.h / 2, NULL, MAX_LONG);
	ppt = (xpoint_t *)xmem_alloc(n * sizeof(xpoint_t));
	n = dot_ellipse(fs, ds, xr.w / 2, xr.h / 2, ppt, n);
...
void mgc_draw_pie_raw(visual_t mgc, const xpen_t *pxp, const xbrush_t *pxb, const xrect_t *pxr, double fang, double tang)
{
	...
	pt_calc_points(&pt, xr.w / 2, xr.h / 2, fang, tang, &clockwise, &largearc, &pt1, &pt2);
	...
	n = dot_arc(fs, ds, xr.w / 2, xr.h / 2, fang, tang, clockwise, NULL, MAX_LONG);
	total += n;
	...
	n = dot_arc(fs, ds, xr.w / 2, xr.h / 2, fang, tang, clockwise, ppt + total, MAX_LONG);
	total += n;
	...
	(*(pdev->drawPoints))(hand, ppt, total, xc, 1, rop);
	xmem_free(ppt);
	...
	if (!is_null_xbrush(pxb))
	{
		...
		mgc_flood_fill_raw(mgc, pxb, pxr, &pt);
	}
}
...
void mgc_draw_sector_raw(visual_t mgc, const xpen_t *pxp, const xbrush_t *pxb, const xpoint_t *ppt_center, const xspan_t *prl, const xspan_t *prs, double fang, double tang)
{
	...
	if (!is_null_xbrush(pxb))
	{
		...
		mgc_flood_fill_raw(mgc, pxb, &xr, &pt1);
	}
}
...
void mgc_draw_equilagon_raw(visual_t mgc, const xpen_t *pxp, const xbrush_t *pxb, const xpoint_t *ppt_center, const xspan_t *pxn, int pn)
{
	...
	copy_point(&pt, ppt_center);
	(*(pdev->LpToDp))(hand, &pt, 1);

	ppt = (xpoint_t*)xmem_alloc(sizeof(xpoint_t)* pn);

	pt_calc_equilater(&pt, xn.s, ppt, pn);
	...
	if (!is_null_xbrush(pxb))
	{
		...
		mgc_flood_fill_raw(mgc, pxb, &xr, &pt);
	}
}
...
void mgc_draw_path_raw(visual_t mgc, const xpen_t *pxp, const xbrush_t *pxb, const tchar_t *aa, const xpoint_t *ppt, int pn)
{
	device_t hand;
	int rop;
	mem_device_ptr pdev;
	...
	XDK_ASSERT(mgc != NULL);
	XDK_ASSERT(aa != NULL && ppt != NULL && pxp != NULL);
	...
	ppt_dev = (xpoint_t*)xmem_alloc(pn * sizeof(xpoint_t));
	xmem_copy((void*)ppt_dev, (void*)ppt, pn * sizeof(xpoint_t));
	(*(pdev->LpToDp))(hand, ppt_dev, pn);
	pa = ppt_dev;
	...
	(*(pdev->drawPoints))(hand, ppt_buf, ppt_size, &xc, 1, rop);
	xmem_free(ppt_buf);
}
...
void mgc_text_out_raw(visual_t mgc, const xfont_t *pxf, const xpoint_t *ppt, const tchar_t *txt, int len)
{
	...
	{
		clean_pixmap(pix);
		w = (*pmf->getCharPixmap)(ft, txt, pix);
		pix->fg_color = PUT_PIXVAL(0, xc.r, xc.g, xc.b);
		pix->bg_used = 0;

		pt.x = x,pt.y = y;
		(*(pdev->LpToDp))(hand, &pt, 1);
		(*(pdev->drawPixmap))(hand, pt.x, pt.y, pix->width, pix->height, pix, 0, 0, rop);
		x += w;
...
void mgc_draw_text_raw(visual_t mgc, const xfont_t *pxf, const xface_t *pxa, const xrect_t *pxr, const tchar_t *txt, int len)
{
	...
	while (n < len)
	{
		...
		clean_pixmap(pix);
		(*pmf->getCharPixmap)(ft, pch, pix);
		pix->fg_color = PUT_PIXVAL(0, xc.r, xc.g, xc.b);
		pix->bg_used = 0;

		if (pt_in_rect(RECTPOINT(&(pa[n])), &xr))
		{
			(*(pdev->drawPixmap))(hand, pa[n].x, pa[n].y, pix->width, pix->height, pix, 0, 0, rop);
		}
		n++;
	}
...
void mgc_gradient_rect_raw(visual_t mgc, const xcolor_t *clr_brim, const xcolor_t *clr_core, const tchar_t *gradient, const xrect_t *pxr)
{
	...
	if (compare_text(gradient, -1, GDI_ATTR_GRADIENT_HORZ, -1, 1) == 0)
		(*(pdev->horzLinear))(hand, &xr, &pt, xc, rop);
	else if (compare_text(gradient, -1, GDI_ATTR_GRADIENT_VERT, -1, 1) == 0)
		(*(pdev->vertLinear))(hand, &xr, &pt, xc, rop);
...
void mgc_alphablend_rect_raw(visual_t mgc, const xcolor_t* pxc, const xrect_t* pxr, int opacity)
{
	...
	(*(pdev->LpToDp))(hand, RECTPOINT(&xr), 2);
	(*(pdev->maskRect))(hand, &xr, pxc, opacity);
}
...
void mgc_draw_image_raw(visual_t mgc, const ximage_t *pmi, const xrect_t *pxr)
{
	...
	(*(pdev->LpToDp))(hand, RECTPOINT(&xr), 2);
	(*(pdev->drawBitmap))(hand, xr.x, xr.y, xr.w, xr.h, buf_bmp, rop);
