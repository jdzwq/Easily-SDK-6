...
*/
ecp_curve_type ecp_get_type(const ecp_group *grp)
{
	if (grp->G.X.p == NULL)
...
static const ecp_curve_info ecp_supported_curves[] =
{
	...
	{ ECP_DP_CURVE25519, 29, 254, "x25519" },
	{ ECP_DP_CURVE448, 30, 447, "x448" },
...
*/
const ecp_curve_info *ecp_curve_info_from_tls_id(sword_t tls_id)
{
	const ecp_curve_info *curve_info;
...
void ecp_group_free(ecp_group *grp)
{
	...
	xmem_zero(grp, sizeof(ecp_group));
}
...
*/
static void ecp_mpi_load(mpi *X, const mpi_uint *p, dword_t len)
{
	X->s = 1;
...
static int _ecp_group_load(ecp_group *grp,
	const mpi_uint *p, dword_t plen,
	const mpi_uint *a, dword_t alen,
	const mpi_uint *b, dword_t blen,
	const mpi_uint *gx, dword_t gxlen,
	const mpi_uint *gy, dword_t gylen,
	const mpi_uint *n, dword_t nlen)
{
...
#define MAX32       N->n * 2
#define A( j ) (j) % 2 ? (dword_t)( N->p[(j)/2] >> 32 ) : \
                         (dword_t)( N->p[(j)/2] )
#define STORE32                                   \
    if( i % 2 ) {                                 \
...
*/
static void add32(dword_t *dst, dword_t src, signed char *carry)
{
	*dst += src;
	*carry += (*dst < src);
}

static void sub32(dword_t *dst, dword_t src, signed char *carry)
{
	*carry -= (*dst < src);
	*dst -= src;
}
...
*/
static int fix_negative(mpi *N, signed char c, mpi *C, dword_t bits)
{
	int ret;
...
#define INIT( b )                                                       \
    ...
    C.p = Cp;                                                           \
    xmem_zero( Cp, C.n * sizeof( mpi_uint ) );                  \
                                                                        \
...
static int ecp_mod_p521(mpi *N)
{
	...
	* P521_WIDTH + 1. Otherwise P521_WIDTH is enough. */
	...
	if (M.n > P521_WIDTH + 1)
		M.n = P521_WIDTH + 1;
	M.p = Mp;
	xmem_copy(Mp, N->p + P521_WIDTH - 1, M.n * sizeof(mpi_uint));
...
static int ecp_mod_p255(mpi *N)
{
	...
	if (M.n > P255_WIDTH + 1)
	{
		set_last_error(_T("ecp_mod_p255"), _T("ERR_ECP_BAD_INPUT_DATA"), -1);
		return C_ERR;
	}

	M.p = Mp;
	xmem_zero(Mp, sizeof Mp);
	xmem_copy(Mp, N->p + P255_WIDTH - 1, M.n * sizeof(mpi_uint));
...
static int ecp_mod_p448(mpi *N)
{
	...
	M.s = 1;
	M.n = N->n - (P448_WIDTH);
	if (M.n > P448_WIDTH) /* Shouldn't be called with N larger than 2^896! */
	{
		set_last_error(_T("ecp_mod_p448"), _T("ERR_ECP_BAD_INPUT_DATA"), -1);
		return C_ERR;
	}

	M.p = Mp;
	xmem_zero(Mp, sizeof(Mp));
	xmem_copy(Mp, N->p + P448_WIDTH, M.n * sizeof(mpi_uint));
...
static int ecp_mod_koblitz(mpi *N, mpi_uint *Rp, dword_t p_limbs,
	dword_t adjust, dword_t shift, mpi_uint mask)
{
	int ret;
	dword_t i;
	mpi M, R;
	...
	if (M.n > p_limbs + adjust)
		M.n = p_limbs + adjust;
	xmem_zero(Mp, sizeof Mp);
	xmem_copy(Mp, N->p + p_limbs - adjust, M.n * sizeof(mpi_uint));
	if (shift != 0)
...
int ecp_group_load(ecp_group *grp, ecp_group_id id)
{
	...
	default:
		...
		set_last_error(_T("ecp_group_load"), _T("ERR_ECP_FEATURE_UNAVAILABLE"), -1);
...
int ecp_point_write_binary(const ecp_group *grp,
	const ecp_point *P,
	int format, dword_t *olen,
	byte_t *buf, dword_t buflen)
{
	int ret = 0;
	dword_t plen;

	if (grp->id == ECP_DP_CURVE25519 || grp->id == ECP_DP_CURVE448)
	{
		plen = mpi_size(&grp->P);
		if (buflen < plen)
		{
			set_last_error(_T("ecp_point_write_binary"), _T("ERR_ECP_BUFFER_TOO_SMALL"), -1);
			return C_ERR;
		}

		*olen = plen;
		MPI_CHK(mpi_write_binary_le(&P->X, buf, plen));
	}
	else
	{
		/*
	...
	const byte_t *buf, dword_t ilen)
{
	...
	if (ecp_get_type(grp) == ECP_TYPE_MONTGOMERY)
	{
		...
		if (ilen != plen)
		{
			set_last_error(_T("ecp_point_read_binary"), _T("ERR_ECP_BAD_INPUT_DATA"), -1);
			return C_ERR;
		}

		MPI_CHK(mpi_read_binary_le(&pt->X, buf, plen));
		mpi_free(&pt->Y);

		if (grp->id == ECP_DP_CURVE25519)
		{
			/* Set most significant bit to 0 as prescribed in RFC7748 §5 */
			MPI_CHK(mpi_set_bit(&pt->X, plen * 8 - 1, 0));
		}
	...
	else
	{
		...
		if (buf[0] != 0x04)
		{
			set_last_error(_T("ecp_point_read_binary"), _T("ERR_ECP_FEATURE_UNAVAILABLE"), -1);
			return ERR_ECP_FEATURE_UNAVAILABLE;
		}
...
int ecp_tls_read_point(const ecp_group *grp,
	ecp_point *pt,
	const byte_t **buf, dword_t buf_len)
{
	byte_t data_len;
	const byte_t *buf_start;
	...
	*/
	if (buf_len < 2)
	{
		set_last_error(_T("ecp_tls_read_point"), _T("ERR_ECP_BAD_INPUT_DATA"), -1);
		return C_ERR;
	}
	...
	byte_t *buf, dword_t blen)
{
	...
	*/
	if (blen < 1)
	{
		set_last_error(_T("ecp_tls_write_point"), _T("ERR_ECP_BAD_INPUT_DATA"), -1);
		return C_ERR;
	}
	...
	const byte_t **buf, dword_t len)
{
	...
	*/
	if (len < 3)
	{
		set_last_error(_T("ecp_tls_read_group_id"), _T("ERR_ECP_BAD_INPUT_DATA"), -1);
		return C_ERR;
	}
	...
	*/
	...
	{
		set_last_error(_T("ecp_tls_read_group_id"), _T("ERR_ECP_FEATURE_UNAVAILABLE"), -1);
		return ERR_ECP_FEATURE_UNAVAILABLE;
	}
...
*/
int ecp_tls_write_group(const ecp_group *grp, dword_t *olen,
	byte_t *buf, dword_t blen)
{
	const ecp_curve_info *curve_info;
	...
	{
		set_last_error(_T("ecp_tls_write_group"), _T("ERR_ECP_BAD_INPUT_DATA"), -1);
		return C_ERR;
	}
	...
	*olen = 3;
	if (blen < *olen)
	{
		set_last_error(_T("ecp_tls_write_group"), _T("ERR_ECP_BUFFER_TOO_SMALL"), -1);
		return C_ERR;
	}
...
static int ecp_modp(mpi *N, const ecp_group *grp)
{
	...
	{
		set_last_error(_T("ecp_modp"), _T("ERR_ECP_BAD_INPUT_DATA"), -1);
		return C_ERR;
	}
	...
	ecp_point *T[], dword_t T_size)
{
	...
	if ((c = xmem_alloc(T_size * sizeof(mpi))) == NULL)
	{
		set_last_error(_T("ecp_normalize_jac_many"), _T("ERR_ECP_ALLOC_FAILED"), -1);
		return C_ERR;
	}
	...
	const ecp_point *P, const ecp_point *Q)
{
	...
	*/
	...
	{
		set_last_error(_T("ecp_add_mixed"), _T("ERR_ECP_BAD_INPUT_DATA"), -1);
		return C_ERR;
	}
	...
	int(*f_rng)(void *, byte_t *, dword_t), void *p_rng)
{
	...
	do
	{
		...
		if (count++ > 10)
		{
			set_last_error(_T("ecp_randomize_jac"), _T("ERR_ECP_RANDOM_FAILED"), -1);
			return C_ERR;
		}
...
*   2 and 7 (in practice, between 2 and ECP_WINDOW_SIZE)
* - m is the MPI, expected to be odd and such that bitlength(m) <= w * d
*   (the result will be incorrect if these assumptions are not satisfied)
*/
static void ecp_comb_recode_core(byte_t x[], dword_t d,
	byte_t w, const mpi *m)
{
	dword_t i, j;
	byte_t c, cc, adjust;
...
* value, it's useful to set ECP_WINDOW_SIZE to a lower value in order
...
static int ecp_select_comb(const ecp_group *grp, ecp_point *R,
	const ecp_point T[], byte_t T_size,
	byte_t i)
{
	int ret;
	byte_t ii, j;
	...
	byte_t *parity_trick)
{
	...
	{
		set_last_error(_T("ecp_comb_recode_scalar"), _T("ERR_ECP_BAD_INPUT_DATA"), -1);
		return C_ERR;
	}
...
*/
static byte_t ecp_pick_window_size(const ecp_group *grp,
	byte_t p_eq_g)
{
	byte_t w;
...
*   functions): allocation, computation, ownership tranfer, xmem_freeing.
*
	...
	ecp_restart_ctx *rs_ctx)
{
	...
	{
		T = xmem_alloc(T_size * sizeof(ecp_point));
		if (T == NULL)
		{
			set_last_error(_T("ecp_mul_comb"), _T("ERR_ECP_ALLOC_FAILED"), -1);
			ret = C_ERR;
			goto cleanup;
		}
	...
	int(*f_rng)(void *, byte_t *, dword_t), void *p_rng)
{
	...
	do
	{
		...
		if (count++ > 10)
		{
			set_last_error(_T("ecp_randomize_mxz"), _T("ERR_ECP_RANDOM_FAILED"), -1);
			return C_ERR;
		}
	} while (mpi_cmp_int(&l, 1) <= 0);
...
static int ecp_check_pubkey_sw(const ecp_group *grp, const ecp_point *pt)
{
	...
	{
		set_last_error(_T("ecp_check_pubkey_sw"), _T("ERR_ECP_INVALID_KEY"), -1);
		return C_ERR;
	}
...
static int ecp_check_pubkey_mx(const ecp_group *grp, const ecp_point *pt)
{
	...
	* (RFC 7748 sec. 5 para. 3). */
	...
	{
		set_last_error(_T("ecp_check_pubkey_mx"), _T("ERR_ECP_INVALID_KEY"), -1);
		return C_ERR;
	}
	...
	const ecp_point *pt)
{
	...
	{
		set_last_error(_T("ecp_check_pubkey"), _T("ERR_ECP_INVALID_KEY"), -1);
		return C_ERR;
	}
	...
	set_last_error(_T("ecp_check_pubkey"), _T("ERR_ECP_BAD_INPUT_DATA"), -1);
	return C_ERR;
}
	...
	const mpi *d)
{
	if (ecp_get_type(grp) == ECP_TYPE_MONTGOMERY)
	{
		...
		{
			set_last_error(_T("ecp_check_privkey"), _T("ERR_ECP_INVALID_KEY"), -1);
			return C_ERR;
		}
	...
	set_last_error(_T("ecp_check_privkey"), _T("ERR_ECP_BAD_INPUT_DATA"), -1);
	return C_ERR;
}
		...
		*/
		do
		{
			...
			*/
			if (++count > 30)
			{
				set_last_error(_T("ecp_gen_privkey"), _T("ERR_ECP_RANDOM_FAILED"), -1);
				return C_ERR;
			}
	...
	ecp_restart_ctx *rs_ctx)
{
	...
	if (ecp_get_type(grp) != ECP_TYPE_SHORT_WEIERSTRASS)
	{
		set_last_error(_T("ecp_muladd_restartable"), _T("ERR_ECP_FEATURE_UNAVAILABLE"), -1);
		return ERR_ECP_FEATURE_UNAVAILABLE;
	}
...
int ecp_check_pub_priv(const ecp_keypair *pub, const ecp_keypair *prv)
{
	...
	{
		set_last_error(_T("ecp_check_pub_priv"), _T("ERR_ECP_BAD_INPUT_DATA"), -1);
		return C_ERR;
	}
...
void ecp_keypair_copy(ecp_keypair *dst, ecp_keypair* src)
{
	ecp_group_copy(&dst->grp, &src->grp);
	mpi_copy(&dst->d, &src->d);
	ecp_copy(&dst->Q, &src->Q);
}
...
*/
static int group_from_specified(const asn1_buf *params, ecp_group *grp)
{
	int ret;
	...
	*/
	...
	{
		set_last_error(_T("group_from_specified"), _T("ERR_PK_KEY_INVALID_FORMAT"), -1);
		return C_ERR;
	}
	...
	*/
	...
	{
		set_last_error(_T("group_from_specified"), _T("asn1_get_tag"), -1);
		return C_ERR;
	}
	...
	*/
	...
	if (len != OID_SIZE(OID_ANSI_X9_62_PRIME_FIELD) || xmem_comp(p, OID_ANSI_X9_62_PRIME_FIELD, len) != 0)
	{
		set_last_error(_T("group_from_specified"), _T("ERR_PK_FEATURE_UNAVAILABLE"), -1);
		return C_ERR;
	}
	...
	if (p != end_field)
	{
		set_last_error(_T("group_from_specified"), _T("ERR_ASN1_LENGTH_MISMATCH"), -1);
		return C_ERR;
	}
	...
	*/
	if ((ret = asn1_get_tag(&p, end_curve, &len, ASN1_OCTET_STRING)) != 0 || (ret = mpi_read_binary(&grp->A, p, len)) != 0)
	{
		set_last_error(_T("group_from_specified"), _T("mpi_read_binary"), -1);
		return C_ERR;
	}

	p += len;

	if ((ret = asn1_get_tag(&p, end_curve, &len, ASN1_OCTET_STRING)) != 0 || (ret = mpi_read_binary(&grp->B, p, len)) != 0)
	{
		set_last_error(_T("group_from_specified"), _T("mpi_read_binary"), -1);
		return C_ERR;
	}
	...
	*/
	...
	if ((ret = ecp_point_read_binary(grp, &grp->G, (const unsigned char *)p, len)) != 0)
	{
		/*
...
*/
int ecp_group_id_from_group(const ecp_group *grp, ecp_group_id *grp_id)
{
	int ret = 0;
	ecp_group ref;
...
*/
static int group_id_from_specified(const asn1_buf *params, ecp_group_id *grp_id)
{
	int ret;
	ecp_group grp;
	...
	if ((ret = group_from_specified(params, &grp)) != 0)
		goto cleanup;

	ret = ecp_group_id_from_group(&grp, grp_id);
...
*/
static int ecp_get_params(unsigned char **p, const unsigned char *end, asn1_buf *params)
{
	int ret;
	...
	* }*/
	...
	if (end - *p < 1)
	{
		set_last_error(_T("ecp_get_params"), _T("ERR_PK_KEY_INVALID_FORMAT"), -1);
		return C_ERR;
	}

	/* Tag may be either OID or SEQUENCE */
	params->tag = **p;
	if (params->tag != ASN1_OID && params->tag != (ASN1_CONSTRUCTED | ASN1_SEQUENCE))
	{
		set_last_error(_T("ecp_get_params"), _T("ERR_PK_KEY_INVALID_FORMAT"), -1);
		return C_ERR;
	}
	...
	*p += params->len;
	...
	if (*p != end)
	{
		set_last_error(_T("ecp_get_params"), _T("ERR_ASN1_LENGTH_MISMATCH"), -1);
		return C_ERR;
	}
...
*/
static int ecp_use_params(const asn1_buf *params, ecp_group *grp)
{
	ecp_group_id grp_id;
	...
	*/
	if (params->tag == ASN1_OID)
	{
		...
		{
			set_last_error(_T("ecp_use_params"), _T("oid_get_ec_grp"), -1);
			return C_ERR;
		}
	}
	else
	{
		if (group_id_from_specified(params, &grp_id) != 0)
		{
			set_last_error(_T("ecp_use_params"), _T("pk_group_id_from_specified"), -1);
			return C_ERR;
		}
	}
	...
	*/
	...
	{
		set_last_error(_T("ecp_use_params"), _T("ERR_PK_KEY_INVALID_FORMAT"), -1);
		return C_ERR;
	}

	if (ecp_group_load(grp, grp_id) != 0)
	{
		set_last_error(_T("ecp_use_params"), _T("ecp_group_load"), -1);
		return C_ERR;
	}
...
*/
static int ecp_get_pubkey(unsigned char **p, const unsigned char *end, ecp_keypair *key)
{
	int ret;
...
*/
static int ecp_keypair_parse_key_der(ecp_keypair *eck, unsigned char *key, int keylen)
{
	int ret;
	...
	*/
	if ((ret = asn1_get_tag(&p, end, &len, ASN1_CONSTRUCTED | ASN1_SEQUENCE)) != 0)
	{
		set_last_error(_T("ecp_keypair_parse_key_der"), _T("asn1_get_tag"), -1);
		return(C_ERR);
	}
	...
	{
		set_last_error(_T("ecp_keypair_parse_key_der"), _T("asn1_get_int"), -1);
		return(C_ERR);
	}

	if (version != 1)
	{
		set_last_error(_T("ecp_keypair_parse_key_der"), _T("ERR_PK_KEY_INVALID_VERSION"), -1);
		return(C_ERR);
	}
	...
	if ((ret = mpi_read_binary(&eck->d, p, len)) != 0)
	{
		...
		set_last_error(_T("ecp_keypair_parse_key_der"), _T("mpi_read_binary"), -1);
		return(C_ERR);
	}
	...
	if (p != end)
	{
		...
		*/
		if ((ret = asn1_get_tag(&p, end, &len, ASN1_CONTEXT_SPECIFIC | ASN1_CONSTRUCTED | 0)) == 0)
		{
			if ((ret = ecp_get_params(&p, p + len, &params)) != 0 || (ret = ecp_use_params(&params, &eck->grp)) != 0)
			{
				ecp_keypair_free(eck);

				set_last_error(_T("ecp_keypair_parse_key_der"), _T("pk_get_ecparams"), -1);
				return(C_ERR);
			}
		}
		else if (ret != ERR_ASN1_UNEXPECTED_TAG)
		{
			...
			set_last_error(_T("ecp_keypair_parse_key_der"), _T("ERR_PK_KEY_INVALID_FORMAT"), -1);
			return(C_ERR);
		}
	}
	...
	if (p != end)
	{
		...
		*/
		if ((ret = asn1_get_tag(&p, end, &len, ASN1_CONTEXT_SPECIFIC | ASN1_CONSTRUCTED | 1)) == 0)
		{
			end2 = p + len;

			if ((ret = asn1_get_bitstring_null(&p, end2, &len)) != 0)
			{
				set_last_error(_T("ecp_keypair_parse_key_der"), _T("asn1_get_bitstring_null"), -1);
				return(C_ERR);
			}

			if (p + len != end2)
			{
				set_last_error(_T("ecp_keypair_parse_key_der"), _T("ERR_ASN1_LENGTH_MISMATCH"), -1);
				return(C_ERR);
			}

			if ((ret = ecp_get_pubkey(&p, end2, eck)) == 0)
			{
				pubkey_done = 1;
			}
			else
	...
	if (!pubkey_done && (ret = ecp_mul(&eck->grp, &eck->Q, &eck->d, &eck->grp.G, NULL, NULL)) != 0)
	{
		ecp_keypair_free(eck);
	...
	if ((ret = ecp_check_privkey(&eck->grp, &eck->d)) != 0)
	{
		...
		set_last_error(_T("ecp_keypair_parse_key_der"), _T("ecp_check_privkey"), -1);
		return(C_ERR);
	}
...
#define PEM_BEGIN_EC           "-----BEGIN EC PRIVATE KEY-----"
#define PEM_END_EC             "-----END EC PRIVATE KEY-----"

int ecp_keypair_parse_key(ecp_keypair *key, unsigned char *buf, int buflen, unsigned char *pwd, int pwdlen)
{
	bool_t b_pem = 0;
	pem_context pem;
	dword_t use_len;

	if (strstr((const char *)buf, PEM_BEGIN_EC) != NULL)
	{
		b_pem = 1;
	}
	...
	if (b_pem)
	{
		...
		if (pem_read_buffer(&pem, PEM_BEGIN_EC, PEM_END_EC, buf, pwd, pwdlen, &use_len) != 0)
		{
			pem_free(&pem);

			set_last_error(_T("ecp_keypair_parse_key"), _T("pem_read_buffer"), -1);
			return(C_ERR);
		}

		if (ecp_keypair_parse_key_der(key, pem.buf, pem.buflen) != 0)
		{
			pem_free(&pem);
	...
	{
		if (ecp_keypair_parse_key_der(key, buf, buflen) != 0)
		{
			pem_free(&pem);
