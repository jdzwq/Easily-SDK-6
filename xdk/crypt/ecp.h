...
#define ERR_ECP_FEATURE_UNAVAILABLE		(C_ERR - 1)
...
typedef struct _ecp_curve_info
{
    ...
    sword_t tls_id;                /*!< The TLS NamedCurve identifier. */
    sword_t bit_size;              /*!< The curve size in bits. */
...
typedef struct _ecp_group
{
    ...
    dword_t pbits;               /*!< The number of bits in \p P.*/
    dword_t nbits;               /*!< For Short Weierstrass: The number of bits in \p P. For Montgomery curves: the number of bits in the private keys. */
    ...
    dword_t T_size;              /*!< The number of pre-computed points. */
}ecp_group;
...
extern "C" {
	...
	* \return          NULL on failure.
	*/
	EXP_API const ecp_curve_info *ecp_curve_info_from_tls_id(sword_t tls_id);
	...
	*/
	EXP_API void ecp_keypair_init(ecp_keypair *key);
	...
	*/
	EXP_API void ecp_keypair_free(ecp_keypair *key);

	EXP_API void ecp_keypair_copy(ecp_keypair *dst, ecp_keypair *src);

	EXP_API int ecp_group_id_from_group(const ecp_group *grp, ecp_group_id *grp_id);

	EXP_API int ecp_keypair_parse_key(ecp_keypair *key, unsigned char *buf, int buflen, unsigned char *pwd, int pwdlen);
	...
	*/
	...
	EXP_API int ecp_test_parse(int verbose);
#endif
