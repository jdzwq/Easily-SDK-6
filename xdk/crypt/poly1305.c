...
*/
static void poly1305_process(poly1305_context *ctx,
	size_t nblocks,
...
*/
static void poly1305_compute_mac(const poly1305_context *ctx,
...
void poly1305_init(poly1305_context *ctx)
{
	...
	xmem_zero(ctx, sizeof(poly1305_context));
}

void poly1305_free(poly1305_context *ctx)
{
	if (ctx == NULL)
		return;
...
int poly1305_starts(poly1305_context *ctx,
	const unsigned char key[32])
{
	...
	xmem_zero(ctx->queue, sizeof(ctx->queue));
	ctx->queue_len = 0U;

	return(0);
}

int poly1305_update(poly1305_context *ctx,
	const unsigned char *input,
	size_t ilen)
{
	size_t offset = 0U;
...
int poly1305_finish(poly1305_context *ctx,
	unsigned char mac[16])
{
	XDK_ASSERT(ctx != NULL);
	XDK_ASSERT(mac != NULL);
...
int poly1305_mac(const unsigned char key[32],
	const unsigned char *input,
	size_t ilen,
	unsigned char mac[16])
{
	...
	ret = poly1305_starts(&ctx, key);
	if (ret != 0)
		goto cleanup;

	ret = poly1305_update(&ctx, input, ilen);
	if (ret != 0)
		goto cleanup;

	ret = poly1305_finish(&ctx, mac);
...
int poly1305_self_test(int verbose)
{
	unsigned char mac[16];
	unsigned i;
	int ret = -1;
	...
	{
		if (verbose != 0)
			printf("  Poly1305 test %u ", i);

		ret = poly1305_mac(test_keys[i],
			test_data[i],
			test_data_len[i],
			mac);
