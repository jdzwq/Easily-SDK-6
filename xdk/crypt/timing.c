...
#include "timing.h"
    ...
    !defined(__HAIKU__)
#error "This module only works on Unix and Windows, see TIMING_C in config.h"
#endif
...
#if !defined(HAVE_HARDCLOCK) && defined(HAVE_ASM) &&  \
...
* Used for testing timing_hardclock.
*/
static void busy_msleep(unsigned long msec)
{
	struct timing_hr_time hires;
	unsigned long i = 0; /* for busy-waiting */
	volatile unsigned long j; /* to prevent optimisation */

	(void)timing_get_timer(&hires, 1);

	while (timing_get_timer(&hires, 0) < msec)
		i++;
    ...
    {                                                                   \
        if( verbose != 0 )                                              \
            ...
            printf( " elapsed(hires)=%lu elapsed(ctx)=%lu status(ctx)=%d\n", \
                            timing_get_timer( &hires, 0 ),      \
                            timing_get_timer( &ctx.timer, 0 ),  \
                            timing_get_delay( &ctx ) );         \
		        }                                                               \
        return( 1 );                                                    \
    } while( 0 )
...
int timing_self_test(int verbose)
{
...

	{
		...
		set_alarm((int)secs);
		while (!timing_alarmed)
			;

		millisecs = timing_get_timer(&hires, 0);
...

	{
		a = 800;
		b = 400;
		timing_set_delay(&ctx, a, a + b);          /* T = 0 */

		busy_msleep(a - a / 4);                      /* T = a - a/4 */
		if (timing_get_delay(&ctx) != 0)
			FAIL;

		busy_msleep(a / 4 + b / 4);                  /* T = a + b/4 */
		if (timing_get_delay(&ctx) != 1)
			FAIL;

		busy_msleep(b);                          /* T = a + b + b/4 */
		if (timing_get_delay(&ctx) != 2)
			FAIL;
	}

	timing_set_delay(&ctx, 0, 0);
	busy_msleep(200);
	if (timing_get_delay(&ctx) != -1)
		FAIL;
...
hard_test:
	...
	millisecs = 1;
	cycles = timing_hardclock();
	busy_msleep(millisecs);
	cycles = timing_hardclock() - cycles;
