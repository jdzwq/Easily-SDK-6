...
dword_t xpdg_revert(const byte_t* pdg_buf, dword_t pdg_len, byte_t* bmp_buf, dword_t bmp_size)
{
	...
	dw = 0;
	dw += xbmp_set_head(&bf, (bmp_buf + dw), (bmp_size - dw));
	dw += xbmp_set_info(&bi, (bmp_buf + dw), (bmp_size - dw));
	dw += xbmp_fill_quad(bi.clrbits, bi.clrused, (bmp_buf + dw), (bmp_size - dw));
...
dword_t xpdg_convert(const byte_t* bmp_buf, dword_t bmp_len, byte_t* pdg_buf, dword_t pdg_size)
{
	...
	dword_t i, j;
	dword_t dRowSize, dIndex, dTotal = 0;
	sword_t us;
	byte_t uc;
	int n;
