...
typedef void(*PF_ERROR_PRINT)(const tchar_t*);
typedef void(*PF_ERROR_DEBUG)(const char*, const char*, unsigned int , const char*);
...
typedef int(*PF_UTF_TO_UCS)(const schar_t*, int, wchar_t*, int);
typedef int(*PF_UCS_TO_UTF)(const wchar_t*, int, schar_t*, int);

typedef struct _if_mbcs_t{
	PF_GBK_TO_UCS	pf_gbk_to_ucs;
	PF_UCS_TO_GBK	pf_ucs_to_gbk;
	PF_UTF_TO_UCS	pf_utf_to_ucs;
	PF_UCS_TO_UTF	pf_ucs_to_utf;
}if_mbcs_t;
#endif
...
typedef void(*PF_PROCESS_WAIT_RUN)(res_proc_t);
typedef void(*PF_PROCESS_WAIT_EXIT)(res_proc_t);
...
typedef struct _if_process_t{
	...
	PF_PROCESS_WAIT_RUN	pf_process_wait_run;
	PF_PROCESS_WAIT_EXIT	pf_process_wait_exit;
