...
bool_t _coap_send_request(coap_context* ppt)
{
	...
	XDK_ASSERT(ppt->type == _XCOAP_TYPE_CLI);
...
bool_t _coap_recv_request(coap_context* ppt)
{
	...
	XDK_ASSERT(ppt->type == _XCOAP_TYPE_SRV);
...
xhand_t	xcoap_server(xhand_t bio)
{
	coap_context* ppt = NULL;
	...
	port = xudp_addr_port(ppt->bio, addr);
	bind = _coap_port(&ppt->havs);
...
xhand_t xcoap_bio(xhand_t coap)
{
	...
	XDK_ASSERT(coap && coap->tag == _HANDLE_COAP);
