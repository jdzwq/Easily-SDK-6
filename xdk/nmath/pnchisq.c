...
double pnchisq(double x, double df, double ncp, int lower_tail, int log_p)
{
	...
	if (!log_p || ans < -1e-8)
		...
		ans = pnchisq_raw(x, df, ncp, 1e-12, 8 * DBL_EPSILON, 1000000, !lower_tail, 0);
		return log1p(-ans);
	}
}
	...
	if (theta < 80) { /* use 110 for Inf, as ppois(110, 80/2, lower.tail=FALSE) is 2e-20 */
		...
		if (lower_tail && f > 0. &&
			...
			for (i = 0; i < 110; pr += log(lambda) - log(++i)) {
				...
				sum = logspace_add(sum, pr + pchisq(x, f + 2 * i, lower_tail, 1));
			...
			for (i = 0; i < 110; pr *= lambda / ++i) {
				...
				sum += pr * pchisq(x, f + 2 * i, lower_tail, 0);
