...
double qt(double p, double ndf, int lower_tail, int log_p)
{
	...
	is_neg_lower = (lower_tail == neg); /* both 1 or 0 == !xor */
    if(neg)
	...
	if ((ndf < 2.1 && P > 0.5) || y > 0.05 + a) { /* P > P0(df) */
	    ...
	    if(P_ok)
		x = qnorm(0.5 * P, 0., 1., /*lower_tail*/1,  /*log_p*/0);
	    else /* log_p && P underflowed */
		x = qnorm(log_P2,  0., 1., lower_tail,	        /*log_p*/ 1);
	...
	/* FIXME: This can be far from optimal when log_p = 1
	 *      but is still needed, e.g. for qt(-2, df=1.01, log=1).
	 *	Probably also improvable when  lower_tail = 0 */

	if(P_ok1) {
	    int it=0;
	    while(it++ < 10 && (y = dt(q, ndf, 0)) > 0 &&
		  R_FINITE(x = (pt(q, ndf, 0, 0) - P/2) / y) &&
		  fabs(x) > 1e-14*fabs(q))
