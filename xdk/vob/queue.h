...
extern "C" {
	...
	EXP_API int queue_count(queue_t que);

	EXP_API	int queue_write(queue_t que, message_t msg);

	EXP_API message_t queue_read(queue_t que);

	EXP_API message_t queue_peek(queue_t que);
...
EXP_API void queue_self_test(void);
#endif
