...
bool_t _socket_sendto(res_file_t so, res_addr_t saddr, int alen, void* buf, dword_t size, async_t* pb)
{
	...
	dw = 0;
	err = WSASendTo((SOCKET)so, &wb, 1, &dw, fg, (SOCKADDR*)saddr, alen, (LPWSAOVERLAPPED)pov, NULL);
	if (!dw && err == SOCKET_ERROR)
	{
		if (!pov)
		{
			dw = 0;
...
bool_t _socket_recvfrom(res_file_t so, res_addr_t saddr, int* plen, void* buf, dword_t size, async_t* pb)
{
	...
	dw = 0;
	alen = *plen;
	err = WSARecvFrom((SOCKET)so, &wb, 1, &dw, &fg, (SOCKADDR*)saddr, &alen, (LPWSAOVERLAPPED)pov, NULL);
	*plen = alen;
...
bool_t _socket_send(res_file_t so, void* buf, dword_t len, async_t* pb)
{
	...
	err = WSASend((SOCKET)so, &wb, 1, &dw, fg, (LPWSAOVERLAPPED)pov, NULL);
	if (err == SOCKET_ERROR)
	{
		if (!pov)
		{
...
bool_t _socket_recv(res_file_t so, void* buf, dword_t len, async_t* pb)
{
	...
	err = WSARecv((SOCKET)so, &wb, 1, &dw, &fg, (LPWSAOVERLAPPED)pov, NULL);
	if (err == SOCKET_ERROR)
	{
		if (!pov)
		{
