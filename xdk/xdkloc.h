...
LOC_API void	_error_print(const tchar_t* str);
LOC_API void	_error_debug(const char* src, const char* func, unsigned int line, const char* exp);
#endif

#ifdef XDK_SUPPORT_MBCS
LOC_API int		c_gbk_to_ucs(const schar_t* gbk, int len, wchar_t* ucs, int max);
LOC_API int		c_ucs_to_gbk(const wchar_t* ucs, int len, schar_t* gbk, int max);
LOC_API int		c_utf_to_ucs(const schar_t* utf, int len, wchar_t* ucs, int max);
LOC_API int		c_ucs_to_utf(const wchar_t* ucs, int len, schar_t* utf, int max);
#endif
...
LOC_API bool_t	_file_size(res_file_t fh, vword_t* fs);
...
LOC_API bool_t	_file_read_range(res_file_t fh, vword_t off, void* buf, dword_t size);
LOC_API bool_t	_file_write_range(res_file_t fh, vword_t off, void* buf, dword_t size);
LOC_API void*	_file_lock_range(res_file_t fh, vword_t off, dword_t size, bool_t write, res_file_t* ph);
LOC_API void	_file_unlock_range(res_file_t mh, vword_t off, dword_t size, void* p);
LOC_API bool_t	_file_truncate(res_file_t fh, vword_t off);
LOC_API vlong_t _file_seek_begin(res_file_t fh);
LOC_API vlong_t _file_seek_end(res_file_t fh);
LOC_API vlong_t _file_seek_bytes(res_file_t fh, vlong_t bytes);
LOC_API vlong_t _file_seek_lines(res_file_t fh, vlong_t lines);
LOC_API dword_t _file_peek_line(res_file_t fh, byte_t* buf, dword_t max);
LOC_API dword_t _file_read_line(res_file_t fh, byte_t* buf, dword_t max);
LOC_API dword_t _file_write_line(res_file_t fh, const byte_t* buf, dword_t len);
...
LOC_API wait_t	_pipe_listen(res_file_t pip, async_t* pb);
...
LOC_API void	_process_wait_run(res_proc_t child);
LOC_API void	_process_wait_exit(res_proc_t child);
