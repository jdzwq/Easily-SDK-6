...
{
	XDK_ASSERT(is_dom_doc_node(ptr));
...
link_t_ptr get_dom_next_leaf_node(link_t_ptr ptr, link_t_ptr pos)
{
	link_t_ptr plk, nlk;
	...
	if (pos == LINK_FIRST)
	{
		while (get_dom_first_child_node(plk))
		{
			plk = get_dom_first_child_node(plk);
		}
	}
	else if (pos == LINK_LAST)
	{
		while (get_dom_last_child_node(plk))
		{
			plk = get_dom_last_child_node(plk);
		}
	}
	else
	{
		plk = get_dom_next_sibling_node(pos);
		if (!plk)
		{
			nlk = get_dom_parent_node(pos);
			if (!nlk)
				return NULL;
			...
			plk = get_dom_first_child_node(nlk);
		}
	}
...
link_t_ptr get_dom_prev_leaf_node(link_t_ptr ptr, link_t_ptr pos)
{
	link_t_ptr plk, nlk;
	...
	{
		plk = get_dom_prev_sibling_node(pos);
		if (!plk)
		{
			nlk = get_dom_parent_node(pos);
			if (!nlk)
			...
			nlk = get_dom_prev_sibling_node(nlk);
			if (!nlk)
				return NULL;

			plk = get_dom_last_child_node(nlk);
		}
	}
...
void set_dom_node_line_cator(link_t_ptr ilk, int point, int cx, int cy)
{
	...
	XDK_ASSERT(point >= 0 && point <= ptt->points - 1);
...
void ins_dom_node_line_cator(link_t_ptr ilk, int point, int cx, int cy)
{
	...
	XDK_ASSERT(point >= 0 && point <= ptt->points);
