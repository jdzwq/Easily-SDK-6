...
void svg_get_canvas_interface(canvas_t canv, drawing_interface* pif)
{
	...
	pif->pf_rect_tm_to_pt = (PF_RECT_TM_TO_PT)svg_rect_tm_to_pt;
	pif->pf_rect_pt_to_tm = (PF_RECT_PT_TO_TM)svg_rect_pt_to_tm;
	pif->pf_size_tm_to_pt = (PF_SIZE_TM_TO_PT)svg_size_tm_to_pt;
	pif->pf_size_pt_to_tm = (PF_SIZE_PT_TO_TM)svg_size_pt_to_tm;
	pif->pf_point_tm_to_pt = (PF_POINT_TM_TO_PT)svg_point_tm_to_pt;
	pif->pf_point_pt_to_tm = (PF_POINT_PT_TO_TM)svg_point_pt_to_tm;
...
void svg_get_visual_interface(visual_t visual, drawing_interface* pif)
{
	...
	pif->pf_text_metric = (PF_TEXT_METRIC)svg_text_metric_raw;
...
void svg_get_visual_measure(visual_t view, measure_interface* pim)
{
	...
	pim->pf_measure_pixel = (PF_MEASURE_PIXEL)svg_pixel_metric_raw;
	pim->pf_measure_font = (PF_MEASURE_FONT)svg_text_metric_raw;
...
void svg_get_canvas_measure(canvas_t canv, measure_interface* pim)
{
	...
	pim->pf_measure_pixel = (PF_MEASURE_PIXEL)svg_pixel_metric;
	pim->pf_measure_font = (PF_MEASURE_FONT)svg_text_metric;
