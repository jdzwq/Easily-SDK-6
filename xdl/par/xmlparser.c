...
bool_t parse_xml(xml_write_interface* pxp, int encode, opera_interface* pbo)
{
	...
	while(ma.ma != STOP)
	{
		...
		if (ma.ma != STOP)
		{
			...
			{
			case NIL_SKIP: //空闲略过
				if (_IsSpaceChar(ma.cur[0])){ ma.sa = NOO; ma.ms = NIL_SKIP; ma.ma = NEXT; }	//忽略起始空格
				else if (ma.cur[0] == _T('<')){ ma.sa = NOO; ma.ms = TG_OPEN; ma.ma = NEXT; } //遇到标记
				else if (ma.cur[0] == _T('\0')) { ma.sa = NOO; ma.ms = NIL_SUCCEED, ma.ma = STOP; }	//终结符
				//else { ma.sa = NOO; ma.ms = NIL_FAILED; ma.ma = STOP; }	//无效字符
				else { ma.sa = NOO; ma.ms = NIL_SKIP; ma.ma = NEXT; }	//无效字符
				break;
			case TG_OPEN: //标记打开
				if (_IsSpaceChar(ma.cur[0])){ ma.sa = NOO; ma.ms = TG_OPEN; ma.ma = NEXT; } //忽略空格
				else if (ma.cur[0] == _T('?')){ ma.sa = NOO; ma.ms = IS_OPEN; ma.ma = NEXT; }	//遇到指令符
				else if (ma.cur[0] == _T('!')){ ma.sa = NOO; ma.ms = DS_OPEN; ma.ma = NEXT; } //遇到声明符
				else if (ma.cur[0] == _T('/')){ ma.sa = NOO; ma.ms = NS_REVER_CLOSE; ma.ma = NEXT; } //准备关闭标记
				else { ma.sa = NOO; ma.ms = NS_OPEN; ma.ma = PAUSE; } //元素开始
				break;
			case IS_OPEN: //指令打开
				if (_IsSpaceChar(ma.cur[0])){ ma.sa = NOO; ma.ms = IS_OPEN; ma.ma = NEXT; } //忽略空格
				else if (_IsNamePre(ma.cur[0])){ ma.sa = NOO; ma.ms = IS_NAME_BEGIN; ma.ma = PAUSE; } //指令名开始
				else { ma.sa = NOO; ma.ms = NIL_FAILED; ma.ma = STOP; } //无效字符
				break;
			case IS_NAME_BEGIN: //指令名开始
				if (_IsNameChar(ma.cur[0])){ ma.sa = NOO; ma.ms = IS_NAME_BEGIN; ma.ma = NEXT; } //继续指令名
				else if (_IsSpaceChar(ma.cur[0])){ ma.sa = NOO; ma.ms = IS_NAME_END; ma.ma = PAUSE; } //指令名结束
				else if (ma.cur[0] == _T('?')){ ma.sa = NOO; ma.ms = IS_NAME_END; ma.ma = PAUSE; } //指令名结束
				else { ma.sa = NOO; ma.ms = NIL_FAILED; ma.ma = STOP; } //无效字符
				break;
			case IS_NAME_END: //指令名结束
				if (_IsSpaceChar(ma.cur[0])){ ma.sa = NOO; ma.ms = IS_NAME_END; ma.ma = NEXT; } //忽略空格
				else if (ma.cur[0] == _T('?')){ ma.sa = NOO; ma.ms = IS_CLOSE; ma.ma = NEXT; } //指令名结束
				else if (_IsNamePre(ma.cur[0])){ ma.sa = NOO; ma.ms = IS_KEY_BEGIN; ma.ma = PAUSE; } //指令属性名开始
				else { ma.sa = NOO; ma.ms = NIL_FAILED; ma.ma = STOP; } //无效字符
				break;
			case IS_KEY_BEGIN: //指令属性名开始
				if (_IsNameChar(ma.cur[0])){ ma.sa = NOO; ma.ms = IS_KEY_BEGIN; ma.ma = NEXT; } //继续属性名
				else if (_IsSpaceChar(ma.cur[0])){ ma.sa = NOO; ma.ms = IS_KEY_END; ma.ma = PAUSE; } //属性名结束
				else if (ma.cur[0] == _T('=')){ ma.sa = NOO; ma.ms = IS_KEY_END; ma.ma = PAUSE; } //属性名结束
				else { ma.sa = NOO; ma.ms = NIL_FAILED; ma.ma = STOP; } //无效字符
				break;
			case IS_KEY_END: //指令属性名结束
				if (_IsSpaceChar(ma.cur[0])){ ma.sa = NOO; ma.ms = IS_KEY_END; ma.ma = NEXT; } //忽略空格
				else if (ma.cur[0] == _T('=')){ ma.sa = NOO; ma.ms = IS_ASIGN; ma.ma = NEXT; } //属性赋值符
				else { ma.sa = NOO; ma.ms = NIL_FAILED; ma.ma = STOP; } //无效字符
				break;
			case IS_ASIGN: //指令属性赋值开始
				if (_IsSpaceChar(ma.cur[0])){ ma.sa = NOO; ma.ms = IS_ASIGN; ma.ma = NEXT; } //忽略空格
				else if (ma.cur[0] == _T('\"') || ma.cur[0] == _T('\'')){ ma.sa = NOO; ma.ms = IS_VAL_QUOT; ma.ma = NEXT; } //过渡到属性赋值,掠过"\""
				else { ma.sa = NOO; ma.ms = NIL_FAILED; ma.ma = STOP; } //无效字符
				break;
			case IS_VAL_QUOT: //括号
				if (_IsTokenChar(ma.cur[0])){ ma.sa = NOO; ma.ms = IS_VAL_BEGIN; ma.ma = PAUSE; } //开始属性值
				else { ma.sa = NOO; ma.ms = NIL_FAILED; ma.ma = STOP; } //无效字符
				break;
			case IS_VAL_BEGIN: //指令属性值开始
				if (ma.cur[0] == _T('\"') || ma.cur[0] == _T('\'')){ ma.sa = NOO; ma.ms = IS_VAL_END; ma.ma = PAUSE; } //过渡到属性值结束
				else if (_IsTokenChar(ma.cur[0])){ ma.sa = NOO; ma.ms = IS_VAL_BEGIN; ma.ma = NEXT; } //继续属性值
				else { ma.sa = NOO; ma.ms = NIL_FAILED; ma.ma = STOP; } //无效字符
				break;
			case IS_VAL_END:  //指令属性值结束
				if (ma.cur[0] == _T('\"') || ma.cur[0] == _T('\'')){ ma.sa = NOO; ma.ms = IS_VAL_END; ma.ma = NEXT; } //过渡到属性值结束
				else if (_IsSpaceChar(ma.cur[0])){ ma.sa = NOO; ma.ms = IS_VAL_END; ma.ma = NEXT; } //忽略空格
				else if (ma.cur[0] == _T('?')){ ma.sa = NOO; ma.ms = IS_CLOSE; ma.ma = NEXT; } //标记中断
				else if (_IsNamePre(ma.cur[0])){ ma.sa = NOO; ma.ms = IS_KEY_BEGIN; ma.ma = PAUSE; } //属性名开始
				else { ma.sa = NOO; ma.ms = NIL_FAILED; ma.ma = STOP; } //无效字符
				break;
			case IS_CLOSE: //指令关闭
				if (_IsSpaceChar(ma.cur[0])){ ma.sa = NOO; ma.ms = IS_CLOSE; ma.ma = NEXT; } //忽略空格
				else if (ma.cur[0] == _T('>')){ ma.sa = NOO; ma.ms = TG_CLOSE; ma.ma = NEXT; } //标记关闭
				else { ma.sa = NOO; ma.ms = NIL_FAILED; ma.ma = STOP; } //无效字符
				break;
			case DS_OPEN: //申明打开
				if (_IsSpaceChar(ma.cur[0])){ ma.sa = NOO; ma.ms = DS_OPEN; ma.ma = NEXT; } //忽略空格
				else if (ma.cur[0] == _T('-')){ ma.sa = NOO; ma.ms = CM_BEGIN; ma.ma = NEXT; } //遇到注释符
				else if (ma.cur[0] == _T('[')){ ma.sa = NOO; ma.ms = CD_HEAD_BEGIN; ma.ma = NEXT; } //遇到界段符
				else if (ma.cur[0] == _T('>')){ ma.sa = NOO; ma.ms = DS_CLOSE; ma.ma = PAUSE; } //过渡到声明关闭
				else { ma.sa = NOO; ma.ms = DS_OPEN; ma.ma = NEXT; } //忽略声明
				break;
			case CM_BEGIN: //注释文本开始
				if (ma.cur[0] == _T('-')){ ma.sa = NOO; ma.ms = CM_TEXT_BEGIN; ma.ma = NEXT; } //过渡到注释文本开始
				else { ma.sa = NOO; ma.ms = NIL_FAILED; ma.ma = STOP; } //无效字符
				break;
			case CM_TEXT_BEGIN: //注释文本开始
				if (ma.cur[0] == _T('-')){ ma.sa = NOO; ma.ms = CM_TEXT_END; ma.ma = NEXT; } //遇到注释结束
				else { ma.sa = NOO; ma.ms = CM_TEXT_BEGIN; ma.ma = NEXT; } //继续注释文本
				break;
			case CM_TEXT_END: //注释文本结束
				if (ma.cur[0] == _T('-')){ ma.sa = NOO; ma.ms = CM_TEXT_END; ma.ma = NEXT; } //过渡到注释结束
				else if (ma.cur[0] == _T('>')){ ma.sa = NOO; ma.ms = CM_END; ma.ma = PAUSE; } //过渡到注释结束
				else { ma.sa = NOO; ma.ms = CM_TEXT_BEGIN; ma.ma = NEXT; } //修正为有效字符
				break;
			case CM_END: //注释结束
				{ ma.sa = NOO; ma.ms = DS_CLOSE; ma.ma = PAUSE; } //过渡到声明关闭
				break;
			case CD_HEAD_BEGIN: //界段头开始
				if (ma.cur[0] == _T('C')){ ma.sa = NOO; ma.ms = CD_HEAD_BEGIN; ma.ma = NEXT; } //掠过界段头
				else if (ma.cur[0] == _T('D')){ ma.sa = NOO; ma.ms = CD_HEAD_BEGIN; ma.ma = NEXT; } //掠过界段头
				else if (ma.cur[0] == _T('A')){ ma.sa = NOO; ma.ms = CD_HEAD_BEGIN; ma.ma = NEXT; } //掠过界段头
				else if (ma.cur[0] == _T('T')){ ma.sa = NOO; ma.ms = CD_HEAD_BEGIN; ma.ma = NEXT; } //掠过界段头
				else if (ma.cur[0] == _T('A')){ ma.sa = NOO; ma.ms = CD_HEAD_BEGIN; ma.ma = NEXT; } //掠过界段头
				else if (ma.cur[0] == _T('[')){ ma.sa = NOO; ma.ms = CD_HEAD_END; ma.ma = NEXT; } //掠过界段头
				else { ma.sa = NOO; ma.ms = NIL_FAILED; ma.ma = STOP; } //无效字符
				break;
			case CD_HEAD_END: //界段头结束
				{ ma.sa = NOO; ma.ms = CD_TEXT_BEGIN; ma.ma = PAUSE; } //开始界段文本
				break;
			case CD_TEXT_BEGIN: //界段文本开始
				if (ma.cur[0] == _T(']')){ ma.sa = NOO; ma.ms = CD_TEXT_END; ma.ma = NEXT; } //遇到界段结束
				else { ma.sa = NOO; ma.ms = CD_TEXT_BEGIN; ma.ma = NEXT; } //继续界段文本
				break;
			case CD_TEXT_END: //界段文本结束
				if (ma.cur[0] == _T(']')){ ma.sa = NOO; ma.ms = CD_TAIL_BEGIN; ma.ma = NEXT; } //遇到界段结束
				else { ma.sa = NOO; ma.ms = CD_TEXT_BEGIN; ma.ma = NEXT; } //修正为有效字符
				break;
			case CD_TAIL_BEGIN:
				if (ma.cur[0] == _T('>')){ ma.sa = NOO; ma.ms = CD_TAIL_END; ma.ma = PAUSE; } //过渡到界段结束
				else if (ma.cur[0] == _T(']')){ ma.sa = NOO; ma.ms = CD_TAIL_BEGIN; ma.ma = NEXT; }//继续界段结束
				else { ma.sa = NOO; ma.ms = NIL_FAILED; ma.ma = STOP; } //无效字符
				break;
			case CD_TAIL_END: //界段结束
				...
				break;
			case DS_CLOSE: //申明结束
				if (ma.cur[0] == _T('>')){ ma.sa = NOO; ma.ms = TG_CLOSE; ma.ma = NEXT; } //过渡到标记关闭
				else { ma.sa = NOO; ma.ms = NIL_FAILED; ma.ma = STOP; } //无效字符
				break;
			case NS_OPEN: //元素打开
				if (_IsSpaceChar(ma.cur[0])){ ma.sa = NOO; ma.ms = NS_OPEN; ma.ma = NEXT; } //忽略空格
				else if (_IsNamePre(ma.cur[0])){ ma.sa = PUSH; ma.ms = NS_NAME_BEGIN; ma.ma = PAUSE; } //指令名开始
				else { ma.sa = NOO; ma.ms = NIL_FAILED; ma.ma = STOP; } //无效字符
				break;
			case NS_NAME_BEGIN: //元素命名开始
				if (_IsNameChar(ma.cur[0])){ ma.sa = NOO; ma.ms = NS_NAME_BEGIN; ma.ma = NEXT; } //元素名继续
				else if (_IsSpaceChar(ma.cur[0])){ ma.sa = NOO; ma.ms = NS_NAME_END; ma.ma = PAUSE; } //元素名结束
				else if (ma.cur[0] == _T('>')){ ma.sa = NOO; ma.ms = NS_NAME_END; ma.ma = PAUSE; }//元素名结束
				else if (ma.cur[0] == _T('/')){ ma.sa = NOO; ma.ms = NS_NAME_END; ma.ma = PAUSE; } //元素名结束
				else { ma.sa = NOO; ma.ms = NIL_FAILED; ma.ma = STOP; }	//无效字符
				break;
			case NS_NAME_END: //元素命名结束
				if (_IsSpaceChar(ma.cur[0])){ ma.sa = NOO; ma.ms = NS_NAME_END; ma.ma = NEXT; } //忽略空格
				else if (ma.cur[0] == _T('>')){ ma.sa = NOO; ma.ms = NS_INNER_CLOSE; ma.ma = NEXT; } //元素名结束
				else if (ma.cur[0] == _T('/')){ ma.sa = NOO; ma.ms = NS_REVER_CLOSE; ma.ma = NEXT; } //元素名结束
				else if (_IsNamePre(ma.cur[0])){ ma.sa = NOO; ma.ms = NS_KEY_BEGIN; ma.ma = PAUSE; } //属性名开始
				else { ma.sa = NOO; ma.ms = NIL_FAILED; ma.ma = STOP; } //无效字符
				break;
			case NS_KEY_BEGIN: //元素属性名开始
				if (_IsNameChar(ma.cur[0])){ ma.sa = NOO; ma.ms = NS_KEY_BEGIN; ma.ma = NEXT; } //继续属性名
				else if (_IsSpaceChar(ma.cur[0])){ ma.sa = NOO; ma.ms = NS_KEY_END; ma.ma = PAUSE; } //属性名结束
				else if (ma.cur[0] == _T('=')){ ma.sa = NOO; ma.ms = NS_KEY_END; ma.ma = PAUSE; } //属性赋值符
				else { ma.sa = NOO; ma.ms = NIL_FAILED; ma.ma = STOP; } //无效字符
				break;
			case NS_KEY_END: //元素属性名结束
				if (_IsSpaceChar(ma.cur[0])){ ma.sa = NOO; ma.ms = NS_KEY_END; ma.ma = NEXT; } //忽略空格
				else if (ma.cur[0] == _T('=')){ ma.sa = NOO; ma.ms = NS_ASIGN; ma.ma = NEXT; } //属性赋值符
				else { ma.sa = NOO; ma.ms = NIL_FAILED; ma.ma = STOP; } //无效字符
				break;
			case NS_ASIGN: //元素属性赋值
				if (_IsSpaceChar(ma.cur[0])){ ma.sa = NOO; ma.ms = NS_ASIGN; ma.ma = NEXT; } //忽略空格
				else if (ma.cur[0] == _T('\"') || ma.cur[0] == _T('\'')){ ma.sa = NOO; ma.ms = NS_VAL_QUOT; ma.ma = NEXT; } //过渡到属性值开始,掠过“\"”
				else { ma.sa = NOO; ma.ms = NIL_FAILED; ma.ma = STOP; } //无效字符
				break;
			case NS_VAL_QUOT:
				if (_IsTokenChar(ma.cur[0])){ ma.sa = NOO; ma.ms = NS_VAL_BEGIN; ma.ma = PAUSE; } //开始属性值
				else if (ma.cur[0] == _T('\"') || ma.cur[0] == _T('\'')){ ma.sa = NOO; ma.ms = NS_VAL_BEGIN; ma.ma = PAUSE; } //过渡到属性值开始
				else { ma.sa = NOO; ma.ms = NIL_FAILED; ma.ma = STOP; } //无效字符
				break;
			case NS_VAL_BEGIN: //元素属性值开始
				if (_IsTokenChar(ma.cur[0])){ ma.sa = NOO; ma.ms = NS_VAL_BEGIN; ma.ma = NEXT; } //继续属性值
				else if (ma.cur[0] == _T('\"') || ma.cur[0] == _T('\'')){ ma.sa = NOO; ma.ms = NS_VAL_END; ma.ma = PAUSE; } //过渡到属性值结束
				else { ma.sa = NOO; ma.ms = NIL_FAILED; ma.ma = STOP; } //无效字符
				break;
			case NS_VAL_END: //元素属性值结束
				if (ma.cur[0] == _T('\"') || ma.cur[0] == _T('\'')){ ma.sa = NOO; ma.ms = NS_VAL_END; ma.ma = NEXT; } //属性值结束
				else if (_IsSpaceChar(ma.cur[0])){ ma.sa = NOO; ma.ms = NS_VAL_END; ma.ma = NEXT; } //忽略空格
				else if (ma.cur[0] == _T('/')){ ma.sa = NOO; ma.ms = NS_REVER_CLOSE; ma.ma = NEXT; } //准备关闭标记
				else if (ma.cur[0] == _T('>')){ ma.sa = NOO; ma.ms = NS_INNER_CLOSE; ma.ma = NEXT; } //标记中断
				else if (_IsNamePre(ma.cur[0])){ ma.sa = NOO; ma.ms = NS_KEY_BEGIN; ma.ma = PAUSE; } //属性名开始
				else { ma.sa = NOO; ma.ms = NIL_FAILED; ma.ma = STOP; } //无效字符
				break;
			case NS_INNER_CLOSE: //元素命名中断
				if (ma.cur[0] == _T('<')){ ma.sa = NOO; ma.ms = TG_OPEN; ma.ma = NEXT; } //标记开始
				else{ ma.sa = NOO; ma.ms = NS_TEXT_BEGIN; ma.ma = PAUSE; } //元素文本开始
				break;
			case NS_TEXT_BEGIN: //元素文本开始
				//if (ma.cur[0] == _T('['){ ma.sa = NOO; ma.ms = NS_TEXT_BEGIN; ma.ma = SKIP_CDT_HEAD; } //界段开始
				//if (ma.cur[0] == _T(']'){ ma.sa = NOO; ma.ms = NS_TEXT_BEGIN; ma.ma = SKIP_CDT_TAIL; } //界段结束
				if (ma.cur[0] == _T('<')){ ma.sa = NOO; ma.ms = NS_TEXT_END; ma.ma = PAUSE; } //过渡到元素文本结束
				else if (_IsTextChar(ma.cur[0])){ ma.sa = NOO; ma.ms = NS_TEXT_BEGIN; ma.ma = NEXT; } //继续元素文本
				else{ ma.sa = NOO; ma.ms = NIL_FAILED; ma.ma = STOP; } //无效字符
				break;
			...
			case NS_REVER_CLOSE: //元素预关闭
				if (ma.cur[0] == _T('>')){ ma.sa = NOO; ma.ms = NS_CLOSE; ma.ma = PAUSE; }//标记结束
				else if (_IsNamePre(ma.cur[0])){ ma.sa = NOO; ma.ms = NS_REVER_BEGIN; ma.ma = PAUSE; } //关闭标记的元素名开始
				else { ma.sa = NOO; ma.ms = NIL_FAILED; ma.ma = STOP; } //无效字符
				break;
			case NS_REVER_BEGIN: //元素关闭结构中的元素名开始
				if (ma.cur[0] == _T('>')){ ma.sa = NOO; ma.ms = NS_REVER_END; ma.ma = PAUSE; } //关闭标记的元素名结束
				else if (_IsNameChar(ma.cur[0])){ ma.sa = NOO; ma.ms = NS_REVER_BEGIN; ma.ma = NEXT; } //继续关闭标记的元素名
				else { ma.sa = NOO; ma.ms = NIL_FAILED; ma.ma = STOP; } //无效字符
				break;
			...
			case NS_CLOSE: //元素关闭
				...
				break;
			case TG_CLOSE: //标记关闭
				{ma.sa = NOO; ma.ms = NIL_SKIP; ma.ma = PAUSE; }
				break;
			default:
				{ma.sa = NOO; ma.ms = NIL_FAILED; ma.ma = STOP; } //无效字符
				break;
			}
		}
		else if (ma.ma != STOP)
