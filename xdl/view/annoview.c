...
void draw_anno(const drawing_interface* pci, link_t_ptr ptr)
{
	link_t_ptr ilk;
	xbrush_t xb = { 0 };
	xpen_t xp = { 0 };
	...
	while (ilk)
	{
		...
		if (compare_text(type, -1, ATTR_ANNO_TYPE_TEXT, -1, 0) == 0)
		{
			...
			(*pci->drw->pf_draw_text)(pci->ctx, &xa, &xr, get_anno_arti_text_ptr(ilk), -1);
		}
		else if (compare_text(type, -1, ATTR_ANNO_TYPE_ICON, -1, 0) == 0)
		{
			...
			draw_gizmo(pci, &xc, &xr, get_anno_arti_text_ptr(ilk));
		}
		else if (compare_text(type, -1, ATTR_ANNO_TYPE_CIRCLE, -1, 0) == 0)
		{
			if(style)
		...
		else if (compare_text(type, -1, ATTR_ANNO_TYPE_RANGE, -1, 0) == 0)
		{
			...
			//(*pci->drw->pf_text_out)(pci->ctx, &xa, &pt1, token, -1);
		...
		else if (compare_text(type, -1, ATTR_ANNO_TYPE_ANGLE, -1, 0) == 0)
		{
			...
			//(*pci->drw->pf_draw_arc)(pci->ctx, &xp, &ppt[0], 2, 2, a1, a2);
