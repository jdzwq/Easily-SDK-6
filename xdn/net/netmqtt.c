...
static bool_t _mqtt_write_pdu(mqtt_context* mqtt, byte_t pdu_type, dword_t pdv_size)
{
	...
	case MQTT_PDU_CONNECT:
		...
		if (!(*pbio->pf_write)(pbio->fd, ctl, &ctl_size))
		{
		...
		if (!(*pbio->pf_write)(pbio->fd, buf, &var_size))
		{
			raise_user_error(NULL, NULL);
		}
	...
	XDK_ASSERT(total == ctl_size + var_size + pdv_size);
...
static bool_t _mqtt_read_pdu(mqtt_context* mqtt, byte_t* pdu_type, dword_t* pdv_size)
{
	...
	*pdu_type = (buf[0] & 0xF0) >> 4;
	...
	{
		n = 1;
		if (!(*pbio->pf_read)(pbio->fd, buf + pos, &n))
		{
			raise_user_error(NULL, NULL);
		}
	...
	case MQTT_PDU_PUBLISH:
		...
		if (!(*pbio->pf_read)(pbio->fd, mqtt->topic_name, &n))
		{
			raise_user_error(NULL, NULL);
		}
		pdu_size -= n;
...
xhand_t xmqtt_scu(xhand_t bio, int scu)
{
	...
	XDK_ASSERT(scu == _MQTT_TYPE_SCU_PUB || scu == _MQTT_TYPE_SCU_SUB);
	...
	xdn_bio_interface(bio, pmqtt->pif);
...
xhand_t xmqtt_bio(xhand_t mqtt)
{
	...
	XDK_ASSERT(mqtt && mqtt->tag == _HANDLE_MQTT);
...
bool_t xmqtt_accept(xhand_t mqtt)
{
	...
	XDK_ASSERT(pmqtt->type == _MQTT_TYPE_SCP_SUB || pmqtt->type == _MQTT_TYPE_SCP_PUB || pmqtt->type == _MQTT_TYPE_SCP_UNK);

	byte_t* pdv_buf = NULL;
	dword_t pdv_size;
	byte_t pdu_type = 0;

	bio_interface* pbio = pmqtt->pif;
	...
	if (!(*pbio->pf_read)(pbio->fd, pdv_buf, &pdv_size))
	{
		raise_user_error(NULL, NULL);
	}
...
bool_t xmqtt_connect(xhand_t mqtt)
{
	...
	XDK_ASSERT(pmqtt->type == _MQTT_TYPE_SCU_SUB || pmqtt->type == _MQTT_TYPE_SCU_PUB);
	...
	if (!(*pbio->pf_write)(pbio->fd, pdv_buf, &pdv_size))
	{
		raise_user_error(NULL, NULL);
	}
...
bool_t xmqtt_subcribe(xhand_t mqtt, const tchar_t* topic, int len)
{
	...
	XDK_ASSERT(pmqtt->type == _MQTT_TYPE_SCU_SUB);
...
bool_t xmqtt_poll_subscribe(xhand_t mqtt, tchar_t* topic, int len)
{
	...
	XDK_ASSERT(pmqtt->type == _MQTT_TYPE_SCP_SUB || pmqtt->type == _MQTT_TYPE_SCP_PUB);
...
bool_t xmqtt_poll_message(xhand_t mqtt, byte_t** pbuf, dword_t* plen)
{
	...
	XDK_ASSERT(pmqtt->type == _MQTT_TYPE_SCU_SUB || pmqtt->type == _MQTT_TYPE_SCP_PUB || pmqtt->type == _MQTT_TYPE_SCP_SUB || pmqtt->type == _MQTT_TYPE_SCP_UNK);
	...
	*plen = 0;
	...
	if (pmqtt->type == _MQTT_TYPE_SCU_SUB)
	{
		...
		if (!(*pbio->pf_read)(pbio->fd, pmqtt->message_data, &(pmqtt->message_size)))
		{
			raise_user_error(NULL, NULL);
		}
...
bool_t xmqtt_publish(xhand_t mqtt, const tchar_t* topic, int len)
{
	...
	XDK_ASSERT(pmqtt->type == _MQTT_TYPE_SCU_PUB || pmqtt->type == _MQTT_TYPE_SCP_SUB);
...
bool_t xmqtt_push_message(xhand_t mqtt, const byte_t* buf, dword_t len)
{
	...
	if (pmqtt->type == _MQTT_TYPE_SCU_PUB)
	{
		...
		if (!(*pbio->pf_write)(pbio->fd, buf, &len))
		{
			raise_user_error(NULL, NULL);
		}
