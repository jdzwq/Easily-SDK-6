...
typedef struct _ssh_context{
	...
	dhm_context kex_dhm;
	//server or client key exchange hash context, eg: sha1_context, sha256_context, sha512_context
	void* kex_sha;
...
static int _ssh_write_packet(ssh_context* pssh, byte_t* payload, dword_t size)
{
	...
	{
		sha256_hmac_starts((sha256_context*)pssh->snd_hmac, pssh->snd_mac, pssh->len_snd_mac, 0);
	}
	else if (a_xsicmp(hmac_name, SSH_HMAC_SHA2_512) == 0)
	{
		sha512_hmac_starts((sha512_context*)pssh->snd_hmac, pssh->snd_mac, pssh->len_snd_mac, 0);
	}
	...
	{
		sha256_hmac_update((sha256_context*)pssh->snd_hmac, num_buf, n);
	}
	else if (a_xsicmp(hmac_name, SSH_HMAC_SHA2_512) == 0)
	{
		sha512_hmac_update((sha512_context*)pssh->snd_hmac, num_buf, n);
	}
	...
	{
		sha256_hmac_update((sha256_context*)pssh->snd_hmac, payload, n);
	}
	else if (a_xsicmp(hmac_name, SSH_HMAC_SHA2_512) == 0)
	{
		sha512_hmac_update((sha512_context*)pssh->snd_hmac, payload, n);
	}
	//encrypt
	if (a_xsicmp(ciph_name, SSH_CIPH_ARC4) == 0 || a_xsicmp(ciph_name, SSH_CIPH_ARC4_128) == 0 || a_xsicmp(ciph_name, SSH_CIPH_ARC4_256) == 0)
	{
	...
	(*pssh->f_rng)(pssh->r_rng, rng_buf, n);
	...
	{
		sha256_hmac_update((sha256_context*)pssh->snd_hmac, rng_buf, n);
	}
	else if (a_xsicmp(hmac_name, SSH_HMAC_SHA2_512) == 0)
	{
		sha512_hmac_update((sha512_context*)pssh->snd_hmac, rng_buf, n);
	}
	//encrypt
	if (a_xsicmp(ciph_name, SSH_CIPH_ARC4) == 0 || a_xsicmp(ciph_name, SSH_CIPH_ARC4_128) == 0 || a_xsicmp(ciph_name, SSH_CIPH_ARC4_256) == 0)
	{
	...
	{
		sha256_hmac_finish((sha256_context*)pssh->snd_hmac, mac_buf);
	}
	else if (a_xsicmp(hmac_name, SSH_HMAC_SHA2_512) == 0)
	{
		sha512_hmac_finish((sha512_context*)pssh->snd_hmac, mac_buf);
	}
...
static int _ssh_read_packet(ssh_context* pssh, byte_t** pbuf, dword_t* psize)
{
	...
	{
		sha256_hmac_starts((sha256_context*)pssh->rcv_hmac, pssh->rcv_mac, pssh->len_rcv_mac, 0);
	}
	else if (a_xsicmp(hmac_name, SSH_HMAC_SHA2_512) == 0)
	{
		sha512_hmac_starts((sha512_context*)pssh->rcv_hmac, pssh->rcv_mac, pssh->len_rcv_mac, 0);
	}
	...
	{
		sha256_hmac_update((sha256_context*)pssh->rcv_hmac, num_buf, n);
	}
	else if (a_xsicmp(hmac_name, SSH_HMAC_SHA2_512) == 0)
	{
		sha512_hmac_update((sha512_context*)pssh->rcv_hmac, num_buf, n);
	}
	...
	*psize = n;
	...
	{
		sha256_hmac_update((sha256_context*)pssh->rcv_hmac, payload, n);
	}
	else if (a_xsicmp(hmac_name, SSH_HMAC_SHA2_512) == 0)
	{
		sha512_hmac_update((sha512_context*)pssh->rcv_hmac, payload, n);
	}
	...
	{
		sha256_hmac_update((sha256_context*)pssh->rcv_hmac, rng_buf, n);
	}
	else if (a_xsicmp(hmac_name, SSH_HMAC_SHA2_512) == 0)
	{
		sha512_hmac_update((sha512_context*)pssh->rcv_hmac, rng_buf, n);
	}
	...
	{
		sha256_hmac_finish((sha256_context*)pssh->rcv_hmac, mac_buf);
	}
	else if (a_xsicmp(hmac_name, SSH_HMAC_SHA2_512) == 0)
	{
		sha512_hmac_finish((sha512_context*)pssh->rcv_hmac, mac_buf);
	}
	...
	if (xmem_comp((void*)mac_buf, (void*)rcv_mac, hmac_len) != 0)
	{
...
static void _ssh_start_kex_secret(ssh_context* pssh)
{
	...
	{
		pssh->kex_sha = (sha512_context*)xmem_alloc(sizeof(sha512_context));
		sha512_starts((sha512_context*)pssh->kex_sha, 0);
		pssh->len_H = 0;
	}
	else if (a_xsicmp(kex_dh, SSH_KEXDH_GROUP16_SHA512) == 0)
	{
...
static void _ssh_update_kex_secret(ssh_context* pssh, const byte_t* buf, dword_t len)
{
	...
	{
		sha512_update((sha512_context*)pssh->kex_sha, buf, len);
	}
	else if (a_xsicmp(kex_dh, SSH_KEXDH_GROUP16_SHA512) == 0)
	{
		sha512_update((sha512_context*)pssh->kex_sha, buf, len);
...
static void _ssh_finish_kex_secret(ssh_context* pssh)
{
	...
	else if (a_xsicmp(kex_dh, SSH_KEXDH_GROUP16_SHA512) == 0)
	{
		...
		pssh->kex_H = (byte_t*)xmem_alloc(pssh->len_H);
		sha512_finish((sha512_context*)pssh->kex_sha, pssh->kex_H);
	}
	else if (a_xsicmp(kex_dh, SSH_KEXDH_GROUP16_SHA512) == 0)
	{
		pssh->len_H = SSH_HMAC_SHA2_512_SIZE;
...
{
	pssh->f_rng = havege_random;
	pssh->r_rng = xmem_alloc(sizeof(havege_state));
	havege_init((havege_state*)pssh->r_rng);
}
...
static void _ssh_uninit(ssh_context* pssh)
{
	...
	if (pssh->r_rng)
	{
		havege_free((havege_state*)pssh->r_rng);
		xmem_free(pssh->r_rng);
	}
}
...
static dword_t _ssh_format_kexinit(ssh_context* pssh, byte_t* buf, dword_t max)
{
	...
	if (buf)
	{
		...
		(*pssh->f_rng)(pssh->r_rng, (buf + total), SSH_COOKIE_SIZE);
	}
...
static bool_t _ssh_make_dh(ssh_context* pssh)
{
	...
	if (pssh->type == SSH_TYPE_SERVER)
	{
		...
		if (C_OK != dhm_make_public(&(pssh->kex_dhm), bits, pssh->kex_F, pssh->len_F, pssh->f_rng, pssh->r_rng))
		{
			return 0;
		}
		...
		if (a_xscmp(kex_sec, SSH_KEYSECRET_RSA) == 0)
		{
			...
			rsa_init((rsa_context*)pssh->kex_ctx, RSA_PKCS_V15, RSA_HASH_SHA1);

			rsa_gen_key((rsa_context*)pssh->kex_ctx, pssh->f_rng, pssh->r_rng, bits, SSH_MIN_PRIM);
		}
	}
	else
	{
		...
		if (C_OK != dhm_make_public(&(pssh->kex_dhm), bits, pssh->kex_E, pssh->len_E, pssh->f_rng, pssh->r_rng))
		{
			return 0;
		}
...
static int _ssh_send_dhinit(ssh_context* pssh)
{
	...
	XDK_ASSERT(pssh->type == SSH_TYPE_CLIENT);
...
static int _ssh_recv_dhinit(ssh_context* pssh)
{
	...
	XDK_ASSERT(pssh->type == SSH_TYPE_SERVER);
...
static bool_t _ssh_parse_pubkey(ssh_context* pssh, const byte_t* buf, dword_t size)
{
	...
	if (a_xsicmp(kex_type, SSH_KEYSECRET_RSA) == 0)
	{
		...
		//read mpint e and n
		p = buf + total;
		if (C_OK != rsa_import_pubkey((rsa_context*)pssh->kex_ctx, &p, (p + n), 0))
			return 0;
...
static bool_t _ssh_make_session(ssh_context* pssh)
{
	...
	sha1_context sha1 = { 0 };
	sha256_context sha2 = { 0 };
	sha512_context sha4 = { 0 };
	...
	//calc the K mpi
	len_K = pssh->kex_dhm.len;
	if (C_OK != dhm_calc_secret(&(pssh->kex_dhm), K, len_K, &len_K, pssh->f_rng, pssh->r_rng))
	{
		return 0;
	}
	...
	if (pssh->type == SSH_TYPE_SERVER)
	{
		...
		if (a_xscmp(kex_sec, SSH_KEYSECRET_RSA) == 0)
		{
			pssh->len_S= rsa_get_len((rsa_context*)pssh->kex_ctx);
			pssh->kex_S = (byte_t*)xmem_alloc(pssh->len_S);

			if (C_OK != rsa_pkcs1_sign((rsa_context*)pssh->kex_ctx, pssh->f_rng, pssh->r_rng, RSA_PRIVATE, RSA_HASH_SHA1, pssh->len_H, pssh->kex_H, pssh->kex_S))
				return 0;
		}
	}
	else
	{
		...
		if (a_xscmp(kex_sec, SSH_KEYSECRET_RSA) == 0)
		{
			if (C_OK != rsa_pkcs1_verify((rsa_context*)pssh->kex_ctx, pssh->f_rng, pssh->r_rng, RSA_PUBLIC, RSA_HASH_SHA1, pssh->len_H, pssh->kex_H, pssh->kex_S))
				return 0;
		}
	}
	...
	else if (a_xsicmp(kex_dh, SSH_KEXDH_GROUP16_SHA512) == 0 || a_xsicmp(kex_dh, SSH_KEXDH_GROUP18_SHA512) == 0)
	{
		...
		{
			//client->server: HASH(K || H || "A" || session_id)
			sha512_starts(&sha4, 0);
			sha512_update(&sha4, K, len_K);
			sha512_update(&sha4, pssh->kex_H, pssh->len_H);
			sha512_update(&sha4, "A", 1);
			sha512_update(&sha4, pssh->pid_sess, pssh->len_sess);
			sha512_finish(&sha4, pssh->rcv_blk);
			...
			while (pssh->len_rcv_blk < blk_size)
			{
				...
				sha512_update(&sha4, K, len_K);
				sha512_update(&sha4, pssh->kex_H, pssh->len_H);
				sha512_update(&sha4, pssh->rcv_blk, pssh->len_rcv_blk);
				sha512_finish(&sha4, pssh->rcv_blk + pssh->len_rcv_blk);
			...
			sha512_update(&sha4, K, len_K);
			sha512_update(&sha4, pssh->kex_H, pssh->len_H);
			sha512_update(&sha4, "B", 1);
			sha512_update(&sha4, pssh->pid_sess, pssh->len_sess);
			sha512_finish(&sha4, pssh->snd_blk);
			...
			while (pssh->len_snd_blk < blk_size)
			{
				...
				sha512_update(&sha4, K, len_K);
				sha512_update(&sha4, pssh->kex_H, pssh->len_H);
				sha512_update(&sha4, pssh->snd_blk, pssh->len_snd_blk);
				sha512_finish(&sha4, pssh->snd_blk + pssh->len_snd_blk);
		...
		if (pssh->type == SSH_TYPE_SERVER)
		{
			...
			sha512_update(&sha4, K, len_K);
			sha512_update(&sha4, pssh->kex_H, pssh->len_H);
			sha512_update(&sha4, "C", 1);
			sha512_update(&sha4, pssh->pid_sess, pssh->len_sess);
			sha512_finish(&sha4, pssh->rcv_key);
			...
			while (pssh->len_rcv_key < key_size)
			{
				...
				sha512_update(&sha4, K, len_K);
				sha512_update(&sha4, pssh->kex_H, pssh->len_H);
				sha512_update(&sha4, pssh->rcv_key, pssh->len_rcv_key);
				sha512_finish(&sha4, pssh->rcv_key + pssh->len_rcv_key);
			...
			sha512_update(&sha4, K, len_K);
			sha512_update(&sha4, pssh->kex_H, pssh->len_H);
			sha512_update(&sha4, "D", 1);
			sha512_update(&sha4, pssh->pid_sess, pssh->len_sess);
			sha512_finish(&sha4, pssh->snd_key);
			...
			while (pssh->len_snd_key < key_size)
			{
				...
				sha512_update(&sha4, K, len_K);
				sha512_update(&sha4, pssh->kex_H, pssh->len_H);
				sha512_update(&sha4, pssh->snd_key, pssh->len_snd_key);
				sha512_finish(&sha4, pssh->snd_key + pssh->len_snd_key);
		...
		if (pssh->type == SSH_TYPE_SERVER)
		{
			...
			sha512_update(&sha4, K, len_K);
			sha512_update(&sha4, pssh->kex_H, pssh->len_H);
			sha512_update(&sha4, "E", 1);
			sha512_update(&sha4, pssh->pid_sess, pssh->len_sess);
			sha512_finish(&sha4, pssh->rcv_mac);
			...
			while (pssh->len_rcv_mac < mac_size)
			{
				...
				sha512_update(&sha4, K, len_K);
				sha512_update(&sha4, pssh->kex_H, pssh->len_H);
				sha512_update(&sha4, pssh->rcv_mac, pssh->len_rcv_mac);
				sha512_finish(&sha4, pssh->rcv_mac + pssh->len_rcv_mac);
			...
			sha512_update(&sha4, K, len_K);
			sha512_update(&sha4, pssh->kex_H, pssh->len_H);
			sha512_update(&sha4, "F", 1);
			sha512_update(&sha4, pssh->pid_sess, pssh->len_sess);
			sha512_finish(&sha4, pssh->snd_mac);
			...
			while (pssh->len_snd_mac < mac_size)
			{
				...
				sha512_update(&sha4, K, len_K);
				sha512_update(&sha4, pssh->kex_H, pssh->len_H);
				sha512_update(&sha4, pssh->snd_mac, pssh->len_snd_mac);
				sha512_finish(&sha4, pssh->snd_mac + pssh->len_snd_mac);
	...
	{
		pssh->snd_hmac = xmem_alloc(sizeof(sha256_context));
	}
	else if (a_xsicmp(snd_hmac, SSH_HMAC_SHA2_512) == 0)
	{
		pssh->snd_hmac = xmem_alloc(sizeof(sha512_context));
	}
	...
	{
		pssh->rcv_hmac = xmem_alloc(sizeof(sha256_context));
	}
	else if (a_xsicmp(rcv_hmac, SSH_HMAC_SHA2_512) == 0)
	{
		pssh->rcv_hmac = xmem_alloc(sizeof(sha512_context));
	}
...
void  xssh_close(xhand_t ssh)
{
	...
	XDK_ASSERT(ssh && ssh->tag == _HANDLE_SSH);

	XDK_ASSERT(pso->type == SSH_TYPE_CLIENT || pso->type == SSH_TYPE_SERVER);
