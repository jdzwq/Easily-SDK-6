/***********************************************************************
	...
	@doc xdu display context document
...
*/
EXP_API visual_t create_display_context(widget_t wt);
...
@INPUT widget_t wnd: the printer setup dialog owner window.
@OUTPUT dev_prn_t* pmod: for returning device mode.
@RETURN bool_t: if succeeds return nonzero, fails return zero.
*/
EXP_API bool_t setup_printer_mode(widget_t wnd, dev_prn_t* pmod);
