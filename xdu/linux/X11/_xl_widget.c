/***********************************************************************
	...
	@module	xl_widget.c | X11 implement file
...
vword_t xlWidgetDelUserProp(widget_t wt, const tchar_t* pname)
{
    ...
    rt = xlWidgetGetUserProp(wt, pname);
	ua = XInternAtom (g_display, pname, False);
...
void xlWidgetCenterWindow(widget_t wt, widget_t owner)
{
    ...
    if (owner)
    {
        xlWidgetGetWindowRect(owner, &owner_rect);
    }
    else
    {
		owner_rect.x = 0;
...
void xlWidgetSetScrollInfo(widget_t wt, bool_t horz, const scroll_t* psl)
{
	...
	if(horz)
	{
		...
		xlWidgetEnumChild(wt, (PF_ENUM_WINDOW_PROC)_update_horz_position, (vword_t)&b);
	}
	else
	{
		...
		xlWidgetEnumChild(wt, (PF_ENUM_WINDOW_PROC)_update_vert_position, (vword_t)&b);
	}
...
{
	dword_t dw = xlWidgetGetStyle(wt);
	
	if (dw & WD_STYLE_NOCHANGE) return 1;

	xlWidgetSetColorMode(wt, (const color_mod_t*)pv);
...
void xlWidgetSetColorMode(widget_t wt, const color_mod_t* pclr)
{
	...
	if (!(pxw->style & WD_STYLE_NOCHANGE))
	{
		...
		xlWidgetSendCommand(wt, COMMAND_COLOR, IDC_SELF, (vword_t)pclr);
	}

	xlWidgetEnumChild(wt, _widget_set_child_color_mode, (vword_t)pclr);
}
