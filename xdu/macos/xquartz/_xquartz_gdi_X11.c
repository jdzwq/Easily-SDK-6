/***********************************************************************
	...
	@module	xquartz_gdi.c | GDI xquartz implement file
...
void xqGdiInit(int osv)
{
	...
	default_xfont(&xf);
	g_fontset = xqGdiCreateFontset(&xf);
}

void xqGdiUnInit(void)
{
	if(g_fontset) xqGdiDestroyFontset(g_fontset);
...
void xqGdiSetXFont(visual_t rdc, const xfont_t* pxf)
{
	...
	xquartz_fontset_t* fnt = (rdc)? TypePtrFromHead(xquartz_fontset_t, ctx->fontset) : NULL;
	XftFont* xft_font;
	char* family = NULL;
	double size = 0.0;
	int weight = 0;
	...
	if(g_fontset)
	{
		xqGdiDestroyFontset(g_fontset);
		g_fontset = NULL;
	}

    g_fontset = xqGdiCreateFontset(pxf);
...
void xqGdiDrawCurve(visual_t rdc, const xpen_t* pxp, const xpoint_t* ppt, int pn)
{
   int n, fs, ds;
	...
	parse_xcolor(&xc, pxp->color);
	xqGdiDrawPoints(rdc, &xc, ppt_buf, n);
...
void xqGdiDrawPath(visual_t rdc, const xpen_t* pxp, const xbrush_t* pxb, const tchar_t* aa, const xpoint_t* pa, int pn)
{
	...
	while (*aa || pn)
	{
		...
		else if (*aa == _T('L'))
		{
			...
			xqGdiDrawLine(rdc, pxp, &pt[0], &pt[1]);
			n = 1;
		}
		else if (*aa == _T('l'))
		{
		...
		else if (*aa == _T('C'))
		{
			...
			xqGdiDrawBezier(rdc, pxp, &pt[0], &pt[1], &pt[2], &pt[3]);
			n = 3;
		}
		else if (*aa == _T('c'))
		{
		...
		else if (*aa == _T('A'))
		{
			...
			xqGdiDrawArc(rdc, pxp, &pt[0], &pt[1], &xs, sflag, lflag);
			n = 3;
		}
...
void xqGdiDrawTriangle(visual_t rdc, const xpen_t* pxp, const xbrush_t* pxb, const xrect_t* pxr, const tchar_t* orient)
{
	xquartz_context_t* ctx = TypePtrFromHead(xquartz_context_t, rdc);
	...
	xqGdiDrawLine(rdc, pxp, &pt[0], &pt[1]);
	xqGdiDrawLine(rdc, pxp, &pt[1], &pt[2]);
	xqGdiDrawLine(rdc, pxp, &pt[2], &pt[0]);
}
...
void xqGdiDrawRound(visual_t rdc,const xpen_t* pxp,const xbrush_t* pxb,const xrect_t* prt,const xsize_t* pxs)
{
	...
	xqGdiDrawPath(rdc, pxp, pxb, ta, pa, 16);
}
...
void xqGdiDrawSector(visual_t rdc, const xpen_t* pxp, const xbrush_t* pxb, const xpoint_t* ppt, const xspan_t* prl, const xspan_t* prs, double arcf, double arct)
{
	xquartz_context_t* ctx = TypePtrFromHead(xquartz_context_t, rdc);
	...
	xqGdiDrawPath(rdc, pxp, pxb, ta, pa, 8);
}

void xqGdiDrawText(visual_t rdc,const xface_t* pxa,const xrect_t* prt,const tchar_t* txt,int len)
{
	xquartz_context_t* ctx = TypePtrFromHead(xquartz_context_t, rdc);
	xquartz_fontset_t* fst = TypePtrFromHead(xquartz_fontset_t, ctx->fontset);
	XftFont* xft_font;
	XftDraw* xft_draw;
	XftColor xft_color;
...
{
    xquartz_context_t* ctx = TypePtrFromHead(xquartz_context_t, rdc);
	xquartz_bitmap_t* bmp = (xquartz_bitmap_t*)rbm;
...
/*
void xqGdiFillRegion(visual_t rdc, const xbrush_t* pxb, res_rgn_t rgn)
{
    xquartz_context_t* ctx = TypePtrFromHead(xquartz_context_t, rdc);
	xcolor_t xc;
	XColor clr;
...
fontset_t xqGdiCreateFontset(const xfont_t* pxf)
{
	...
	fst = (xquartz_fontset_t*)xmem_alloc_handle(sizeof(xquartz_fontset_t));
...
{
	xquartz_fontset_t* fst = TypePtrFromHead(xquartz_fontset_t, ft);
