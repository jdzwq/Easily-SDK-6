/***********************************************************************
	...
	@module	xquartz_widget.c | xquartz implement file
...
xquartz_atoms_t  g_atoms = {0};

static int xquartz_to_keycode(int xk)
{
    switch (xk)
    {
...
static xquartz_widget_t* GETXDUSTRUCT(Window win)
{
    ...
    return (xquartz_widget_t*)GET_VOID_NET(bys, 0);
}

static void SETXDUSTRUCT(Window win, xquartz_widget_t* p)
{
    byte_t bys[VOID_SIZE] = {0};
...
static bool_t _MessageTranslate(XEvent* pmsg)
{
	xquartz_widget_t* pxw;
	accel_table_t* pac;
	char keystr[5] = {0};
	KeySym keysys = 0;
	Status status = 0;
...
static int _MessageDispatch(XEvent* pmsg)
{
	xquartz_widget_t* pxw;
	widget_t wt;
	if_dispatch_t* pif;
...
static void _MessageFetch(XEvent* pmsg, Window win)
{
...
static bool_t _MessagePeek(XEvent* pmsg)
{
...
{
	xquartz_widget_t* pxw_par = (wparent)? TypePtrFromHead(xquartz_widget_t, wparent) : NULL;
	widget_t wt;
    Window win, rot, par;
	...
	pxw = (xquartz_widget_t*)xmem_alloc_handle(sizeof(xquartz_widget_t));

	pxw->head.tag = _HANDLE_WIDGET;
	pxw->parent = (par == rot)? 0 : par;
	pxw->self = win;
	pxw->style = wstyle;
...
{
	xquartz_widget_t* pxw = TypePtrFromHead(xquartz_widget_t, wt);
	xquartz_widget_t* pxw_owner = TypePtrFromHead(xquartz_widget_t, owner);
...
vword_t xqWidgetDelUserProp(widget_t wt, const tchar_t* pname)
{
    ...
    rt = xqWidgetGetUserProp(wt, pname);
	ua = XInternAtom (g_display, pname, False);
...
void xqWidgetCenterWindow(widget_t wt, widget_t owner)
{
    ...
    if (owner)
    {
        xqWidgetGetWindowRect(owner, &owner_rect);
    }
    else
    {
		owner_rect.x = 0;
...
{
	xquartz_widget_t* pxw = TypePtrFromHead(xquartz_widget_t, (widget_t)pa);
...
void xqWidgetShow(widget_t wt, dword_t sw)
{
	...
	case WS_SHOW_MAXIMIZE:
		...
		XSendEvent(g_display, RootWindow(g_display, DefaultScreen(g_display)), False, SubstructureRedirectMask | SubstructureNotifyMask, (XEvent*)&ev);
...
{
	xquartz_widget_t* pxw = (wt)? TypePtrFromHead(xquartz_widget_t, wt) : NULL;
...
void xqWidgetSetScrollInfo(widget_t wt, bool_t horz, const scroll_t* psl)
{
	...
	if(horz)
	{
		...
		xqWidgetEnumChild(wt, (PF_ENUM_WINDOW_PROC)_update_horz_position, (vword_t)&b);
	}
	else
	{
		...
		xqWidgetEnumChild(wt, (PF_ENUM_WINDOW_PROC)_update_vert_position, (vword_t)&b);
	}
...
{
	dword_t dw = xqWidgetGetStyle(wt);
	
	if (dw & WD_STYLE_NOCHANGE) return 1;

	xqWidgetSetColorMode(wt, (const color_mod_t*)pv);
...
void xqWidgetSetColorMode(widget_t wt, const color_mod_t* pclr)
{
	...
	if (!(pxw->style & WD_STYLE_NOCHANGE))
	{
		...
		xqWidgetSendCommand(wt, COMMAND_COLOR, IDC_SELF, (vword_t)pclr);
	}

	xqWidgetEnumChild(wt, _widget_set_child_color_mode, (vword_t)pclr);
}
...
int	xqWidgetDoMain(widget_t wt)
{
	...
	while(1/*_widget_is_valid(wt)*/)
    {
		while (XPending(g_display))
		{
			_MessageFetch(&msg, (Window)0);
			...
			if (_MessageTranslate(&msg))
				continue;

			_MessageDispatch(&msg);
		}
