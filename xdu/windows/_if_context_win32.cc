...
int _font_size(visual_t rdc, int height)
{
	...
	size = ROUNDINT((float)(height - tm.tmInternalLeading) * 72.0 / (float)GetDeviceCaps(hDC, LOGPIXELSY));
...
void _cast_pt_to_mm(visual_t rdc, bool_t horz, xspan_t* pxn)
{
	...
	if (horz)
		mm = (float)((float)pxn->s / htpermm);
	else
		mm = (float)((float)pxn->s / vtpermm);
...
void _cast_mm_to_pt(visual_t rdc, bool_t horz, xspan_t* pxn)
{
	...
	if (horz)
		pt = ROUNDINT(pxn->fs * htpermm);
	else
		pt = ROUNDINT(pxn->fs * vtpermm);
