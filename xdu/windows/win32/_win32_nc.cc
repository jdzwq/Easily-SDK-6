...
static void _WidgetDrawLogo(visual_t rdc, const xcolor_t* pxc, const xrect_t* prt)
{
	...
	winGdiDrawRound(rdc, &xp, &xb, &xr, NULL);

	xr.x = prt->x + prt->w / 2 + 1;
	xr.y = prt->y;
	xr.w = prt->w / 2 - 2;
	xr.h = prt->h / 2 - 2;
	winGdiDrawRect(rdc, &xp, &xb, &xr);
...
static void _WidgetDrawEdge(widget_t wt, visual_t rdc, const xrect_t* prt)
{
	...
	ws = winWidgetGetStyle(wt);
	winCalcWidgetBorder(ws, &bd);
	...
	winGdiExcludeRect(rdc, &rtScr);

	winWidgetGetColorMode(wt, &clrs);
	default_xpen(&xp);
	format_xcolor(&clrs.clr_bkg, xp.color);
	default_xbrush(&xb);
	format_xcolor(&clrs.clr_bkg, xb.color);

	winGdiDrawRect(rdc, &xp, &xb, prt);
}
...
static void _WidgetDrawHScroll(widget_t wt, visual_t rdc, const xrect_t* prt)
{
	...
	dword_t wstyle = winWidgetGetStyle(wt);

	winCalcWidgetBorder(wstyle, &bd);

	if (!bd.scrh)
		return;

	winWidgetGetScrollInfo(wt, 1, &sl);
	...
	winGdiDrawRect(rdc, NULL, &xb, &rtScr);
	...
	if (sl.max + sl.page / 2 > sl.min)
	{
		...
		winGdiDrawRound(rdc, &xp, &xb, &rtScr, NULL);
		...
		winGdiDrawEllipse(rdc, &xp, &xb, &rtScr);
	}
}
...
static void _WidgetDrawVScroll(widget_t wt, visual_t rdc, const xrect_t* prt)
{
	...
	dword_t ws = winWidgetGetStyle(wt);
	winCalcWidgetBorder(ws, &bd);

	if (!bd.scrw)
		return;

	winWidgetGetScrollInfo(wt, 0, &sl);
...
static void _WidgetDrawTitleBar(widget_t wt, visual_t rdc, const xrect_t* prt)
{
	...
	winGdiDrawPath(rdc, &xp_shadow, &xb_shadow, aa, pa, n);
	...
	/*caption*/
	len = winWidgetGetTitle(wt, txt, RES_LEN);
	if (len)
	{
		...
		winGdiDrawText(rdc, &xa, &rtScr, txt, len);
	}
	...
	if (ws & WD_STYLE_SIZEBOX)
	{
		...
		rtScr.y += rtScr.h / 2;
		rtScr.h /= 2;
		winGdiDrawRect(rdc, &xp, &xb, &rtScr);
...
int winWidgetNcCalcScroll(widget_t wt, bool_t horz, const xpoint_t* pxp)
{
	...
	if (horz)
	{
		winWidgetGetScrollInfo(wt, 1, &sc);
	...
	{
		winWidgetGetScrollInfo(wt, 0, &sc);
