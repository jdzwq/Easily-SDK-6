...
LRESULT CALLBACK XdcWidgetProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_NCPAINT:
		...
		winWidgetGetWindowRect(widget, &xr);
		xr.x = xr.y = 0;
		winWidgetNcDrawFrame(widget, (visual_t)&(wct.head), &xr);
	...
	case WM_NCHITTEST:
		...
		xp.x = LOWORD(lParam);
		xp.y = HIWORD(lParam);
		hit = winWidgetNcHintTest(widget, &xp);
	...
	case WM_NCLBUTTONUP:
		...
		{
			scroll_t scr = { 0 };
			winWidgetGetScrollInfo(widget, 1, &scr);
		...
		}else if(hit == HINT_HSCROLL)
		{
			...
			xp.x -= rt.left;
			xp.y -= rt.top;
			int pos =winWidgetNcCalcScroll(widget, 1, &xp);
		...
		}else if(hit == HINT_VSCROLL)
		{
			...
			xp.x -= rt.left;
			xp.y -= rt.top;
			int pos =winWidgetNcCalcScroll(widget, 0, &xp);

			scroll_t scr = { 0 };
			winWidgetGetScrollInfo(widget, 0, &scr);
			if (pos != scr.pos)
			{
	...
	case WM_CREATE:
		...
		pws = (win32_widget_t*)xmem_alloc_handle(sizeof(win32_widget_t));
	...
	case WM_MOUSEWHEEL:
		...
		{
			(*pev->pf_on_wheel)(widget, (bool_t)0, (int)GET_WHEEL_DELTA_WPARAM(wParam));
			return 0;
		}
	...
	case WM_MOUSEHWHEEL:
		...
		if (pev && pev->pf_on_wheel)
		{
			(*pev->pf_on_wheel)(widget, (bool_t)1, (int)GET_WHEEL_DELTA_WPARAM(wParam));
			return 0;
		}
		else if (pev && pev->pf_on_scroll)
		{
	...
	case WM_SYSCOMMAND:
		...
		if ((wParam & 0xFFF0) == SC_CONTEXTHELP && pev && pev->pf_on_syscmd_click)
		{
			...
			xp.x = (int)(short)LOWORD(lParam);
			xp.y = (int)(short)HIWORD(lParam);
			(*pev->pf_on_syscmd_click)(widget, &xp);
			return 0;
		}
		break;
	case WM_TIMER:
...
LRESULT CALLBACK XdcSubclassProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam, UINT_PTR uIdSubclass, DWORD_PTR dwRefData)
{
	...
	case WM_MOUSEWHEEL:
		...
		{
			if ((*pev->sub_on_wheel)(widget, (bool_t)0, (int)GET_WHEEL_DELTA_WPARAM(wParam), (uid_t)uIdSubclass, pev->delta))
				return 0;
		}
		else if (pev && pev->sub_on_scroll)
		{
			...
			if (GET_WHEEL_DELTA_WPARAM(wParam) < 0)
			{
				if ((*pev->sub_on_scroll)(widget, (bool_t)0, scr.nMin, (uid_t)uIdSubclass, pev->delta))
					return 0;
			}
			else
			{
	...
	case WM_MOUSEHWHEEL:
		if (pev && pev->sub_on_wheel)
		{
			if ((*pev->sub_on_wheel)(widget, (bool_t)1, (int)GET_WHEEL_DELTA_WPARAM(wParam), (uid_t)uIdSubclass, pev->delta))
				return 0;
		}
		else if (pev && pev->sub_on_scroll)
		{
			...
			if (GET_WHEEL_DELTA_WPARAM(wParam) > 0)
			{
				if ((*pev->sub_on_scroll)(widget, (bool_t)1, scr.nMin, (uid_t)uIdSubclass, pev->delta))
					return 0;
			}
			else
			{
	...
	case WM_SYSCOMMAND:
		if ((wParam & 0xFFF0) == SC_MOUSEMENU && pev && pev->sub_on_sysclr_click)
		{
			...
			xp.x = (int)(short)LOWORD(lParam);
			xp.y = (int)(short)HIWORD(lParam);
			if ((*pev->sub_on_sysclr_click)(widget, &xp, (uid_t)uIdSubclass, pev->delta))
				return 0;
		}
		else if ((wParam & 0xFFF0) == SC_CONTEXTHELP && pev && pev->sub_on_syslog_click)
		{
			...
			xp.x = (int)(short)LOWORD(lParam);
			xp.y = (int)(short)HIWORD(lParam);
			if ((*pev->sub_on_syslog_click)(widget, &xp, (uid_t)uIdSubclass, pev->delta))
				return 0;
		}
		break;
	case WM_TIMER:
...
widget_t winWidgetCreate(const tchar_t* wname, dword_t wstyle, const xrect_t* pxr, widget_t wparent, const if_dispatch_t* pev)
{
	...
	HWND hParent = (wparent)? ((win32_widget_t*)wparent)->self : NULL;
...
#else
	b = GetWindowSubclass(pws->self, (SUBCLASSPROC)XdcSubclassProc, (UINT_PTR)sid, &p);
	if (b) return 0;

	b = SetWindowSubclass(pws->self, (SUBCLASSPROC)XdcSubclassProc, (UINT_PTR)sid, (DWORD_PTR)0);
	if (!b) return 0;
...
void winWidgetDelSubproc(widget_t wt, uid_t sid)
{
	...
	RemoveWindowSubclass(pws->self, (SUBCLASSPROC)XdcSubclassProc, (UINT_PTR)sid);
...
void winWidgetSetAccel(widget_t wt, const accel_table_t* pact, int n)
{
	win32_widget_t* pws = (win32_widget_t*)wt;
...
{
	win32_widget_t* pws = (win32_widget_t*)wt;
	win32_widget_t* pws_owner = (win32_widget_t*)owner;
...
void winWidgetSetUserId(widget_t wt, uid_t uid)
{
	...
	if (GetWindowLongPtr(pws->self, GWL_STYLE) & WS_CHILD)
		SetWindowLongPtr(pws->self, GWLP_ID, (LONG_PTR)uid);
...
uid_t winWidgetGetUserId(widget_t wt)
{
	...
	if (GetWindowLongPtr(pws->self, GWL_STYLE) & WS_CHILD)
		return (uid_t)GetWindowLongPtr(pws->self, GWLP_ID);
	else
		return pws->uid;
}
...
void winWidgetSetUserProp(widget_t wt, const tchar_t* pname, vword_t pval)
{
	win32_widget_t* pws = (win32_widget_t*)wt;
...
bool_t winWidgetIsOwnerNc(widget_t wt)
{
	...
	return (pws->style & WD_STYLE_OWNERNC)? 1 : 0;
}
...
bool_t winWidgetEnumChild(widget_t wt, PF_ENUM_WINDOW_PROC pf, vword_t pv)
{
	...
	return (bool_t)EnumChildWindows(pws->self, (WNDENUMPROC)_enum_child_window, (LPARAM)&ep);
}
...
void winWidgetCenterWindow(widget_t wt, widget_t owner)
{
	...
	bChild = GetWindowLong(pws->self, GWL_STYLE) & WS_CHILD;
	if (bChild)
	{
	...
	SetWindowPos(pws->self, NULL, rtChild.left, rtChild.top, rtChild.right - rtChild.left, rtChild.bottom - rtChild.top, SWP_NOACTIVATE | SWP_NOSIZE | SWP_NOZORDER | SWP_NOREDRAW);
}

void winWidgetSetCursor(widget_t wt, int curs)
{
	switch (curs)
	{
	case CURSOR_SIZENS:
...
bool_t winWidgetKeyState(widget_t wt, int key)
{
	int ks;
...
void winWidgetMove(widget_t wt, const xpoint_t* ppt)
{
	...
	SetWindowPos(pws->self, NULL, rt.left, rt.top, rt.right - rt.left, rt.bottom - rt.top, SWP_NOSIZE | SWP_NOACTIVATE | SWP_NOZORDER | SWP_NOREDRAW);
}
...
void winWidgetSize(widget_t wt, const xsize_t* pxs)
{
	...
	SetWindowPos(pws->self, NULL, rt.left, rt.top, rt.right - rt.left, rt.bottom - rt.top, SWP_NOMOVE | SWP_NOACTIVATE | SWP_NOZORDER | SWP_NOREDRAW);
}
...
void winWidgetTake(widget_t wt, int zor)
{
	...
	SetWindowPos(pws->self, wnd, 0, 0, 0, 0, SWP_NOSIZE | SWP_NOMOVE | SWP_NOACTIVATE | SWP_NOREDRAW);
}
...
void winWidgetShow(widget_t wt, dword_t sw)
{
	...
	else if (sw == WS_SHOW_FULLSCREEN)
	{
		...
		AdjustWindowRect(&rt, (DWORD)GetWindowLongPtr(pws->self, GWL_STYLE), 0);
...
void winWidgetSetTitle(widget_t wt, const tchar_t* token)
{
	...
	dw = (DWORD)GetWindowLongPtr(pws->self, GWL_STYLE);
...
void winWidgetSetScrollInfo(widget_t wt, bool_t horz, const scroll_t* psl)
{
	...
	if(horz)
	{
		...
		winWidgetEnumChild(wt, (PF_ENUM_WINDOW_PROC)_update_horz_position, (vword_t)&b);
	}
	else
	{
		...
		winWidgetEnumChild(wt, (PF_ENUM_WINDOW_PROC)_update_vert_position, (vword_t)&b);
	}
}

void winWidgetSetDiaph(widget_t wt, float b)
{
	win32_widget_t* pws = (win32_widget_t*)wt;
	DWORD dw;
	...
	SetWindowLong(pws->self, GWL_EXSTYLE, dw | WS_EX_LAYERED);
...
{
	dword_t dw = winWidgetGetStyle(wt);
		
	if (dw & WD_STYLE_NOCHANGE)
		return 1;

	winWidgetSetColorMode(wt, (const color_mod_t*)pv);
...
void winWidgetSetColorMode(widget_t wt, const color_mod_t* pclr)
{
	...
	winWidgetSendCommand(wt, COMMAND_COLOR, IDC_SELF, (vword_t)pclr);

	winWidgetEnumChild(wt, (PF_ENUM_WINDOW_PROC)_widget_set_child_color_mode, (vword_t)pclr);
}
...
void winMessagePosition(xpoint_t* ppt)
{
...
void winAdjustWidgetSize(dword_t ws, xsize_t* pxs)
{
#ifdef WINCE
	RECT rt = { 0 };
